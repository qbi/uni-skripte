...
Im Unterschied zu Universalprozessoren sind Spezialprozessoren
...
Computeranwendungen mit PCs oder Workstations. Sie sind nützlich für
...
ihrem Inneren leistungsfähige Rechner verborgen: \textbf{Eingebettete
Systeme}.
...
Einfache, kostengünstige Mikrocontroller (\textmu{}C) arbeiten mit 4/8Bit-Prozessoren
und die Codegröße nimmt den meisten Platz auf der Chipfläche ein und
dominiert damit die Kosten des \textmu{}C. Die Anforderungen an seine
Rechenleistung sind eher gering. Die Klasse der Hochleistungs-\textmu{}C
...
Telekommunikation. An diese \textmu{}C werden hohe Berechnungsanforderungen
gestellt, etwa in der Regelungstechnik oder der Signalverarbeitung.
Diese \textmu{}C bilden oft den Kern eines \emph{System-On-A-Chip}.
...
und bearbeiten mehrere 10\,000 Zeilen Hochsprachencode. Es werden
...
einzelnen Prozesse dennoch eingehalten werden können. Jedoch gibt es
...
\texttt{ASIPs} (\emph{application specific instruction processors}), für
...
Architektur (\texttt{RISC}, Parallelverarbeitung), den großen
Entwicklungsteams um diese Prozessoren sowie des Einsatzes neuester \texttt{VLSI-}Technologien. Für \emph{kundenspezifische Lösungen} spricht der
...
Signalerfassung (\texttt{OPTO-ASIC}).
...
ermöglicht die Verwendung von \texttt{CAE/CAD}-Werkzeugen, also die
...
Die \textbf{Anforderungsebene} stellt zunächst eine informelle Beschreibung
...
In der Anforderungsphase wird weiterhin ein Modell, oder eine Attrappe des
...
Auf der folgenden \textbf{Spezifikationsebene} wird eine \emph{detaillierte}
...
sein, oder als mathematische Beschreibung, etwa durch  \texttt{UML-}Diagramme, Kontroll- und Datenflussgraphen (\texttt{CDFG}),
Statecharts oder Automaten vorliegen.

Nun wird auf der \textbf{Architekturebene} der Plan für die gesamte
...
Blockdiagramm erfasst. Beispielsweise könnte für ein \texttt{GPS-}System
...
Die \textbf{Komponentenentwurfsebene} dient zur Abschätzung der
...
Schliesslich wird auf der \textbf{Systemintegrationsebene} eine
...
digitalen Signalprozessoren (\texttt{DSPs}) dar. Diese könnte man
als "`\emph{elektronische Systeme zur digitalen Signalverarbeitung}"'
definieren.
...
konvertiert.} oder \texttt{AD-}Wandlern.
...
Realzeit-Verarbeitung. \texttt{DSPs} müssen ihre Aufgaben effizient
ausführen bei gleichzeitiger Minimierung von Kosten, Leistungsverbrauch,
Speicherbedarf und Entwicklungszeit.

Da \texttt{DSP-}intensive Aufgaben heutzutage den Flaschenhals in vielen
Rechneranwendungen darstellen und unter Berücksichtigung des
Kostenfaktors sind \emph{general-purpose processors} (\gpps) nicht
zum Einsatz in diesem Bereich geeignet. \texttt{DSPs} werden gewöhnlich
...
in einigen Bereichen auch \gpps zunehmend Betätigungsfelder, etwa
Sprach- und Audiodatenkompression, Filterung, Modulation,
Spracherkennung oder Signalsynthese und v.\,m. Mit dem \texttt{LINGO} wurde
ein graphisches Format zur Darstellung von \texttt{DSP-}bezogenen Formeln
geschaffen.

\subsection{Algorithmen für digitale Signalprozessoren}

In digitalen Signalprozessoren sind Algorithmen fest verdrahtete, oder auch
...
werden jedoch immer mehr schnelle Multiplizierer, etwa der \texttt{Radix-4-} Multiplizierer\footnote{Vorlesung, Rechnerarchitektur I.}
...
\item \textbf{Konvergenzverfahren}
\end{enumerate}


\paragraph{Der \texttt{CORDIC-}Algorithmus} stellt ein solches
Konvergenzverfahren dar, dass auf dem Prinzip der
\emph{Koordinatentransformation} aufbaut. Die Idee ist, aufwendige
Multiplikationen, wie sie in den Reihenentwicklungen vorkommen, durch
\emph{einfache} Operationen, wie Addition, Schiebeoperationen und Vergleiche
zu ersetzen. \texttt{CORDIC-}Prozessoren sollen eine
Möglichkeit schaffen mit ein und derselben Hardwarestruktur eine Menge
von Standardfunktionen berechnen zu können.

Der Ansatz des CORDIC-Verfahrens ist die Tatsache, dass man bei der Drehung
des Vektors $(\begin{smallmatrix}1\\0\end{smallmatrix})$ um den
Winkel~$\theta$ den Vektor
$(\begin{smallmatrix}\cos\theta\\\sin\theta\end{smallmatrix})$ erhält -- also
\todo{Bildchen malen}
den Sinus und den Cosinus des Winkels~$\theta$. Diese Drehung kann man durch
Teildrehungen nachvollziehen, deren Berechnung so gewählt ist, dass sie ohne
Multiplikation bzw. nur mit Tabellenabfragen und Schiebeoperationen auskommt.

Die allgemeine Beziehung für die Drehung eines Vektor
$(\begin{smallmatrix}x\\y\end{smallmatrix})$ um einen Winkel~$\theta$ wird
durch die \autoref{eq:1} beschrieben. Zur Vereinfachung beschreibt man diese
mit nur einer Winkelfunktion; \autoref{eq:2}.
...
Die Drehung um den Winkel~$\theta$ zerlegt man in Drehungen um kleinere, fest
vorgegebene Winkel~$\alpha_{i}$ ($i=0,\dotsc,n-1$), so dass $\theta$ eine
Linearkombination der $\alpha_{i}$ ist; \autoref{eq:3}. Man beschreibt also
die Drehung schrittweise um fest vorgegebene Winkel, wobei man in mathematisch
negativer Richtung dreht, wenn man den Winkel~$\theta$ überschritten hat. Dies
wird durch den Vorfaktor $\sigma_{i}$ gesteuert. Die $\alpha_{i}$ wählt man
so, dass sich $\tan\alpha_{i} = 2^{-i}$ ergibt.

\begin{gather}
  \label{eq:3}
  \theta = \sum_{i=0}^{n-1} \sigma_{i} \cdot \alpha_{i}
...
\begin{align*}
  \begin{pmatrix}
    \cos(\beta+\gamma)& -\sin(\beta+\gamma)\\
    \sin(\beta+\gamma)& \cos(\beta+\gamma)
  \end{pmatrix}
  ={}&  \begin{pmatrix}
       \cos\beta& -\sin\beta\\
       \sin\beta& \cos\beta
     ...
     \begin{pmatrix}
       ...
       \sin\gamma& \cos\gamma
     \end{pmatrix}\\
  ={}&  \frac{1}{\sqrt{1+\tan^{2}\beta}} \cdot
     \begin{pmatrix}
       1& -\tan\beta\\
       \tan\beta& 1
     \end{pmatrix}\\
  &\cdot \frac{1}{\sqrt{1+\tan^{2}\gamma}} \cdot
     \begin{pmatrix}
       1& -\tan\gamma\\
       \tan\gamma& 1
     \end{pmatrix}
...
Verallgemeinert gilt also für die Drehung um einen zusammengesetzten
Winkel~$\theta=\sum_{i=0}^{n-1} \sigma_{i}\alpha_{i}$ die \autoref{eq:4}. Da
alle $\sigma_{i}\in\{-1,1\}$ sind und der Tangens symmetrisch zum
Koordinatenursprung ist, gilt
$\tan(\sigma_{i}\alpha_{i})=\sigma_{i}\tan\alpha_{i}$, und da die $\alpha_{i}$
so gewählt sind, dass $\tan\alpha=2^{-i}$ gilt, kann man die \autoref{eq:4} zu
\autoref{eq:5} umformen.
...
\begin{align}
  ...
  \end{pmatrix}
  &= \prod_{i=0}^{n-1}
     \frac{1}{\sqrt{1+\tan^{2}(\sigma_{i}\alpha_{i})}} \cdot
     \begin{pmatrix}
       1& -\tan(\sigma_{i}\alpha_{i})\\
       \tan(\sigma_{i}\alpha_{i})& 1
     \end{pmatrix} \cdot
     \begin{pmatrix}
       x\\y
     \end{pmatrix}\\
  \label{eq:5}
  % &= \underbrace{\prod_{i=0}^{n-1} \frac{1}{\sqrt{1+\tan^{2}\alpha_{i}}}}_{=K_{n}}
  %    \cdot \prod_{i=0}^{n-1}
  %    \begin{pmatrix}
  %      1& -\sigma_{i}\tan\alpha_{i}\\
  %      \tan\alpha_{i}& 1
  %    \end{pmatrix} \cdot
  %    \begin{pmatrix}
  %      x\\y
  %    \end{pmatrix}
  &= \underbrace{\prod_{i=0}^{n-1} \frac{1}{\sqrt{1+2^{-2i}}}}_{=k}
     \cdot \prod_{i=0}^{n-1}
     \begin{pmatrix}
       1& -\sigma_{i}2^{-i}\\
       \sigma_{i}2^{-i}& 1
...
Den Vorfaktor~$k$ kann man im Voraus berechnen und als Konstante abspeichern,
Da die Teilwinkel~$\alpha_{i}$ fest gewählt sind. Die Berechnung kann man
jetzt mit folgender Iterationsvorschrift beschreiben:
\begin{align*}
  x_{i+1}&= x_{i} - \sigma_{i} 2^{-i} y_{i}\\
  y_{i+1}&= y_{i} + \sigma_{i} 2^{-i} x_{i}
\end{align*}

Um die Drehrichtung -- also den Wert von $\sigma_{i}$ -- zu bestimmen, führt
man eine Hilfsvariable~$z_{i}$ (\autoref{eq:6}) ein, die den noch zu drehenden
Winkel misst. Ist dieser kleiner null -- wurde also zu weit gedreht --, muss
die kommende Drehung in die mathematisch negativer Richtung erfolgen --
$\sigma_{i}=-1$ --, andernfalls ist $\sigma_{i}=1$.

\begin{align}
  \label{eq:6}
  z_{0} &= \theta\\
  z_{i+1} &= z_{i} - \sigma_{i} \alpha_{i} = z_{i} - \sigma_{i} \arctan(2^{-i})
\end{align}

Das Vorgehen, bei dem man mit dem
Vektor~$(\begin{smallmatrix}1\\0\end{smallmatrix})$ beginnt und die
Hilfsvariable~$z_{i}$ auf null bringt, nennt man \textbf{Rotationsmodus}. Wenn
man mit einem Vektor $(\begin{smallmatrix}x\\y\end{smallmatrix})$ beginnt und
diesen "`auf die $x$"~Achse dreht"' -- formal bedeutet das, dass die
$y$"~Komponente auf null gebracht wird --, kann man so die Länge des Vektors
in der $x$"~Komponente und den Winkel zur $x$"~Achse in der $z$"~Komponente
bestimmen. Dieser Ablauf wird \textbf{Vektormodus} genannt.

\todo{Was über die Verallgemeinung auf lineares und hyperbolisches
...
\paragraph{Vergleich.} Vorteilhaft beim \texttt{CORDIC-}Algorithmus ist die
...
Logarithmus- oder Wurzelfunktion dem \texttt{CORDIC-}Algorithmus überlegen.
Die Einheitlichkeit der Hardwarestruktur ist jedoch bei ihm nicht
gegeben. 

\todo{Genauer erklären, warum Bitalgorithmen uneinheitlich sind. Eigentlich
  ist es auch immer die gleiche Iterationsvorschrift, bloß mit anderen
  Faktoren in der Iterationsvorschrift. Oder?}

\todo{Der
  CORDIC ist auch nicht so einheitlich, wenn man alle Modi berücksichtigt.
  Dann ergibt sich nämlich auch ein weiterer Nachteil, dass eine Multipikation
  mit dem Korrekturfaktor erforderlich ist.\\
  CORDIRC ist für trigonometrische Funktionen gut. Bitalgorithmen für
  Division, Multiplikation, Wurzel- und Logarithmusfunktion.}
...
und mathematische Operation ausführen können, so gilt für \texttt{ASIC},
...
der selbe Aufwand, wie für einen \texttt{ASIC}.
...
\item regelmäßigen konfigurierbaren Logikblöcken (\texttt{CLB}) und
...
Die \texttt{CLBs} werden durch \emph{Schaltmatrizen} gesteuert. Diese
...
besteht im \emph{rapid prototyping}, also dem Test von \texttt{ASICs}.
...
Für die \textbf{Systemspezifikation} gibt es zwei Methoden. Zum einen eine
...
Systementwurfsprache wie \texttt{VHDL}, \texttt{Verilog} oder \texttt{SystemC}.
...
Bei der \textbf{Hardwaresynthese} wird die Funktionspezifikation unter
Verwendung von \texttt{EDA-}Werkzeugen (\emph{electronic design
...
Layoutnetzliste erstellt werden. Durch \textbf{Verifikation}, also durch
...
für sogenannte \texttt{DISC}-Prozessoren (\emph{dynamic instruction set
computers}).
...
\paragraph{Definition. } Ein \textbf{Problemgraph} $G(V,E)$ ist
...
\paragraph{Definition. } Ein \textbf{Ressourcengraph} $G_R(V_R, E_R)$ ist
...
\paragraph{Definition. } Ein Paar $( G(V,E), G_R(V_R, E_R))$ heißt \textbf{Spezifikation}.
...
Es existieren zum einen der \texttt{ASAP}-Algorithmus und zum anderen der
\texttt{ALAP}-Algorithmus.  Die Idee des \texttt{ASAP}-A. ist jede Operation
so \emph{früh} wie möglich zu starten und liefert damit die gesuchte
\emph{minimale Latenz} $L^S$. Der \texttt{ALAP}- A. startet jede Operation
...
sich um Optimierungsprobleme zur Ablaufplanung, die \texttt{NP-hart} sind.
...
Verfahren dagegen ist das \texttt{ILP} (\emph{integer linear programming}).
Dabei wird aus den durch \texttt{ASAP/ALAP}-berechneten Werten ein
...
Zusammenhang spricht man von der \emph{Designkrise} und \texttt{IPs}
...
als IPs angeboten. Dabei werden Schaltungsblöcke, ein gesamtes Bauteil
...
\begin{itemize}
	...
	\item \textbf{L}atenzzeit $L$ ist die maximal benötigte Zeit um eine
		kleine Nachricht zu übertragen
	\item \textbf{o}verhead $o$ ist der Zeitbedarf für das Senden/Empfangen
		(beim Senden/Empfangen ist ein Prozessor gesperrt für andere
		Operationen)
	\item \textbf{g}ap $g$ ist untere Schranke für die Zeit, die jeder
		Prozessor bis zum Absenden einer neuen Nachricht abwarten muss
	\item \textbf{P}rozessoren $P$, jeder kann optional einen lokalen
		Speicher besitzen
\end{itemize}
...
ALUs zur Verfügung, wobei einzelne ALUs für ihre Aufgabe optimiert
	...
	\item Kontrolle einer zentralen Steuereinheit
		\begin{itemize}
			\item RE führen gleichzeitig die selbe Operation auf
				verschiedenen Daten aus (\textbf{SIMD})
		\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{datenparallele} Algorithmen
\end{itemize}
...
\begin{itemize}
	\item ILLIAC IV:
		\begin{itemize}
			...
			\item 64 parallele Verarbeitungseinheiten, Steuereinheit
				(SE)
			\item 40ns Taktzyklus, lokaler Speicher $2K\cdot64Bit$
	...
	\item MasParI und II.
		\begin{itemize}
			...
			\item Vorrechner, Steuerfeldeinheit, Feld aus 16384 PEs
			\item PE: 32Bit Architektur mit 4Bit Datenpfaden
			\item MP2: 32Bit Datenpfade
			\item lokales Netz: Verbindung der Nachbar-PEs
			\item globales Netz: 3-stufiges CLOS-Netzwerk
		\end{itemize}
\end{itemize}

\textbf{Neue Entwicklung: On-Chip SIMD-Architekturen}
...
\textbf{Beispiel III: Earth Simulator}
...
\begin{itemize}
	...
	\item \texttt{UMA} - \emph{uniform memory access model}:
		\begin{itemize}
		...
		\end{itemize}
	\item \texttt{NUMA} - \emph{non-uniform memory access model}: 
		\begin{itemize}
		...
		\end{itemize}
	\item Weiter Unterscheidung von \texttt{NUMA} bzgl Cache-Zugriff:
		\begin{itemize}
			\item \texttt{CC-NUMA}: \emph{cache coherent} für das gesamte
				System
			\item \texttt{NCC-NUMA}: \emph{non-cache coherent} - CC nur
				innerhalb eines Knotens
			\item \texttt{COMA}: \emph{cache only memory architecture}:
	...
	\item Traditional Directories:
		\begin{itemize}
			...
			\item Variante 3: n-pointers + rundruf; verbesserte variante
				2: zusätzlicher Platz für n Zahlen der ProzessorIDs, 
...
\begin{itemize}
	...
	\item für beide Varianten gilt: beim nächsten Zugriff des Prozessors
		...
		\end{itemize}
	\item Beispiel: \texttt{MESI}-Protokoll
		\begin{center}
		...
		\begin{itemize}
			...
			\item jede Cachezeile kann 4 Zustände annehmen:
				\begin{itemize}
					\item \textbf{M}odified: Zeile im Cache modifiziert,
						nur lokal verfügbar
					\item \textbf{E}xclusive: Zeile identisch mit
						Hauptspeichereintrag, nur lokal verfügbar
					\item \textbf{S}hared: Zeile identisch mit
						Hauptspeichereintrag, evtl. auf mehreren Caches
						verfügbar
					\item \textbf{I}nvalidate: Zeile enthälte ungültige
						Daten
				\end{itemize}
		\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item \texttt{NORMA}: \emph{no remote memory access model}
	\item \textbf\texttt{UCA}: \emph{uniform communication architecture}
		\begin{itemize}
			\item Zwischen allen Prozessoren können gleichlange
				Nachrichten mit einheitlicher Übertragungszeit gesendet
				werden.
		\end{itemize}
	\item \textbf\texttt{NUCA}: \emph{non uniform communication architecture}
		\begin{itemize}
...
\paragraph{Definition.} Ein \textbf{Cluster} ist ein \emph{paralleles,
...
\begin{itemize}
	\item \texttt{SSI}: (\emph{single systeme image})
		\begin{itemize}
			\item Darstellung eines \emph{einheitlichen} Systemabbildes
		\end{itemize}
	\item \texttt{SAI}; (\emph{system availability infrastructur})
		\begin{itemize}
...
\subsection {Gesetz von \texttt{Amdahl}}
...
\begin{itemize}
	...
	\item $T_O(n,p)=0$ !
		\begin{center}
			$T_1(n)=T_{seriell}(n) + T_{parallel}(n) = s \cdot T_1(n) +
			(1-s)\cdot T_P(n)$\\
...
\begin{itemize}
	...
	\item \emph{Speicherkopplung}:
		...
		\end{itemize}
	\item \emph{Kanalkopplung}: \texttt{DMA-}Kopplung:
		\begin{itemize}
	...
	\item \emph{Priorität}
		\begin{itemize}
			\item Rangordnungen von Stationen mit gleichzeitigem
				Sendewunsch
		\end{itemize}
	\item \textbf{FAZIT:} Flexibel, aber zu langsam und auch zu aufwendig
	...
	\item Routing im Hypercube
		\begin{itemize}
			...
			\item Maske $M=xor(SA,ZA)=11111$ 
				\begin{table}[h!]
					\centering
					\texttt{\begin{tabular}{|r|r|r|}\hline
						Maske & Zwischenadresse alt & Zwischenadresse
						neu\\\hline
						1111\textbf{1} & 0101\textbf{0} & 0101\textbf{1}\\
						111\textbf{1}1 & 010\textbf{1}1 & 010\textbf{0}1\\
						11\textbf{1}11 & 01\textbf{0}01 & 01\textbf{1}01\\
						1\textbf{1}111 & 0\textbf{1}101 & 0\textbf{0}101\\
						\textbf{1}1111 & \textbf{0}0101 & \textbf{1}0101\\\hline
								   &             & \\
								   &             & \textbf{10101}\\\hline
					\end{tabular}
					} % \tt
...
\begin{itemize}
	...
	\item Anzahl Kreuzschinenverteiler: $n\cdot\ld(n)-\frac{n}{2}$
	\item Pro Stufe $n/2$ Verteiler $\rightarrow$ $2\cdot\ld(n)-1$ Stufen
		nötig
\end{itemize}
...
\begin{itemize}
	\item Anzahl Stufen: $\ld(n)\cdot(1+\ld(n))/2$
...
% LaTeX_master_file: ../skript.latex
