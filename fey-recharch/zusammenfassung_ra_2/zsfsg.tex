...
\texttt{ASIP's} (\emph{application specific instruction processors}), für
...
digitalen Signalprozessoren (\texttt{DSP's}) dar. Diese könnte man
...
Realzeit-Verarbeitung. \texttt{DSP's} müssen ihre Aufgaben effizient
...
Kostenfaktors sind \emph{general-purpose processors} (\texttt{GPP's}) nicht
zum Einsatz in diesem Bereich geeignet. \texttt{DSP's} werden gewöhnlich
...
in einigen Bereichen auch \texttt{GPP's} zunehmend Betätigungsfelder, etwa
...
\subsection{Algorithmen für \texttt{DSP's}}

In \texttt{DSP's} sind Algorithmen fest-verdrahtete, oder auch
...
Die \texttt{CLB's} werden durch \emph{Schaltmatrizen} gesteuert. Diese
...
besteht im \emph{rapid prototyping}, also dem Test von \texttt{ASIC's}.
...
Zusammenhang spricht man von der \emph{Designkrise} und \texttt{IP's}
