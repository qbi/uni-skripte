...
\newcommand{\frage}[1]{
\begin{tabular}[]{ll}
	{\bf \textcolor{red}{ Frage:} } & {\sl \textcolor{red}{#1}}\\
\end{tabular}
...
\begin{itemize}
			...
			\item 6-Tupel $G=(P,T,F,K,W,M_0)$ mit
				\begin{itemize}
					...
					\item $T=\{t_1,t_2,\dots,t_n\}$ Menge der
						Transitioenn
					\item $K: P \longrightarrow N \cup \{\infty\}$ Kapazität der
						Stellen
					\item $W: F \longrightarrow N$ Gewicht der Kanten
						in $F$
					\item $M_0: P \longrightarrow N_0$
	...
	\item \emph{nicht statisch}, Veränderung, wenn sich Marken durch das
		Netz bewegen
	\item Zustand eines Netzes: $M: P \longrightarrow N_0$, sd. $M(p)$
	...
	\item Vor- und Nachbereich für Netzknoten $x$:\\ $*x =
		...
		\begin{itemize}
			\item Definition: \emph{Schaltbereitschaft} einer Transition
				...
				$M[t$>
				\begin{enumerate}
					\item $\forall p \in *t \setminus t*: M(p) \geq W(p,t)$ 
						\begin{itemize}
							\item {\sl alle p aus dem Vorbereich von t
								haben mind. die benötigten Markierungen
								der Kante von p nach t}
						\end{itemize}
					\item $\forall p \in t* \setminus *t: M(p) \leq
						K(p)-W(t,p)$ 
						\begin{itemize}
							\item {\sl in alle p aus dem Nachbereich von t
								passen mind. soviele Markierungen wie
								das Gewicht von $(t,p)$}
						\end{itemize}
					\item $\forall p \in t* \cap *t: (M(p) \leq K(p) -
						W(t,p) + W(p,t)) \land (M(p)+W(t,p) - W(p,t)
						\geq 0)$
						\begin{itemize}
							\item {\sl für alle p, die sowohl Vor- als auch
								Nachbereich sind genügt der Platz für
								die Differenz von eingehenden und
								ausgehenden Marken {\bf und} nach dem
...
\begin{itemize}
	...
	\item $\delta : Z \times X \rightarrow Z$
		Zustandsüberführungsfunktion
	\item $\gamma$: Ausgabefunktion - Unterscheidung in \emph{Mealy, Moore}-Automat:
		\begin{itemize}
			\item Mealy: $\gamma : Z \times X \rightarrow Y$
			\item Moore: $\gamma : Z \rightarrow Y$
		\end{itemize}
\end{itemize}
%
		...
		\begin{itemize}
			\item 5-Tupel $G=(V,E,cons,prod,d)$ mit
				\begin{itemize}
					...
					\item $cons: E \longrightarrow N_0$ (konsumierte
						Marken beim Feuern)
					\item $prod: E \longrightarrow N_0$ (produzierte
						Marken beim Feuern)
				...
				\begin{enumerate}
					...
					\item \emph{verhaltensorientiert} 
						\begin{itemize}
							...
							\item sequentiell (mit {\tt process}-Modifizierer)
	...
	\item {\bf Aufgabe:}
		\begin{itemize}
			\item Abbildung einer \emph{technologieunabhängigen}
				Optimierung einer Boolschen Netzliste {\bf auf} eine
...
\begin{itemize}
	\item mit Superzuständen und Subzuständen, sowie {\tt
		AND}-Dekomposition
...
\begin{itemize}
	\item enstehen bei Auflösung der {\tt AND}-Dekomposition von
		Subzuständen
...
%
\frage{Was besagt Moore's Gesetz und wie lange wird es noch gültig sein?
Warum?}
\begin{itemize}
...
\begin{itemize}
	\item {\bf 1-fach-Assoziativ (direct mapping)}
		\begin{itemize}
			\item jeder Adresse A eines Hauptspeicherblocks wird direkt ein
				Block B aus dem Cache zugewiesen
			\item Z.Bsp. $B=A\%N$, wobei N $\dots$ Anzahl der Cacheblöcke
		\end{itemize}
	\item {\bf n-fach assoziative Abbildung}
		\begin{itemize}
...
\begin{itemize}
	...
	\item Aufbau:
		\begin{itemize}
			\item {\tt | 24 Bit Tag | 6 Bit Index | 2 Bit Offset |}
