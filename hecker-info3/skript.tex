...
% rubber: clean $base.thm skript.alg
...
%%       - Beweise sind manchmal "verstreut", besser direkt zur
%%       jeweiligen Behauptung anfügen
%%       - Fehler im Textsatz ab Zeile 1490
...
\emph{Algorithmen} und \emph{Datenstrukturen}. Sie gehören zusammen
...
Während ein \highl{Algorithmus} die \emph{notwendigen Schritte zum Lösen
eines Problems} beschreibt, dienen \highl[Datenstruktur]{Datenstrukturen}
...
Wie so oft beginnt ein Vorhaben mit den zwei Fragen "`Was habe
ich?"' und "`Was möchte ich erhalten?"'. In \autoref{131003a}
...
\highl{Input}. Gesucht ist ein Algorithmus, der die zugehörigen
...
\highl{Segmentschnitt-Problem} zu betrachten. Eine
...
solches Segmentschnitt-Problem. Die triviale Idee, jede Strecke mit
jeder anderen zu vergleichen und somit alle möglichen Schnittpunkte zu
ermitteln, charakterisiert einen sogenannten
\highl{Brute-Force-Algorithmus}, der mittels erschöpfenden Probierens
zu einer Lösung gelangt. Solche Algorithmen sind im Allgemeinen sehr
ineffizient und tatsächlich benötigt dieses Verfahren
$n^2$~Vergleiche. Dabei ist $n$ die Anzahl der Rechteckkanten. Besteht
der Input nämlich nicht wie hier aus einigen wenigen Linien, sondern
sollen statt dessen eine Million Strecken untersucht werden, dann sind
bereits $10^{12}$~Vergleiche erforderlich.
...
der Eingabe \emph{und} der Länge der Ausgabe abhängig, man spricht
von einem \highl[Algorithmus!output-sensitiver]{output-sensitiven Algorithmus}.
...
Würde der Strahl stetig über die Segmente streichen, wäre die Zahl der
Prüfpunkte 
überabzählbar  und das Verfahren wäre nicht maschinell
...
Zu diesem Zweck definiert man eine Menge mit endlich vielen so genannten \highl{event
points} (\highl{Ereignispunkte}), an denen Schnitte gesucht werden sollen. Im
...
\begin{quote}
  Speichere die horizontalen Linien und prüfe beim Auftreten einer
  senkrechten Linie, ob sie sich mit einer aktuell gemerkten
  horizontalen schneidet.
\end{quote}
...
\begin{enumerate}
 ...
 \item Menge $Y \coloneqq \emptyset$  
  \begin{itemize}
   \item \texttt{INSERT} (Aufnahme) der horizontalen Strecken bzw.
   \item \texttt{DELETE} (Entfernen)
  \end{itemize}
  der zugehörigen $y$-Werte
 \item bei den event points (vertikale Strecken):
  \begin{itemize}
   \item \texttt{SEARCH} (Suche) im Vertikalintervall nach $y$-Werten aus der 
    Menge $Y$
...
\begin{definition}[Datenstruktur]
  ...
  werden, nennt man \highl{Datenstruktur}.
\end{definition}

\begin{definition}[Unterstützte Operationen]\label{ADT}
  Eine Operation heißt \highl[Operation!unterstützte]{unterstützt},
  wenn sie für eine Eingabe der Länge $n$ proportional nicht mehr als
  $\log(n)$~Teilschritte und somit proportional
  $\log(n)$~Zeiteinheiten benötigt.
\end{definition}

\begin{definition}[Abstrakter Datentyp (ADT)]
  Eine Datenstruktur, zusammen mit den von ihr unterstützten Operationen, 
  heißt \highl[Datentyp!abstrakter]{abstrakter Datentyp} (\highl{ADT}).
\end{definition}

\begin{definition}[Dictionary]
  Der abstrakte Datentyp, der das Tripel der Operationen \texttt{INSERT},
  \texttt{DELETE}, \texttt{SEARCH} unterstützt, heißt \highl{Dictionary}.
\end{definition}

Der obige \textsc{Plane-Sweep}-Algorithmus benötigt $\OO(n\log(n))$~Schritte
für das Sortieren (siehe unten $\OO$"=Notation). Da es sich um einen output-sensitiven
 ...
 \item[Maschinenmodell:] RAM (Random access machine)
  \begin{itemize}
   ...
   \item Operationen wie Addition, Subtraktion, Multiplikation und Division
    sind in \emph{einem} Schritt durchführbar
  \end{itemize}
...
\texttt{INSERT}, \texttt{DELETE} und \texttt{SEARCH} unterstützt, so
...
Das \highl[Handlungsreisenden!Problem des]{Problem des
  Handlungsreisenden}\index{Travelling Salesman}, manchmal auch mit \highl{TSP} abgekürzt, sieht wie folgt aus:
...
\begin{description}
 ...
 \item[NP:] Die Klasse der Probleme, die für eine Eingabe der Größe $n$
  \emph{nichtdeterministisch} in $Pol(n)$ Zeit gelöst werden können 
...
\begin{definition}[Effiziente Algorithmen]
  ...
  \highl[Laufzeit!polynomielle]{polynomielle Laufzeit}, heißt \highl[Algorithmus!effizienter]{effizient}.
...
\begin{bemerk}
  ...
  Inputreihenfolgen gleichwahrscheinlich sind.  Dieser Algorithmus
  braucht auch im \highl{average case} $\OO(n^2)$. Für das Sortieren
...
Des Weiteren gilt die folgende Regel: $(f_1(n)+ \cdots +f_m(n)) \in \OO
(\max\{f_1(n),\dotsc,f_m(n)\})$, mengentheoretisch ausgedrückt gilt
also: $\OO(f_1(n)) \cup \cdots \cup \OO(f_m(n)) = \OO
...
Für das gesamte Verfahren \textsc{MergeSort} ergibt sich die Rekurrenz $T(n)=2T(n/2)+\OO(n)$, die zu einer Komplexität von $\OO(n
...
Viele Algorithmen enthalten rekursive Aufrufe. Die Zeitkomplexität
solcher Algorithmen wird oft durch Rekurrenzen beschrieben. Dieses
Kapitel liefert vier Ansätze zum Lösen von Rekurrenzen.

\begin{definition}[Rekurrenzen]
  \highl[Rekurrenz]{Rekurrenzen} sind Funktionen, welche durch ihren
  eigenen Wert für kleinere Argumente beschrieben werden. Für den
  Trivialfall muß wie bei jeder rekursiven Funktion eine Lösung
  angegeben werden.
\end{definition}

Begonnen wird mit der Methode der vollständigen Induktion
(Substitutionsmethode), bei der zur Lösung von Rekurrenzgleichungen im
ersten Schritt eine mögliche Lösung der Gleichung erraten wird, die im
zweiten Schritt mittels vollständiger Induktion bestätigt werden muß.
...
Eingabe ist eine sortierte Folge $a_1 < \ldots < a_{n}$ und ein Wert
...
\begin{quote}
  Denke dir eine natürliche Zahl $a$ zwischen 0 und 1000. Wetten, daß
  ich mit 10 Fragen herausbekomme, welche Zahl du dir gedacht hast!
\end{quote}
...
$a_1$     & $a_2$           & \ldots   &  $ a_{\frac{n}{2}}$ & $ \vert a_{\frac{n}{2}+1}$ & \ldots  & $a_n$ \\
          &       ?         & $\geq$  & oder                  & $<$                          &  ?     &\\
...
\begin{definition}
  $a(i<j) \coloneqq \Set{(a_1, \ldots , a_n) | a_i \in S \wedge
  a_i < a_j}$
\end{definition}
...
\begin{figure}
 \centering
  ...
  level 3/.style={sibling distance=20mm}, level 4/.style={sibling distance=30mm}, every node/.style={circle,draw}]
    \node {$3<4$}
    child {node {$2<3$}
      child {node {$1<2$}
        child {node [rectangle,draw] {$1<2<3<4$}}
        child }
      child {node {$\mathcal{V}$}
        child
        child }}
    child {node {$1<2$}
      child {node {$2<4$}
        child {node [rectangle,draw] {$1<2<4<3$}}
        child {node {$4<1$}
          child {node {$2<3$}
            child {node [rectangle,draw] {$4<1<2<3$}}
            child {node {$1<3$}
              child {node [rectangle,draw] {$4<1<3<2$}}
              child {node [rectangle,draw] {$4<3<1<2$}}}}
          child }}
      child {node {$2<1$}
        child {node {$\,$}}
        child {node {$\mathcal{W}$}}}};
  \end{tikzpicture}
...
Von Interesse ist, ob $ g(x_1 , \ldots , x_n) < 0 $, wobei $ g(x_1 , \ldots , x_n) = c_1 x_1 + \cdots + c_n x_n + d $ mit
$ c_1, \ldots , c_n, d $ als Konstanten und $ x_1, \ldots, x_n  $ als Variablen.
...
Beim Elementtest ist eine Menge $\M$ mit $\M\subseteq\R^n$ sowie ein
variabler Vektor $(x_1, \ldots , x_n)$ gegeben. Es wird getestet, ob
$(x_1, \ldots , x_n) \in \M$, wobei $\M$ fest ist.
	
Bei der $\varepsilon$"=Closeness sieht die Fragestellung etwas anders
aus. Als Eingabe sind wieder $n$~reelle Zahlen $a_1, \ldots , a_n $
und $ \varepsilon > 0 $ gegeben. Von Interesse ist aber, ob es zwei
Zahlen in der Folge gibt, deren Abstand kleiner oder gleich
$\varepsilon$ ist.

Viel kürzer ist die mathematische Schreibweise: $\exists i,j (1 \leq
i, j \leq n)\colon\abs{a_i - a_j} < \varepsilon$?
...
geprüft, ob $\abs{a_i - a_{i+1}} < \varepsilon$ für $i<n$.

\begin{satz} 
  Wenn $\varepsilon$"=Closeness die Komplexität $\Omega(n \log n)$ hat,
  so auch das Sortieren.
\end{satz}		
		
\begin{satz}
  Die Menge $\M$ habe $m$~Zusammenhangskomponenten. Dann gilt, dass
  jeder Algorithmus im linearen Modell im Worst Case mindestens $\log
  m$~Schritte braucht, wenn er den Elementtest löst.
\end{satz}
		 	
\begin{beweis}
  Jeder Algorithmus $A$ im linearen Modell liefert einen
  Entscheidungsbaum mit Knoten, in denen für alle möglichen
  Rechenabläufe gefragt wird, ob $g(x_1, \ldots , x_n) < 0$
  ist. Jetzt genügt es zu zeigen, daß der Entscheidungsbaum mindestens
  soviele Blätter hat, wie die Menge $\M$
  Zusammenhangskomponenten. Mit dem eben bewiesenen folgt, daß dies
  äquivalent zu einer Höhe des Entscheidungsbaumes von $\log
  (\text{card}(\M))=\log m$ ist. Nun sei $b$ ein Blatt des Baumes.

  \begin{definition}
  $E(b) \coloneqq  \Set{\vec x \in \R^n |\text{Alg. }A \text{
      landet bei der Eingabe von } \vec x = (x_1, \ldots ,x_n) \text{
      in Blatt } b}$
  \end{definition}
  ...
  $ g(x_1$, & \ldots & , $ x_n) < 0 $ ? \\
  ja / & & $ \backslash $ nein\\
  $ g(x_1, \ldots, x_n) < 0 $ & & $ g(x_1, \ldots x_n) \geq 0 $
  \end{tabular}
...
\begin{definition}	
  $\K$ ist genau dann \highl{konvex}, wenn $\forall p,q\colon p\in \K
  \wedge q \in \K \rightarrow \overline{pq} \subseteq \K$.
\end{definition}
  
Nun  gilt $\R^n = \bigcup_{b \text{ist Blatt}} E(b)$ also
$\M = \R^n \cap \M = \bigcup_{b   Blatt} E(b) \cap \M = \ldots = \bigcup_{b \in \B} E(b) $
für eine bestimmte Teilmenge $\B$ aller Blätter des Entscheidungsbaumes. Weil jede Menge $E(b)$ zusammenhängend ist, kann
diese Vereinigung höchstens $\abs{B}$ viele Zusammenhangskomponenten haben. Die Anzahl aller Blätter des
...
Die Komplexität des Elementtests ist also $ \Omega (\log m) $	
  ...
  $ \M_{\varepsilon^i} \coloneqq  \lbrace ( a_1, \ldots , a_n ') \in \R^n \vert \forall   i \ne j : \vert a_i - a_j 
  \vert \geq \varepsilon \rbrace$ ist ein spezielles Elementtestproblem.
				
  $\pi$ sei eine Permutation $ \pi(1, \ldots ,n) $, dann ist
  $ \M(\pi ) \coloneqq  \lbrace (a_1, \ldots , a_n) \in \M \vert a_{\pi (1)} < a_{\pi (2)} < \ldots a_{\pi (n)}  \rbrace $
...
\textsc{MakeHeap}, \textsc{Insert}, \textsc{Max} und \textsc{ExtractMax} unterstützt heißt \highl{Heap} (oft auch \highl{Priority Queue} genannt).
 \end{definition}
...
Ein Verfahren heißt \highl{In place}, wenn es zur Bearbeitung der Eingabe unabhängig von der Eingabegröße nur zusätzlichen
...
\begin{definition}[Binärer Heap]
  Ein \highl[Max-Heap!binärer]{Binärer Max-Heap} ist ein spezieller
  Binärbaum (wird im Folgenden nochmals definiert) mit den
  Eigenschaften, daß der Wert jedes Knotens jeweils größer oder gleich den
  Werten seiner Söhne ist und daß außer dem Level mit der Höhe 0 alle
  Level voll besetzt sein müssen. Jeder Level wird von links beginnend
  aufgefüllt. Hat also ein Blatt eines Heaps die Höhe 1 im gesamten
  Heap, so haben auch alle rechts davon stehenden Blätter genau
  dieselbe Höhe.

  Dies ist äquivalent dazu, daß für eine Folge $F=k_{1}, k_{2},
  \ldots, k_{n}$ von Schlüsseln für alle $i$ mit $2\leq i\leq n$ die
  Beziehung $k_{i}\leq k_{\floor{ \nicefrac{i}{2}}}$ gilt
  (Heap-Eigenschaft), wobei kein Eintrag im Feld undefiniert sein
  darf.
\end{definition}
...
Ein \highl{Binärbaum} ist ein Baum, bei dem jeder Knoten vom Grad höchstens 3 ist. Ein Knoten mit
...
Ein \highl[Binärbaum!vollständiger]{vollständiger Binärbaum} hat zusätzlich die Eigenschaften, daß genau ein Knoten den Grad 2 besitzt und alle Blätter die
...
Dann nimmt es das letzte im Heap gespeicherte Blatt und setzt es als
die Wurzel ein. Mit dem Aufruf von Heapify() wird die
...
\begin{algorithm}
  \Begin{
\If{$key[x]>k$}{Fehler älterer Schlüsselwert ist größer}
$key[x]\leftarrow k$\;
$y\leftarrow x$\;
$z\leftarrow p[y]$\;
\While{$z\neq$ NIL AND $key[y]>key[z]$}
{tausche $key[y]$ und $key[z]$\;
$y\leftarrow z$\;
$z\leftarrow p[y]$}}
...
Initialisiere Struktur $V$\;
\While{$V\setminus W\neq\emptyset$}
{$v\leftarrow\min (V\setminus W)$\;
\FuncSty{ExtractMin}$(V\setminus W)$\;
$W\leftarrow W\cup\{v\}$\;
\For{$\forall w$ in \FuncSty{Succ} $(v)\setminus W$}
{\If{$d(r)+l(vw)<d(w)$}{\FuncSty{DecreaseKey} ($w, d(v)+l(vw)$)}}
}}
...
DecreaseKey, MergeSort, Heapify}, emphstyle=\textsc, escapeinside=~~}  
  ...
  Ein Sortierverfahren heißt \highl[Sortierverfahren!stabiles]{stabil}, falls mehrfach vorhandene Elemente in der Ausgabe in der Reihenfolge auftauchen, in der sie auch
...
\section{Weitere Sortieralgorithmen}
  ...
  \section{Binäre Suchbäume}
   ...
   \begin{definition}
   Ein Suchbaum heißt \highl{Blattsuchbaum}, falls die Elemente der dynamischen Menge im Gegensatz zum normalen Suchbaum nur in den
...
Allerdings haben Top down 2-3-4-Bäume auch den Nachteil, daß sie schwerer implementierbar sind als die 2-3-4 Bäume. 
    Dafür sind Rot-Schwarz-Bäume besser geeignet. %\todo{Wofür besser geeignet? Implementierung?}      

\section{Rot-Schwarz-Bäume}
 
\begin{definition}[Rot-Schwarz-Bäume]
\highl[Rot-Schwarz-Baum]{Rot-Schwarz-Bäume}  sind binäre Suchbäume mit folgenden zusätzlichen Eigenschaften:
      \begin{enumerate}
...
Dann gibt es jeweils für den rechten und den linken Teilbaum zwei Fälle. Dabei sind die Fälle für die beiden Teilbäume analog und
 ...
 \item Sei $\Bh(li[x]) = \Bh[x]-1$, dann gibt mindestens soviele
   ...
   damit\todo{Unten steht das Wort root im Mathesatz.}
...
\begin{algorithm}
  \While{Farbe von $p[z]=$ ROT}
  {\If{$p[z] = \text{links}[p[p[z]]]$ \tcp{Vater ist linker Sohn von Opa von $z$}}
  {$y\coloneqq[p[p[z]]]$ \tcp{$y$ ist Onkel von $z$}
    \If{Farbe$[y] =$ ROT \tcp{Vater und Onkel rot}}
    {Farbe$[p[z]] \coloneqq$ SCHWARZ \tcp{Vater wird schwarz}
      Farbe$[y] \coloneqq$ SCHWARZ \tcp{Onkel wird schwarz}
      Farbe$[p[p[z]]]\coloneqq$ ROT \tcp{Opa wird schwarz}
      $z\coloneqq p[p[z]]$}
    {\ElseIf{$z = rechts[p[z]]$}
  {$z\coloneqq p[z]$\;
    \FuncSty{Linksrotation}$(T, z)$\;
    \FuncSty{Farbe}$[p[z]]\coloneqq$ SCHWARZ\;
    \FuncSty{Farbe}$[p[p[z]]]\coloneqq$ ROT\;
    \FuncSty{Rechtsrotation}$(T, p[p[z]])$}
}
}
\Else{wie oben, recht und links vertauscht}
}
\FuncSty{Farbe}$[Root[T]]\coloneqq$ SCHWARZ
\caption{Korrigiere$(T,z)$}
\end{algorithm}
...
Dazu muß zuerst \highl{Optimalität} definiert werden. Hier ist damit folgendes gemeint:
 \begin{definition}
   Ein binärer Suchbaum heisst \highl[Suchbaum!optimaler]{optimal}, wenn die Summe $\sum_{i=1}^n p_i(t_i+1)$ minimal ist, wobei
...
\begin{itemize}
    \item Gesamtproblem $(a_1, \ldots,a_n)$, d.\,h. $n$~Schlüsselwerte mit den dazu gehörigen Wahrscheinlichkeiten $p_i$ für $i=1, \dotsc, n$
    \item Tiefen der Knoten $t_1,\ldots,t_n$
...
\end{description}
Dazu definiert man die Teilprobleme $(i,j)=(a_i,\ldots,a_j)$. Der Baum für das optimale Teilproblem wird mit $T(i,j)$
...
$s\leftarrow 2$\;
\While{$s\neq n$}
{$\alpha\leftarrow top[S]$\;
$\beta\leftarrow$ zweites Element aus $S$\;
\While{($p_{s+1}, \alpha,\beta$) keine Linksdrehung}
{\FuncSty{Pop}\;
$\alpha\leftarrow\beta$\;
$\beta\leftarrow$ neues zweites Element im Stapel}
\FuncSty{Push} ($S,p_{s+1}$)\;
$s\leftarrow s+1$}
Gib $S$ aus}
...
Diese wichtige Operation wird aber von \highl{Binomial-Heaps} unterstützt.
...
\Begin{
 ...
 $links[x]\coloneqq x$\;
 $rechts[x]\coloneqq x$\;
 $mark[x]\coloneqq false$ \tcp{Bis hier neue Wurzelliste nur aus $x$}
 Verknüpfe mit Wurzelliste $H$\;
\If{$\min[H]=$ Nil ODER $key[x]<key[\min[H]]$}
{$min[H]\coloneqq x$}
   $n[H]\coloneqq n[H]+1$
}
  \caption{Insert$(H,x)$}
...
$=t(H)+D(n)=\OO(t(H)+D(n))$ und die Potentialdifferenz ist $\leq
D(n)-t(H)$
...
besteht darin, eine vertikale Gerade, die \highl{Sweepline}, von
...
der \highl{event points}, sie kann häufig statisch implementiert werden.
...
\item[$t_i$ für $t=(x_1,\ldots,x_n)$:] Zugriff auf die $i$-te Komponente des 
