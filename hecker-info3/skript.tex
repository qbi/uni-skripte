...
Im ersten Schritt wird die Eingabe aufgesplittet. Das heißt, die
Eingabe wird sukzessive in zwei Hälften geteilt.
...
lautet also, wie f(n) im Vergleich zu $\textup{T}(\frac{n}{b})$ wächst, die Vergleichsgröße ist dabei $n^{\log_ba}$.
Im ersten Fall wächst f(n) langsamer, im zweiten gleich schnell und im dritten polynomial schneller.

Sei $\textup{T}(n)=a \textup{T}(\frac{n}{b})+\textup{f}(n)$ mit $ a \geq 1,b > 1$, 
...
a \textup{f}(\frac{n}{b})  \leq c \textup{f}(n)) \rightarrow \textup{T}(n) \in \Theta(\textup{f}(n))$
\end{enumerate}
...
Mit a = 1 und b = 2 gilt $\log_ba = 0$, also $\Theta(\textup{n}^{\log_ba})=\Theta(1)$ und der zweite Fall kommt zur Anwendung.
Also ist $  \textup{T}(n) \in \Theta(n^{\log_ba}\log n)=\Theta(\log n) $.

\subsubsection{Weitere Beispiele}
\begin{enumerate}
\item $ \textup{T}(n) = 9 \textup{T}(\frac{n}{3}) + 3 n\log n, \textup{also}a = 9,b = 3
...
\item $ \textup{T}(n) = 3 \textup{T}(\frac{n}{4}) + 2n\log n$.
...
\begin{lstlisting}[frame=tlrb, mathescape=true, title=\textsc{S-Quicksort}, gobble=1]{S-Quicksort}
 ...
 Elseif $\abs{A}>2$ Then
   ...
   $A_{1}\coloneqq \{x\leq m, x\in A,\textup{sodaß} \abs{A_{1}}=\ceil{\frac{\abs{A}}{2}}\}$
   $A_{2}\coloneqq \{x\geq m, x\in A, \textup{sodaß} \abs{A_{2}}=\floor{ \frac{\abs{A}}{2}}\}$
   S-Quicksort$(A_{1})$
   S-Quicksort$(A_{2})$
 End
 \end{lstlisting}
...
\begin{lstlisting}[frame=tlrb, mathescape=true, title=\textsc{Union\textnormal{(H$_1$, H$_2$)}},gobble=1]{Union2(H1,H2)}
 H:=Make-Heap
 min[H]:=min[H$_1$, H$_2$]
 Verkn~ü~pfe die Wurzellisten von $H_2$ und H miteinander $\vartriangleleft$ ~\mbox{\todo{Fehl?}}~
...
$\textup{S}(k) = 2 S(\frac{k}{2})+k\leq 2 (\frac{k}{2} \log \frac{k}{2})+k=$
...
$ \textup{T}(n) = 3 \textup{T}(\frac{n}{4}) + 2n\log n \Rightarrow a=3,b=4$ und $\log_ba<1 \rightarrow 
...
a \textup{f}(\frac{n}{b})  \leq c \textup{f}(n)$ existieren. Gibt es also ein 
$c<1\colon  \forall^{\infty}_{n}\colon  3\textup{f}(\frac{n}{4}) \leq c\textup{f}(n)$? Ja, denn

\begin{align*}
3\textup{f}(\frac{n}{4}) & \leq & c \textup{f}(n)\\
