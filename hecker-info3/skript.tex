...
\item   $\log^{*}n \coloneqq \min \Set{i |\log^{(i)}n \leq1}$. Es gilt $ \lim_{n \rightarrow \infty}{\log^{*}n} = + \infty$
\end{enumerate}
...
\begin{lstlisting}[frame=tlrb, mathescape=true, title=\textsc{S-Quicksort}, gobble=1]{S-Quicksort}
 ...
 Elseif $\abs{A}>2$ Then
   ...
   $A_{1}\coloneqq \{x\leq m, x\in A,\textup{sodaß} \abs{A_{1}}=\lceil\frac{\abs{A}}{2}\rceil\}$
   $A_{2}\coloneqq \{x\geq m, x\in A, \textup{sodaß} \abs{A_{2}}=\lfloor \frac{\abs{A}}{2}\rfloor\}$
   S-Quicksort$(A_{1})$
   S-Quicksort$(A_{2})$
 End
 \end{lstlisting}
...
\begin{definition}[Die Ackermann-Funktion und ihre Inverse]
 ...
 und ihre Inverse
 \begin{gather*}
   \alpha(m,n)\coloneqq \min\Set{i \geq 1 | A(i,\lfloor\frac{m}{n}\rfloor)> \log n}
 \end{gather*}
\end{definition}
...
\chapter{Beispiele}
%\mbox{\todo{okay noch ziemlich popelig, ich lasse mir noch mehr einfallen bzw. strukturiere}}
