...
des Entscheidungsbaumes von \(\log (card(\M))=\log m\) ist. Nun sei $b$ ein Blatt des Baumes.
  ...
  \begin{definition}
  \(E(b) := \lbrace \vec x \in \R^n : \) Alg. A landet bei der Eingabe von \( \vec x = (x_1, \dots ,x_n) \) in Blatt \( b \rbrace \)
  \end{definition}
...
Die Komplexität des Elementtests ist also \( \Omega (\log m) \)	
  ...
  \( \M_{\varepsilon^i} := \lbrace ( a_1, \dots , a_n ') \in \R^n \vert \forall   i \ne j : \vert a_i - a_j 
  \vert \geq \varepsilon \rbrace\) ist ein spezielles Elementtestproblem.
				
  $\pi$ sei eine Permutation \( \pi(1, \dots ,n) \), dann ist
  \( \M(\pi ) := \lbrace (a_1, \dots , a_n) \in \M \vert a_{\pi (1)} < a_{\pi (2)} < \dots a_{\pi (n)}  \rbrace \)
...
S$_k$:=Minimalzahl von Knoten, die Nachfolger eines Knotens $v$ vom Rang $k$ sind ($v$ selbst mitgezählt).
 ...
 Habe v den Rang 0 (keinen Sohn), dann ist S$_0$=1 und S$_1$=2. Ab $k \geq 2$ gilt Lemma 1:
