...
\begin{proof}
  ...
  \item["`$\Rightarrow$"'] Wir haben die Unabhängigkeit von $\xi_1,
    ...
    \begin{align*}
      ...
      P(\overline{\xi_1^{-1}}(\times_{i=1}^n B_i))= P(\text{"`}[\xi_1,
      \ldots, \xi_n]\in\times_{i=1}^n B_i\text{"'})= P(\text{"`$\xi_i\in B_i$"'})\\
...
\begin{satz}
  ...
  wobei $f_1*f_2(x)\colon= \int f_1(x-y)f_2(y)\ell(dy)$.
\begin{proof}
...
\begin{satz}
  ...
  \item Sei $P(\text{"`$\xi=c$"'})=1\Rightarrow E\xi=c$ 
...
\begin{proof}
  ...
  \item
    ...
    \item $P_\xi= \delta_{E\xi}\Rightarrow D\xi= \int(x-E\xi)^2 P_\xi
      (dx)= (E\xi-E\xi)^2=0$
    \item $E(\xi-E\xi)^2=0 \Rightarrow P(\text{"`$(\xi-E\xi)^2=0$"'})=1= 
      P(\text{"`$\xi- E\xi$"'})$ 
    \end{itemize}
...
\begin{proof}
  ...
  das Integral monoton ist, folgt $E\xi\geq E(a\chi_{\{\hat{\omega} |
    ...
    \xi(\hat{\omega})\geq a\}}=
  aP(\{\hat{\omega}|\xi(\hat{\omega})\geq a)= aP(\text{"`$\xi\geq a$"'})$. 
\end{proof}
\end{satz}
...
\label{satz:563}
  ...
  \[P\left(\text{"`}\lvert\frac{1}{n}\sum_{i=1}^n \xi_i-a\rvert
    \geq\varepsilon\text{"'}\right)\leq \frac{\delta^2}{n\varepsilon^2}\qquad 
    \varepsilon > 0\]
\begin{proof}
...
\item $P_{\xi_i}=B_{1,p}=\nicefrac{1}{2}(\delta_1+\delta_0)$ mit
  ...
  D\xi_i=\nicefrac{1}{2} (1-\nicefrac{1}{2})=\nicefrac{1}{4}$
\item $\lvert\sum_{i=1}^{250} \xi_i-125\rvert \geq 25=\colon A$ entspricht der
  Aussage, dass ich gewinne.
\end{itemize}
Aus \autoref{satz:563} folgt nun: $P(A)=P(\text{"`}\lvert \nicefrac{1}{n}
\sum \xi_i-a\rvert\geq\varepsilon\text{"'})\leq 
