...
\usepackage{xcolor}
\usepackage[draft=false,colorlinks,bookmarksnumbered,linkcolor=blue,breaklinks,hyperfootnotes=false]{hyperref}
\usepackage{babel}
...
{\itshape
  ...
  \begin{itemize}
   ...
   \item \href{mailto:jimmi_usa2000@yahoo.com}{Kim Muckelbauer
    \texttt{<jimmi\_usa2000@yahoo.com>}} (2007)
   \item \href{mailto:wichti@online.de}{Carolin Rabsahl} (2007)
   \item \href{mailto:mario-ullrich@gmx.de}{Mario Ullrich
    \texttt{<mario-ullrich@gmx.de>}} (2007)
  \end{itemize}
}
...
\{0, 1\}$
\item System der Ereignisse - $\CF\in\FP(\Omega)$\\
...
$P$ ist eine Abbildung $P\colon\CF\rightarrow[0,1]$ und $P(A)$ bezeichnet
...
\subsection{Eigenschaften von \texorpdfstring{$\CF$}{F}}
...
\subsection{Eigenschaften von \texorpdfstring{$P(A)$}{P(A)}}

\begin{itemize}
\item Beispiel: $n$-mal Würfeln: $h_n(A)=\frac{\text{Anzahl des
      Eintretens von } A}{n}\xrightarrow{n\rightarrow\infty} =\colon P(A)$.
  Man bezeichnet $h_{n}(A)$ als \highl[Häufigkeit!relative]{relative
...
\section{\texorpdfstring{$\sigma$}{sigma}-Algebra}
...
\begin{definition}
  Die Menge $\CF$ heißt \highl{Algebra}, wenn gilt:
  \begin{enumerate}[(1)]
  \item $\emptyset\in\CF$
...
\begin{proof}
  ...
  $\sigma$-Algebra $\CF$ den Punkt (3) der Definition erfüllt. Denn die ersten
  beiden Eigenschaften teilen Algebra und $\sigma$"~Algebra. Es
...
\begin{remark}
  ...
  \item Die Umkehrung von \autoref{satz:s-alg} ist in der Regel
    ...
    \[\Omega=\N; \CF\colon=\{A\subseteq\N| A\text{ endlich oder} A^c\text{ endlich}\}\]
    Eigenschaften:
    \begin{enumerate}[(1)]
    \item klar
    \item Sei $A\in\CF$. Dann ist zu zeigen, dass entweder $A^c\in\CF$
    ...
    \item Für $A,B\in\CF$ ist zu zeigen, dass $(A\cup B)$ endlich
      ...
      \item $A^c, B^c$ endlich $\Rightarrow A^c\cap B^c\subseteq A^c$
        endlich $\Rightarrow (A\cup B)^c$ endlich $\Rightarrow A\cup
        B\in\CF$
      \item $A,B^c$ endlich $\Rightarrow A^c\cap B^c\subseteq B^c$
        endlich $\Rightarrow (A\cup B)^c$ endlich $\Rightarrow A\cup B
        \in\CF$
...
\begin{satz}\label{satz:122}
  Sei $\CF$ eine Algebra über $\Omega$. Dann ist $\Omega\in\CF$ und für alle
...
\begin{proof}
  ...
  \item[zu a)] Nach Eigenschaft (3) gilt der Punkt a) für $n=2$. Also haben
    wir $A_{1}, A_{2}\in\CF\Rightarrow A_{1}\cup A_{2}\in\CF$. Nun gelte:
    $\bigcup_{k=1}^n A_k\in\CF  (A_1,\ldots,A_n\in\CF)$. Im Induktionsschritt
    können wir zeigen: $A_1,\ldots,A_{n+1}\in\CF (\bigcup_{k=1}^{n+1}
  ...
  \item[zu b)] Nach Eigenschaft (3) kann folgender Schluss gezogen
    ...
    Regeln bedeutet dies: $\left(\bigcup_{k=1}^n A_k^{c}\right)^c
...
\label{satz:s-alg-abg-123}
  Sei $\CF$ eine $\sigma$-Algebra. Dann gilt
  \begin{gather*}
    \bigcap_{k=1}^\infty A_k\in\CF \qquad A_1,A_2,\ldots\in\CF
  \end{gather*}
  (Beweis wie in \autoref{satz:122} mit $n=\infty$)
\end{satz}

\begin{remark}
  Eine $\sigma$-Algebra ist i.\,A. \emph{nicht} gegenüber
  überabzählbarer Vereinigung bzw. Durchschnitt abgeschlossen.
\end{remark}
...
\begin{beispiel}
  ...
  \item Sei $\emptyset\subset A\subset\Omega, \CF=\{\emptyset, \Omega,
    A, A^c\}$. Der Nachweis, dass dies eine $\sigma$"~Algebra ist, lässt sich
    leicht führen. 
  \end{enumerate}
\end{beispiel}
...
\begin{proof}
  ...
  \begin{itemize}
  \item[zu (1)] Für $i\in I$ gilt, dass $\emptyset\in\CF_i
...
\begin{definition}
  ...
  \[\sigma(\FE)\colon=\bigcap_{\FE\subseteq\CF} \CF\]
...
\begin{remark}
  \begin{enumerate}
   \item Aus \autoref{satz:s-erzeug} folgt $\sigma(\FE)$ ist eine
  $\sigma$-Algebra. Gleiche Konstruktion ist auch für alle
  Algebren möglich.
   \item $\FE_{1}\subseteq\FE_{2}\Rightarrow
    \sigma(\FE_{1})\subseteq\sigma(\FE_{2})$
   \item Sei $\FE\subseteq\FB$ und $\FB$ eine $\sigma$-Algebra. Dann folgt,
    $\sigma(\FE)\subseteq\FB$ 
  \end{enumerate}
...
\label{satz:b-erzeugen}
  Jedes der folgenden Mengensysteme von Intervallen ist ein Erzeugendensystem für
  $\FB$ (Es gilt: $\FE_1,\ldots,\FE_8$ sind abzählbar.):
  \begin{align*}
    \FE_1&\colon=\{(\alpha,\beta)|\alpha,\beta\in\R \text{ und rational}\}\\
    \FE_2&\colon=\{[\alpha,\beta)|\alpha,\beta\in\R \text{ und rational}\}\\
    \FE_3&\colon=\{(\alpha,\beta]|\alpha,\beta\in\R \text{ und rational}\}\\
    \FE_4&\colon=\{[\alpha,\beta]|\alpha,\beta\in\R \text{ und rational}\}\\
    \FE_5&\colon=\{(-\infty,\beta)|\beta\in\R \text{ und rational}\}\\
    \FE_6&\colon=\{(-\infty,\beta]|\beta\in\R \text{ und rational}\}\\
    \FE_7&\colon=\{(\alpha,\infty)|\alpha\in\R \text{ und rational}\}\\
    \FE_8&\colon=\{[\alpha,\infty)|\alpha\in\R \text{ und rational}\}
  \end{align*}
\begin{proof}
  \begin{itemize}
  \item[zu $\FE_1$] Sei $\FE_{1}\subseteq\FE=\{A\subseteq\R| A \text{
...
\begin{definition}
  ...
  \item Eine nichtnegative Funktion $\mu\colon \CF\rightarrow[0,+\infty]$ auf
    $\CF$ heißt
...
\begin{beispiel}
  \begin{enumerate}[(1)]
  \item \highl{Nullmaß} $\CO\colon \CO(A)=0$ für $A\in\CF$
  ...
  \item Sei $\Omega$ abzählbar und $\CF=\FP(\Omega)$. Dann ist das
    \highl{Zählmaß} $\mu(A)\colon=\#(A)$ die Anzahl der
...
\label{satz:massfamilie-131}
  ...
  reller Zahlen. Durch
  \[\mu(A)\colon=\sum_{i\in I} \alpha_i \mu_i(A)\quad (A\in\CF)\]
...
\begin{proof}
  ...
  \item[zu 1)] $\mu(\emptyset)=\sum_{k=1}^\infty \alpha_i\mu_i
    (\emptyset)=0$. Denn $\mu_{i}(\emptyset)=0$.
  \item[zu 2)] Seien $(A_k)_{k=1}^\infty$ paarweise disjunkt aus
    $\CF$.
    \begin{align*}
      \mu(\bigcup_{k=1}^\infty A_k) &= \sum_{i\in I} \alpha_i \mu_i
	 \left(\bigcup_{k=1}^\infty A_k\right) = \sum_{i\in I}
	 \alpha_{i}\sum_{k=1}^\infty \mu_i (A_k)\\
      &= \sum_{k=1}^\infty \underbrace{\sum_{i\in I} \alpha_i
	 \mu_i(A_k)}_{\mu(A_k)} = \sum_{k=1}^\infty \mu(A_k)
    \end{align*}
        Im letzten Schluss musste der große Umordnungssatz angewendet werden.
  \end{itemize}
\end{proof}
\end{satz}

\begin{beispiel}
  Sei $\Omega=\N$ und $\CF=\FP(\N)$. Dann kann man das Maß wie folgt
  definieren: $\mu=\sum_{k=1}^{\infty} \alpha_{k} \delta_{k}$.
\end{beispiel}
...
\label{satz:folgenmass}
  ...
  Diese Eigenschaft heißt \highl[Additivität!endliche]{endliche Additivität}.
\begin{proof}
  Wir setzen $B_k\colon=
  \begin{cases}
    A_k & k=1,\ldots,n\\
    \emptyset & k>n
  ...
  \item $A_i\cap A_j=A_i\cap B_j=\emptyset$ für $i\neq j$ \todo{stimmen die
  Indizes?}
  \item $\mu(A_k)=\mu(B_k)$ für $k=1,\ldots,n$
  \item $\mu(B_k)=0$ für $(k>n)$
  \end{enumerate}
...
\begin{remark}
  \begin{enumerate}
  \item Für $n=\infty$ im \autoref{satz:folgenmass} wird die Additivität als 
    \highl{$\sigma$-Additivität} bezeichnet.
  ...
  \item \highl{Subadditivität}: Für alle
    ...
    \sum_{k=1}^n \mu(A_k)$. Der Beweis wird per Induktion geführt.
  \end{enumerate}
\end{remark}

\begin{beispiel}
  Sei $\Omega=\N, \CF=\FP(\N)$ und das Maß ist definiert: $\mu(A)=
  \begin{cases}0 & A < \infty\\ +\infty & A = \infty\end{cases}$. Sei weiter
  $A_{k}=\{2k\}$. Dann gilt $\mu(\bigcup_{k=1}^{n} A_{k})=\sum_{k=1}^{n}
  \mu(A_{k})=0$. Denn auch die Vereinigung ist wieder endlich. Wenn wir zur
  unendlichen Vereinigung übergehen, ändert sich die Situation. Die Menge, die
  durch die Vereinigung entsteht, ist die Menge der geraden Zahlen. Diese ist
  unendlich als gilt: $\mu(\bigcup_{k=1}^{\infty}
  A_{k})=+\infty\neq0=\sum_{k=1}^{\infty} A_{k}$.
\end{beispiel}
...
\label{satz:elemEigen}
  ...
  \item $A\subseteq B\Rightarrow \mu(A)\leq\mu(B)$ (Monotonie des Maßes)
...
\begin{proof}
  ...
  \item[zu (b) und (c)] $A\subseteq B\Rightarrow B=A\cup(B\setminus
    ...
    Wir haben $\mu(B\setminus A)\geq 0\Rightarrow\mu(A)\leq\mu(B)$ und Teil
    (b) ist gezeigt.\\
    Durch Umstellen erhält man $\mu(B\setminus A)=\mu(B) -
    \mu(A)$. Hierzu muss $\mu(A)<+\infty$ sein.
  \item[zu (a)] Wie oben haben wir: $A\cup B=A\cup (B\setminus A)\Rightarrow \mu(A\cup
    B)= \mu(A\cup (B\setminus A))= \mu(A)+\mu(B\setminus A)$.\\
    Es gilt: $B\setminus A\subseteq B\overset{(b)}{\Rightarrow}
    \mu(B\setminus A)\leq\mu(B)\Rightarrow \mu(A\cup B)-\mu(A)\leq \mu(B)
...
\begin{proof}
  \begin{itemize}
  \item[zu a)] Für den Beweis ist die Forderung $\mu<\infty$ nicht notwendig.
    Setzen $B_1\colon=A_1, B_{n+1}=A_{n+1}\setminus A_n$. Damit
  ...
  \item[zu b)] $A_n\supseteq A_{n+1}\Rightarrow A_n^c\subseteq
    ...
    In der Literatur werden z.\,T. schwächere Forderungen angesetzt.
  \end{itemize}
\end{proof}
\end{satz}
...
direkte Produkte von Mengen: $\times_{i=1}^n A_i\colon=\{[w_1,\ldots, w_n]|
w_i\in A_i (i=1,\ldots,n)\}$. Das Ziel ist es, einen Maßraum mit
der Grundmenge $\Omega=\times_{i=1}^n \Omega_i$ zu bauen.

\begin{definition}
  \begin{itemize}
  \item Die Menge \[\otimes_{i=1}^n \CF_i\colon=\sigma(\{\times_{i=1}^n
    A_i|A_i\in \CF_i (i=1,\ldots,n)\})\]
    heißt \highl{Produkt} der $\sigma$-Algebren $\CF_1,\ldots,\CF_n$.
  \item Die Menge \[\times_{i=1}^n [\Omega_i,\CF_i]\colon=[\times_{i=1}^n
    \Omega_i, \otimes_{i=1}^n \CF_i]\]
    heißt \highl{Produkt} der messbaren Räume $[\Omega_1,\CF_1,\ldots,
    \Omega_n,\CF_n]$.
  \end{itemize}
\end{definition}
...
\begin{remark}
  \begin{enumerate}
  \item Das Produkt der $\sigma$-Algebren $\otimes \CF_i$ ist eine $\sigma$-Algebra auf $\times_{i=1}^n
    \Omega_i$.
  \item Für $n=2$ nutzen wir die Schreibweise $\Omega_1\times\Omega_2,
    \CF_1\otimes\CF_2$.
  \end{enumerate}
\end{remark}
...
\begin{beispiel}
  ...
  \item Sei $\Omega_1$ bis $\Omega_n$ abzählbar:
    ...
    $\{\underbrace{[\omega_1,\ldots,\omega_n]}_{\times_{i=1}^n
      \{\omega_i\}}\} \in
    \otimes_{i=1}^n \CF_i$ für $\omega_i\in\Omega_i, i=1,\ldots,n$. Der Rest
...
\begin{definition}
  ...
  \highl[Produktmaß]{Produkt der Maße} $\mu_1,\ldots,
...
(b_i-a_i)\quad (a_i<b_i, i=1,\ldots,n)\]
\end{definition}
...
\begin{proof}
  Wir haben $\ell([a,b))=b-a$ mit $a<b$. Nun betrachten wir die
  Mengenfolge $B_k\colon=[x,x+\frac{1}{n})\Rightarrow B_k\supseteq B_{k+1}
...
\begin{proof}
  \begin{itemize}
  \item Es gilt $[a,b]=[a,b)\cup\{b\}$. Der Durchschnitt beider Mengen ist
...
\begin{satz}
  Für alle abzählbaren $A\subseteq\R^n$ gilt:
  \[\ell^{(n)} (A)=0\]
\begin{proof}
...
\begin{satz}
  ...
  gelten:
  \begin{enumerate}[(a)]
  \item $0\leq P(B)\leq 1\quad(B\in\CF)$
  \item $P(B^c)=1-P(B)\quad(B\in\CF)$
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[(a)]
...
\begin{definition}
  Sei $P$ ein Wahrscheinlichkeitsmaß auf $[\R^n,\FB_n]$ mit $n\geq 1$. Eine
  Abbildung $F\colon\R^n\rightarrow\R$ heißt \highl{Verteilungsfunktion} von (zu)
  $P$,  wenn gilt:
  \begin{gather*}
    F(x_1,\ldots,x_n)=P(\times_{i=1}^n (-\infty,x_i))\qquad (x_{1},\dotsc,
       x_{n}\in\R)
  \end{gather*}
\end{definition}

\begin{satz*}
  Ein Wahrscheinlichkeitsmaß $P$ ist durch seine Verteilungsfunktion $F$
  eindeutig bestimmt. Dabei sind die $P$ und $F$ wie oben definiert.
  \begin{proof}
    Beweisidee: Wenn man ein durchschnittsabgeschlossenes Erzeugendensystem
    hat und darauf ein Wahrscheinlichkeitsmaß kennt, kennt man es auf der
    gesamten $\sigma$-Algebra.
  \end{proof}
\end{satz*}
...
\label{satz:eigensch-vert-152}
  ...
  \begin{enumerate}[(F1)]
  \item $F(x)\leq F(y)\quad \forall (x\leq y)$ (monoton nicht fallend bzw. wachsend)
  \item $\lim_{n\rightarrow\infty} F(x_n)=F(x)$ mit $x\in\R, x_n\uparrow x$ (linksseitig stetig)
  \item $\lim_{x\rightarrow+\infty} F(x)=1$
  \item $\lim_{x\rightarrow-\infty} F(x)=0$
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[(F1)]
  \item Sei $F(x)\colon=P((-\infty,x))$ mit $x\in\R$. Dann gilt $(-\infty,
    x)\subseteq (-\infty, y)$, da $x\leq y$. Nach 
    \autoref{satz:elemEigen} Punkt b hat man $F(x)\leq F(y)\Rightarrow P((-\infty, x))\leq
  ...
  \item Seien $x\in\R$ und $x_n\leq x_{n+1}, \lim x_n=x$. Damit folgt:
    ...
    x))=P(\bigcup_{n=1}^\infty(-\infty, x_n))=\lim_{n\rightarrow\infty} P((-\infty, x_n))=
    \lim F(x_n)$.
  \item Sei $x_n \uparrow +\infty$\footnote{d.\,h. $x_n\leq
      x_{n+1}, x_n\xrightarrow{n\rightarrow\infty}+\infty\Rightarrow
    (-\infty,x_{n})\leq(-\infty,x_{n+1})$}. Damit
...
\begin{satz*}
  Sei $F\colon\R\rightarrow\R$ mit den Eigenschaften (F1) bis (F4). Dann
...
\begin{remark}
  ...
  \item Sei $F$ die Verteilungsfunktion zu einem
    ...
    \begin{enumerate}[($\hat{\text{F}}$1)]
    \item $F(x_1,\ldots,x_n)\leq F(y_1,\ldots,y_n)$ für  $(x_k\leq y_k,
      k=1,\ldots, n)$
    ...
    \end{enumerate}
  \item Sei nun $F\colon\R^n\rightarrow[0,1]$ mit den Eigenschaften
    ($\hat{\text{F}}$1) bis ($\hat{\text{F}}$4). Dann muss nicht notwendigerweise
...
\begin{beispiel}
  Sei $\lambda>0$. Wir setzen $F_{\lambda}(x)\colon=
  \begin{cases}
    1-e^{-\lambda x} & x>0\\
    0 & x \leq 0
  \end{cases}$. Die Eigenschaften (F1) bis (F4) gelten hier und sind
  trivial nachzuweisen. Das Wahrscheinlichkeitsmaß $\Ex_\lambda$
  mit der Verteilungsfunktion $F_\lambda$ heißt
  \highl{Exponentialverteilung} mit dem Parameter $\lambda$.
  \begin{proof}
    Für die Eigenschaft (F1) nehmen wir o.\,B.\,d.\,A. an, dass $x\leq y$
    gilt. Dann können wir folgende Fälle unterscheiden:
    \begin{enumerate}[1. F{a}ll]
     \item $y\leq0\Rightarrow F_{\lambda}(x)=F_{\lambda}(y)$
     \item $y>0, x\leq0\Rightarrow F_{\lambda}(x)=0,
      F_{\lambda}(y)=1-e^{-\lambda y}\geq0\Rightarrow F_{\lambda}(x)\leq
      F_{\lambda}(y)$
     \item $0<x\leq y\Rightarrow e^{\lambda x}\leq e^{\lambda y}\Rightarrow
      e^{-\lambda x}\geq e^{-\lambda y}\Rightarrow1-e^{-\lambda
      x}\leq1-e^{-\lambda y}\Rightarrow F_{\lambda}(x)\leq F_{\lambda}(y)$.
    \end{enumerate}
    Die Eigenschaften (F2) und (F4) sind sofort klar. Zu (F3) ist
    $e^{\lambda x}\xrightarrow{x\rightarrow+\infty} +\infty\Rightarrow
    e^{-\lambda x}\xrightarrow{x\rightarrow+\infty}0 \Rightarrow 1-e^{-\lambda
    x}\xrightarrow{x\rightarrow+\infty}1$. Somit gelten alle vier Bedingungen.
  \end{proof}
\end{beispiel}

\begin{beispiel}
  Sei $A\in\FB_n$ mit $0<\ell^{(n)}(A)\leq +\infty$. Wir setzen:
  \[\Gl_A(B)\colon=\frac{\ell^{(n)}(A\cap B)}{\ell^{(n)} (A)}\qquad
  (B\in\FB_n)\]
  Dann heißt $\Gl_A$ die \highl{Gleichverteilung} zu $A$. Es gilt natürlich,
  dass $\Gl_{A}$ ein Wahrscheinlichkeitsmaß ist.

  Für den Spezialfall
  $A=[0,1]$ mit $\Gl_{A}(B)=\ell([0,1]\cap B)$ ist als Übungsaufgabe zu
  betrachten.
\end{beispiel}

\begin{remark}
  Die Gleichverteilung ist ein Wahrscheinlichkeitsmaß auf
  $[\R^n,\FB_n]$. Denn es gilt für $(A_{k})_{k=1}^\infty$ paarweise disjunkte
  Mengen aus $\FB$:
  \begin{align*}
    ...
    \Gl_A\left(\bigcup_k B_K\right)&= \frac{\ell^{(n)}((\bigcup
      B_k)\cap A)}{\ell^{(n)} (A)}= \frac{\ell^{(n)}(\bigcup (B_{k}\cap
...
\begin{satz}
  ...
  Die Folge wird auch \highl{Tensorprodukt} genannt.
\begin{proof}
  Wir müssen zeigen, dass $P$ ein Wahrscheinlichkeitsmaß ist. Dazu wissen wir,
  dass $P$ ein Maß ist. Daher müssen wir noch zeigen, dass $P(\R^{n})=1$. Es
  gilt,
  $P(\R^n)=P(\times_{k=1}^{n}\R^{n})=(\times_{k=1}^n P_k)(\R^{nx})=\prod_{k=1}^n
  \underbrace{P_k(\R)}_{=1}=1$.

  Nun ist noch zu zeigen, dass $F$ eine Verteilungsfunktion zu $P$ ist, d.\,h.
...
\begin{satz}
  ...
  \item $P(\{a\})=\underbrace{F(a+0)}_{\lim_{x_{n}\downarrow a} F(a)}-F(a)$
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[zu (1)]
  \item Die Frage ist zunächst, wie sich das Intervall durch Halbstrahlen
    darstellen lässt. Wir wissen, $(-\infty, b)\setminus (-\infty, a)= [a,b)$ und $(-\infty,
    a)\subseteq (-\infty, b)$. Nach \autoref{satz:elemEigen}~c ist
    $P([a,b))= P((-\infty, b))\setminus (-\infty, a))=P((-\infty,
    b))- P((-\infty, a))= F(b)-F(a)$
  \item Es gilt, $\{a\}=\bigcap_{n\geq 1} [a,a+\frac{1}{n})$ und
    $[a,a+\frac{1}{n}) \supseteq [a,a+\frac{1}{n+1})$. Wegen
    \autoref{satz:stetigkeit} Punkt~b ist $P(\{a\})=P(\bigcap_{n\geq 1}
    [a,a+\frac{1}{n})) = \lim
...
\begin{satz}
  ...
  $[\R,\FB]$, $\alpha_{i}\geq 0$ mit $i\in I$ und $P=\sum_{i\in I} \alpha_i
...
\begin{proof}
  ...
  P_i((-\infty, x))= \sum_{i\in I} \alpha_{i} F_i(x)$.
\end{proof}
\end{satz}
...
\begin{beispiel}
  ...
  \item \highl{Binomialverteilung}: Gegeben
    ...
    \[B_{n,p}\colon= \sum_{i=0}^n \binom{n}{i} p^i (1-p)^{n-i} \delta_i\]
  ...
  \item \highl[Verteilung!geometrische]{geometrische Verteilung}:
    Gegeben ist $0<q<1$. 
    \[G_q\colon=\sum_{i=1}^\infty q^{i-1} (1-q) \delta_i\]
...
Hier soll ein allgemeiner Integralbegriff eingeführt werden. Wir brauchen das
später für das Rechnen mit dem Erwartungswert.

Man stelle sich vor, jemand führt $n$ Würfe mit einem Würfel aus und berechnet
$\nicefrac{1}{n} \sum_{i=1}^{n} x_{i}$. Für unendlich viele Würfe konvergiert
das gegen den Erwartungswert $E\xi=\int \xi(\omega) P(d\omega)$.

Es wird zwei wichtige Spezialfälle geben:
\begin{enumerate}
  \item $\sum_{k} k\alpha_{k}$
 \item $\int_{-\infty}^{\infty} f(x)\,dx$
\end{enumerate}


\section{Messbare Funktionen (Abbildungen)}
...
X$ und definieren das \highl{Urbild} einer Menge:
$\fmineins(A)\colon=\{\omega\in\Omega| f(\omega)\in
A\}$ mit $A\subseteq X$. Dies ist nicht zu verwechseln mit inversen
...
\begin{satz}[Elementare Eigenschaften des Urbilds]
  ...
  \item $\fmineins(A^c)=(\fmineins(A))^c\quad (\forall A\subseteq X)$, dabei
    ist $\fmineins(A^c)$ das Komplement in $X$ und $(\fmineins(A))^c$ das
    Komplement in $\Omega$.
  \item $\fmineins(\bigcup_{i\in I} A_i)= \bigcup_{i\in I} \fmineins
    (A_i)\quad (A_{i}\subseteq X, i\in I)$
...
\begin{proof}
  ...
  \item $\omega\in\fmineins(A^c)\Leftrightarrow f(\omega)\in A^c
    ...
    \fmineins (A)\Leftrightarrow \omega\notin\fmineins(A)\Leftrightarrow
    \omega\in (\fmineins(A))^c 
    \Rightarrow \fmineins(A^c)=(\fmineins(A))^c$
  \item selbst in Übung
   \item selbst in Übung
  \end{enumerate}
\end{proof}
\end{satz}
...
\begin{definition}
  Sei $f\colon\Omega\rightarrow X$. Die Abbildung $f$ heißt
 \highl[Abbildung!messbare]{$(\CF,\FX)$-messbar}\index{messbar}, wenn gilt:
  \[\fmineins(A)\in\CF\qquad (A\in\FX)\footnote{Schreibweise:
    $f\colon[\Omega, \CF]\rightarrow [X,\FX]$}\]
  In diesem Fall heißt das Maß $\mu\circ\fmineins$, definiert durch
  $\mu\circ\fmineins(A)=\mu(\fmineins(A))$ für alle  $A\in\FX$,
  \highl{Bildmaß} von $\mu$ bezüglich $f$.
\end{definition}

\begin{remark}
  \begin{enumerate}[1.)]
   \item Die Definition von $\mu\circ f^{-1} (A)$ ist sinnvoll, weil
    $f^{-1}(A)\in\CF$ für alle $A\in\FX$.
  ...
  \item Im Fall das $\Omega, X$ metrische Räume sind und $\CF, \FX$
    ...
    Stetigkeit von $f$, dass $f$ auch messbar ist. Um das zu zeigen, wähle man
    $\FE= \{A\subseteq X\colon A \text{ offen}\}$. Nach der Definition ist
    $\FX=\sigma(\FE)\Rightarrow f^{-1}(A)\in\CF$ für ein $A\in\FE$. Somit ist
    klar, dass, wenn $f$ stetig ist, folgt, dass $f^{-1}(A)$ offen ist und
    damit ist $f^{-1}(A)\in\CF$. (siehe auch den unten stehenden
    Satz)
  \item Sei $\fmineins\colon \FP(X)\rightarrow\FP(\Omega)$ mit $\mu\colon
  ...
  \item Ist $\mu\circ\fmineins$ ein Maß auf $[X,\FX]$. Hierzu ist
    klar, dass $\mu\circ\fmineins\colon\FX\rightarrow[0,+\infty]$. Es ist
...
\begin{satz*}
  Sei $f\colon\Omega\rightarrow X$.  $f$ ist $(\CF,\FX)$-messbar, wenn für ein
  $\FE\subseteq \FP(X)$ mit $\sigma(\FE)=\FX$ gilt:
  \begin{gather*}
    f^{-1} (A)\in\CF\qquad (A\in \FE)
  \end{gather*}
\end{satz*}

\begin{beispiel}
  \begin{enumerate}
   \item Die "`Konstanten"' sind messbar, d.\,h. wir haben
    $f\colon\Omega\rightarrow X$ mit $f(\omega)=c$. Dann ist
    $\fmineins(A)=
    \begin{cases}
      \emptyset & c\notin A\\
      \Omega & c\in A
    \end{cases}$. Da nun die leere Menge und auch $\Omega$ Element von
    $\CF$ sind, gilt $\fmineins(A)\in\CF$. Damit ist die Funktion immer
    $(\CF, \FX)$-messbar.
  \item Sei $\CF=\{\emptyset, \Omega\}$ und $[X,\FX]=[\R,\FB]$. Zu
    zeigen ist, ob $f$ $(\R,\FB)$-messbar ist. Dies ist genau dann der Fall,
    wenn ein $c\in\R$ mit $f(\omega)=c$ existiert \dots 
  \item Sei $\CF=\FP(\Omega)$. Dann ist jede Abbildung
    $f\colon\Omega\rightarrow X$ messbar. $\CF=\FP(\Omega)\Rightarrow
    \fmineins(A)\in\CF\ (A\subseteq X)$. Für beliebige $\FX$ ist $f$
    $(\FP(\Omega),\FX)$"~messbar.
  \end{enumerate}
\end{beispiel}
...
\begin{satz}
  ...
  Seien $f_1\colon[\Omega_1,\CF_1]\rightarrow[\Omega_2,\CF_2]$ und $f_2\colon
  [\Omega_2, \CF_2]\rightarrow[\Omega_3,\CF_3]$. Dann gilt $f_2\circ
  f_1\colon [\Omega_1,\CF_1]\rightarrow[\Omega_3,\CF_3]$. Die
  Hintereinanderausführung messbarer Funktionen ist wieder messbar.
\begin{proof}
  Im ersten Schritt zeigen wir: $(f_{2}\circ f_{1})^{-1}(A)=
  f_{1}^{-1}(f_{2}^{-1}(A))$. Sei dazu $\omega\in(f_{2}\circ f_{1})^{-1}(A)
  \Leftrightarrow (f_{2}\circ f_{1})(\omega)\in A\Leftrightarrow
  f_{2}(f_{1}(\omega))\in A\Leftrightarrow f_{1}(\omega)\in f_{2}^{-1}(A)
  \Leftrightarrow \omega\in f_{1}^{-1}(f_{2}^{-1}(A))$. Dieses Ergebnis
  verwenden wir nun für den weiteren Beweis.
  
  Sei $A\in\CF_3$. Es ist nun zu zeigen, dass $(f_2\circ f_1)^{-1} (A)
  \in \CF_{1}$. Dazu stellen wir fest, dass $(f_2\circ f_1)^{-1} (A)=
  f_{1}^{-1}(f_{2}^{-1}(A))$. Da aber $f_{2}$ messbar ist, folgt
  $f_{2}^{-1}(A)\in \CF_{2} \Rightarrow f_{1}^{-1}(f_{2}^{-1}(A))\in \CF_{1}$.
  Das ist aber nichts anderes als $(f_{2}\circ f_{1})^{-1}(A)\in\CF_{1}$.
  \end{proof}
\end{satz}
...
\begin{satz}
  ...
  Die Abbildung $f\colon\Omega\rightarrow\R$ ist genau dann messbar, wenn
  eine der folgenden Aussagen richtig ist ($x, x_1, x_2$ sind dabei
  immer rational):
  \begin{enumerate}[(1)]
  \item $\{\omega\in\Omega|f(\omega)<x\}\in\CF$
  \item $\{\omega\in\Omega|f(\omega)\leq x\}\in\CF$
  \item $\{\omega\in\Omega|f(\omega)>x\}\in\CF$
  \item $\{\omega\in\Omega|f(\omega)\geq x\}\in\CF$
  \item $\{\omega\in\Omega|x_1\leq f(\omega)<x_2\}\in\CF$
  \item $\{\omega\in\Omega|x_1< f(\omega)<x_2\}\in\CF$
  \item $\{\omega\in\Omega|x_1\leq f(\omega)\leq x_2\}\in\CF$
  \item $\{\omega\in\Omega|x_1< f(\omega)<x_2\}\in\CF$
  \end{enumerate}
\begin{proof}
  ...
  System mit $\fmineins(B)\in\CF$ ist, dann ist $f$ messbar. Es gilt:
  $\{\omega\in\Omega | f(\omega)<x\}=\fmineins((-\infty,x))$. Aus der
...
\label{satz:verkn-messbar-fkt-214}
  Seien $f,g\colon[\Omega,\CF]\rightarrow[\R,\FB]$ messbare Funktionen. Dann
  sind die Funktionen $f+g$ und $fg$ ebenfalls messbare Funktionen.
\begin{proof}
  Seien $f,g\colon\Omega\rightarrow\R^2$. Diese Funktionen werden wie folgt
  umgewandelt: $h\colon=[f,g]\colon\Omega\rightarrow\R^2, [f,g](\omega)=
  ...
  g^{-1}(B))\in\CF$) gilt für alle $A,B\in\FB\colon \hmineins(A\times B)\in
  \CF$. Und es folgt weiter $\sigma(\{A\times B|A\in\FB, B\in\FB\}) =
  \FB_2$. Damit ist $h$ messbar.\\
  Wir setzen nun $a(x_1,x_2)=x_1+x_2, b(x_1,x_2)=x_1x_2,
  a,b\colon\R^{2}\rightarrow\R$, stetig. Also ist
...
\begin{satz}
  Sei $(f_i)_{i\in I}\colon[\Omega,\CF]\rightarrow[\R,\FB]$ eine abzählbare
  Familie messbarer Funktionen,
  ...
  messbar.\footnote{Erläuterung: $(\sup f_i)(\omega)\colon= \sup
    f_i(\omega)$ mit $\omega\in\Omega$}
\begin{proof}
  \begin{enumerate}[1. F{a}ll]
  \item Sei $f\colon=\sup f_i$. Dann ist $f$ eine Abbildung von $\Omega$
  ...
  \item Sei $f\colon=\inf f_1\Rightarrow f\colon\Omega\rightarrow\R$. Wegen
    \autoref{satz:messb-eigensch213} (4) genügt es zu zeigen, dass
    $\bigcap_{i\in I}\{\omega| f_{i}(\omega)\geq x\}= \{\omega\in\Omega |
    f(\omega)\geq x\}\in\CF$. Dazu $f(\omega)\geq
    x\Leftrightarrow f_i(\omega)\geq x (i\in I)\Rightarrow
    \{\omega\in\Omega | f(\omega)\geq x\}=\bigcup_{i\in I}
    \{\omega\in\Omega | f_i(\omega)\geq x\}=\bigcup_{i\in I} \{\fmineins_i
      ([x,+\infty))\in\FB\}\Rightarrow \{\omega\in\Omega| \inf_{i}
    f_{i}(\omega) \geq x\}\in\FB$ für $x\in\R$. Dann folgt nach
    \autoref{satz:messb-eigensch213} Punkt 4, dass $\inf_{i} f_{i}$ messbar
    ist.
  \end{enumerate}
\end{proof}
\end{satz}
...
\begin{satz}
  Sei $f$ eine messbare Funktion ($f\colon[\Omega,\CF]\rightarrow [\R,\FB]$).
  Dann gelten:
  \begin{enumerate}[(1)]
  \item $af+b$ ist messbar. $(a,b\in\R)$
  \item $f^+\colon=\max (f,0)$ ist messbar und der positive Ast der Funktion.
  \item $f^-\colon=\min (f,0)$ ist messbar und der negative Ast der Funktion.
  \item $|f|$ ist messbar.
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[zu (1)]
  \item Wir wissen, dass "`konstante"' Funktionen immer messbar
    ...
    gilt die Behauptung.
  \item Wir setzen $f_1=f, f_2=0$. Dann folgt, dass $f^{+}=\max(f_{1},
    f_{2})$. Beide Funktionen sind messbar und
...
\label{satz:indimess-218}
  Sei $A\subseteq\Omega$. Die Indikatorfunktion $\chi_A(\omega)=
  \begin{cases}1 & \omega\in A\\ 0 & \omega\notin A\end{cases}$ ist genau
...
\begin{proof}
  ...
  \end{cases}\]
  Für die Gleichheit müssen wir beide Richtungen zeigen: Sei $\chi_{A}$
  messbar. Dann ist $\{1\}\in\FB\Rightarrow\chi_{A}^{-1}(\{1\})= A\in \CF$.\\
  Sei $A\in\CF$. Dann ist zu zeigen, dass $\chi_{A}$ messbar ist, d.\,h.
  $\chi_{A}^{-1}(B)\in\CF$ für $B\in\FB$. Es folgt, dass $\chi_{A}^{-1}(B)\in
  \{\emptyset, \Omega, A,A^{c}\}$ und somit genügt es zu zeigen, dass
  $\{\emptyset, \Omega, A,A^{c}\}\subseteq\CF$ ist. Wir wissen, dass
  $\emptyset, \Omega\in\CF$ sind. Weiterhin ist nach Voraussetzung
  $A\in\CF\Rightarrow A^{c}\in\CF$ und es folgt die Behauptung.
\end{proof}
\end{satz}
...
\begin{definition}
  Sei $f\colon\Omega\rightarrow\R$. Die Funktion $f$ heißt
...
\begin{remark}
  ...
  \item Einfache Funktionen sind messbar. Denn nach dem vorhergehenden
    ...
    messbarer Funktionen sind wieder messbar. (Zum Beweis siehe
    \autoref{satz:verkn-messbar-fkt-214} und \autoref{satz:indimess-218}).
  \end{enumerate}
\end{remark}

\begin{satz}
  Sei $f\colon\Omega\rightarrow\R$. Dann gelten
  \begin{enumerate}[(1)]
...
\begin{proof}
  ...
  \item
    ...
    \item["`$\Leftarrow$"'] Setzen $\{b_1,\ldots,b_n\}=\{f(\omega)|
      \omega\in\Omega\}$. Die $b_{i}$ sind paarweise verschieden. Weiter
      setzen wir $B_i=\fmineins(\{b_i\})$ für $i=1,\dotsc,n$. Es gilt, dass
      alle $B_{i}$ paarweise disjunkt sind und das deren Vereinigung $\Omega$
      ergibt. 
      Damit folgt
...
\label{satz:limfolge-2110}
  ...
  mit $f_n\leq f_{n+1}$ (monoton wachsend) und $f=\limn f_n (f_n \uparrow f)$.
\begin{proof}
  Der Beweis erfolgt konstruktiv, d.\,h. man konstruiert eine spezielle
  Folge $(f_n)_{n=1}^\infty$ mit den Eigenschaften aus dem Satz:
  \begin{align*}
    f_n &\colon=\sum_{i=0}^{4^n-1} \frac{i}{2^n}
  \chi_{\fmineins([\frac{i}{2^n}, \frac{i+1}{2^n}))} +2^n
  \chi_{\fmineins ([2^n,+\infty))}\\
    f_{n}(\omega) &\colon=
       \begin{cases}
	 \nicefrac{i}{2^{n}} & \nicefrac{i}{2^{n}} \leq f(\omega)<\frac{i+1}{2^{n}}\\
	 2^{n} & f(\omega)\geq2^{n}
	 \end{cases} \quad i=0,\dotsc,4^{n}-1
  \end{align*}
...
Im folgenden ist immer $\ofmu$ ein Maßraum und wir betrachten $f\colon
...
\begin{definition}[1. Stufe]
  ...
  \[\int f(\omega)\mu(d\omega)\colon=\sum_{i=1}^n
...
\begin{remark}
  ...
  \item Das Integral ist linear, d.\,h. $f,g\geq0$ und einfach,
...
\begin{definition}[2. Stufe]
  ...
  \[\int fd\mu \colon= \limn\int f_nd\mu\]
\end{definition}
...
\begin{remark}
  ...
  \item Die Definition zweiter Stufe ist eine Erweiterung der
    Definition erster Stufe. Denn sei $f\geq0$ einfach. Dann setzen wir
    $f_{n}\colon= f$ mit $n=1,2,\dotsc \Rightarrow 0\leq f_{n}\leq f_{n+1}$.
    Die $f_{n}$ seien messbar. Dann folgt nach der Definition zweiter Stufe:
    $\int f\,d\mu=\lim\int f_{n}\,d\mu=\int f\,d\mu$.
  ...
  \item Für $0\leq f\leq g$ gilt: $0\leq \int fd\mu\leq\int gd\mu$,
    ...
    \Rightarrow\int gd\mu\geq\int fd\mu$. Dies folgt aus $\int f\,d\mu\geq0$
    für die erste Stufe.
   \item Monotonie: $0\leq f\leq g\Rightarrow\int f\,d\mu\leq\int g\,d\mu$.
    Siehe hierzu die vierte Bemerkung bei der Definition der ersten Stufe.
  \end{enumerate}
\end{remark}
...
\begin{definition}[3. Stufe]
  ...
  \[\int fd\mu\colon=\int f^+d\mu-\int(-f^-)d\mu\]
\end{definition}
...
\begin{remark}
  ...
  \item $\int fd\mu$ für $f\geq 0$ und messbar existiert immer. Es ist
    ...
    Funktionen erklärt. Ohne die Bedingung $\int f\,d\mu<+\infty$ ist das
    Integral nicht erklärt.
...
\begin{satz}[Integral bezüglich diskreter Maße]
  ...
  reeller Zahlen, $(\omega_i)_{i\in I}\subseteq\Omega$ und $\mu=
  \sum_{i\in I}\alpha_i \delta_{\omega_i}$\footnote{Maße von diesem
    Typ heißen \highl[Maß!diskretes]{diskrete Maße}}. Eine messbare Funktion
  $f$ (d.\,h. $f\colon[\Omega, \CF]\rightarrow [\R,\FB]$) ist genau dann
  \highl[integrierbar]{$\mu$-integrierbar}, wenn gilt $\sum_{i\in I} \alpha_i
  |f(\omega_i)|<+\infty$. In diesem Fall gilt:
  \[\int fd\mu=\sum_{i\in I} \alpha_i f(\omega_i)\]
\begin{proof}
 \setcounter{enumi}{-1}
  \begin{enumerate}[1. F{a}ll]
  \item Sei $f=\chi_A$ mit  $(A\in\CF)$. Dann folgt nach der Definition des
    Integrals erster Stufe, dass für $g=\sum a_{i} \chi_{A_{i}}$ (einfach und
    größergleich 0)
    gilt, $\int g\,d\mu_{i}=\sum a_{i} \mu(A_{i})$. Damit folgt, 
    $\int f\,d\mu=\int\chi_A\,d\mu=\mu(A)=\sum_{i\in I} \alpha_i
  ...
  \item Sei $f=\sum_{k=1}^n a_k \chi_{A_k}$ mit $a_k\geq 0,
    A_k\in\CF$. Dann ist $f\geq 0$, einfach und $\int fd\mu=\sum_{k=1}^n a_k\mu(A_k)=\int
    ...
    \sum_{i\in I} \alpha_i f(\omega_i)$
  \item Seien $f\geq 0$ und messbar, $0\leq (f_{m})_{m=1}^{\infty} \uparrow f$
    einfach. Nach der Definition zweiter Stufe folgt nun, dass $\int
    f\,d\mu=\limn\int f_m\,d\mu$. Nach dem vorigen Fall folgt:
    ...
    f(\omega_i)$. 
  \item  Sei $f$ messbar und gelte $f=f^{+}-f^{-}$. Wir wissen nach der
    Definition des Integrals zweiter Stufe:
    \begin{align*}
      \int f^{+}\,d\mu &= \int_{i\in I} \alpha_{i} f^{+}(\omega_{i})\\
      \int f^{-}\,d\mu &= \int_{i\in I} \alpha_{i} f^{-}(\omega_{i})
    \end{align*}
    Weiterhin wissen wir, dass $f$  genau dann $\mu$-integrierbar ist, wenn
    $\int\abs{f}\,d\mu<\infty$. Damit sind auch die Integrale der
    beiden Teilfunktionen kleiner als unendlich.
    Wenn $f$ $\mu$-integrierbar ist, dann folgt aus der Definition
    dritter Stufe:
    \begin{multline*}
      \int f\,d\mu
      = \int f^+-(-f^-)d\mu
      = \sum_{i\in I} \alpha_i f^+(\omega_i)-\sum_{i\in I}
         \alpha_i (-f^-)(\omega_i)\\
      = \sum_{i\in I} \alpha_i \big(f^+(\omega_i)-(-f^-)(\omega_i)\big)
      = \sum_{i\in I} \alpha_i \big(f^+(\omega_i)+f^-(\omega_i)\big)
      = \sum_{i\in I} \alpha_i f(\omega_i)
    \end{multline*}
  \end{enumerate}
...
\begin{definition}
  Der Wahrscheinlichkeitsraum $(\Omega,\CF,\mu)$ heißt \highl{diskret}, wenn
  $\Omega$ abzählbar, $\CF=\FP(\Omega)$ und $\mu$ diskret ist.
\end{definition}

\begin{beispiel}
  \begin{enumerate}
  \item Sei $\mu=\delta_{\omega_0}$ und ein festes $\omega_0\in\Omega$
  ...
  \item Wir betrachten die Reihe $\sum_{k=0}^m a_k$. Dabei ist auch
    ...
    $\mu(A)= \#A$. Setzen $f\colon\Omega\rightarrow\R$ mit $f(k)\colon=a_k$
    für $k\in\Omega$. Damit folgt, $\int fd\mu=\sum_{k=0}^m a_k$ und $f$ ist
...
\label{satz:riemann-222}
  Seien $f\colon\R\rightarrow\R$ stetig, $f$ stetig mit Stammfunktion $F$,
  $a,b\in\R$ und $a<b$. Dann ist $f\chi_{[a,b]}$ bezüglich $\ell$ integrierbar
  und es gilt:
  \[\int f\chi_{[a,b]}\,d\ell=\int_a^b f(x)\,dx=F(b)-F(a)\]
\begin{proof}
  ...
  $f\chi_{[a,b]}$ ebenfalls. Wir konstruieren einen Funktionenfolge
  $(f)_{n=1}^{\infty}$. Nach der Definition zweiter Stufe wissen
  wir, dass $\int f\chi_{[a,b]}d\ell=\limn\int f_nd\ell$, falls $0\leq
  f_n \uparrow f$ und $f_n$ einfach. Im folgenden wird $f_n$ als
  untere Darbouxsche Summe konstruiert. Dabei wird das Intervall $[a,b]$ in
  Intervalle $z_{k}^{(n)}$ zerlegt.
  ...
  \begin{pspicture}(6,8)
    ...
    \rput[t](4.5,0){$b$}
  \end{pspicture}
    \caption{Untere Darbouxsche Summen}
  \end{figure}

  \todo{Ergänzung von Carolin einfügen}
  \begin{align*}
    z_{k+1}^{(n)}-z_k^{(n)} &=\frac{b-a}{2^n} f_n(x)=0\quad (x\notin[a,b])\\
    b_{k}^{(n)} &\colon= \inf_{z_{k}^{(n)}\leq x< z_{l+1}^{(n)}}\\
    f_{n}(x) &= \sum_{k} b_{k}^{(n)} \chi_{[z_{k}^{(n)}, z_{k+1}^{(n)})}(x)\\
    &\Rightarrow f_{n}\geq0, f_{n}\leq f_{n+1}, \lim f_{n}=f\chi_{[a,b]},
       f_{n} \text{ einfach}\\
    \Rightarrow \int f\chi_{[a,b]}\,d\ell &= \limn \int f_{n}\,d\ell\\
    &=\limn \sum_{k} b_{k}^{(n)} \underbrace{\ell([z_{k}^{(n)},
       z_{k+1}^{(n)}))}_{\frac{b-a}{2^{n}}}\\
    &= \limn \sum_{k} b_{k}^{(n)} \frac{b-a}{2^{n}}= \int_{a}^{b} f(x)\,dx=
       F(b)-F(a)
  \end{align*}

  Damit ist die Behauptung für $f\geq0$ gültig. Sei nun $f$ "`beliebig"'
  (insbesondere aber stetig, beachte die Voraussetzungen). Damit sind $f^{+},
  (-f^{-})$ stetig und das Integral $\int f^{+}\chi_{[a,b]}\,d\ell=
  \int(f\chi_{[a,b]})^{+}= \int_{a}^{b} f^{+}(x)\,dx$. Selbiges gilt natürlich
  für $(-f^{-})$. Es ist klar, dass $\int_{a}^{b} f^{+}(x)\,dx+ \int_{a}^{b}
  (-f^{-})(x)\,dx= \int_{a}^{b} f(x)\,dx$. Dabei wurde die Linearität des
  Riemannintegrals ausgenutzt. Wegen der Linearität des Lebegueintegrals folgt
  auch $\int f^{+}\chi_{[a,b]}\,d\ell-\int (-f^{-})\chi_{[a,b]}\,d\ell= \int
  f\chi_{[a,b]}\,d\ell$. Aus den obigen Aussagen folgt schließlich die
  Behauptung.

  \todo{untenstehendes kann evtl. entfernt werden}
  Es ist bekannt, dass das Riemannsche Integral der Limes der
  Darbouxschen Summen ist. Somit haben wir $\limn\int f_n\,d\ell=
...
\begin{remark}
  ...
  \item Die Behauptung gilt auch dann, wenn nur vorausgesetzt wird,
    dass $f$ auf $[a,b]$ riemann-integrierbar ist.
  \item Der \autoref{satz:riemann-222} ist ausdehnbar auf die Fälle
    $a=-\infty, b=\infty$. Dabei ist die Forderung, dass $f$ auf $[a,b]$
    $\ell$-integrierbar ist, notwendig. Das heißt nun aber
    $\int_{a}^{b}\abs{f(x)}\,dx<+\infty$, d.\,h. $\int_{a}^{b}
    \abs{f(x)}\,dx<+\infty$.
  \end{enumerate}
\end{remark}
...
\begin{satz}[Übertragungssatz]
  ...
  messbarer Raum und weiter $h\colon[\Omega_1,\CF_1]\rightarrow[\Omega_2,\CF_2],
  f\colon[\Omega_2, \CF_2]\rightarrow[\R,\FB]$, wobei $f$ bezüglich $\mu\circ
  h^{-1}$
  integrierbar ist. Dann ist $f\circ h \colon\Omega_1\rightarrow\R$
  bezüglich $\mu$ integrierbar und es gilt:
  \begin{gather}
    \label{eq:uesatz}
    \int f\circ h\,d\mu=\int f\,d\mu\circ\hmineins
  \end{gather}
\begin{proof}
  \begin{enumerate}[1. F{a}ll]
 \setcounter{enumi}{-1}
  \item Sei $f=\chi_A$ mit  $(A\in\CF_2)$ und damit messbar. Wir betrachten
    die Verknüpfung $f\circ h(\omega)=\chi_A(h(\omega))$. Es ist klar, dass
    $\chi_A\circ h$ eine Indikatorfunktion auf $\Omega_1$ sein muss, d.\,h.
    $\exists B\subseteq\Omega_1$ mit $\chi_A\circ h=\chi_B$ und
    $B=\{\omega\in\Omega_{1}\colon \chi_{A}(h(\omega))=1\}$.\\
    $\chi_B(\omega)=1\Leftrightarrow \chi_A(h(\omega))=1
    \Leftrightarrow h(\omega)\in A\Leftrightarrow w\in\hmineins(A)
    \Rightarrow B=\hmineins(A)\in\CF_1$. Dabei ist
    $B=\{\omega\in\Omega_{1}\colon \chi_{A}(h(\omega))=1\}$.
    \[\Rightarrow \chi_A \circ h = \chi_{\hmineins(A)}\]
    $\int f\circ h\,d\mu=\int\chi_{\hmineins(A)}\,d\mu=\mu(\hmineins(A))
    =\mu\circ\hmineins(A)=\int\chi_A\,d\mu\circ\hmineins=\int
    f\,d\mu\circ\hmineins$
  \item Sei $f\geq 0$ und einfach, d.\,h. $f=\sum_{i=1}^n a_i
    \chiai$ mit $A_i\in\CF_2, a_i\geq 0$. Das Integral $\int f\,d\mu
    \circ h^{-1}$ entspricht wegen der Linearität der Summe
    $\sum_{i=1}^n a_i\int \chiai\,d\mu\circ\hmineins=\sum_{i=1}^n a_i
    \int \chiai \circ\hmineins\,d\mu=\int\sum_{i=1}^n a_i \chiai \circ
    \hmineins\,d\mu=\int f\circ h\,d\mu$
  \item Wir wählen $(f_n)_{n=1}^\infty$ mit $0\leq f_n\leq f_{n+1},
    ...
    f_n\circ h d\mu$. Es ist klar, dass $g_n\colon=f_n\circ h$ die
...
\begin{beispiel}
  Sei $\Omega_2=\R, h\colon\Omega_1\rightarrow\R, f(x)=x,
  f\colon\R\rightarrow\R$. Dann gilt $\int h(\omega)\mu(d\omega)=\int
...
\begin{beispiel}
  \begin{enumerate}
  \item Sei $\mu=\delta_{\omega_0}\Rightarrow f=g\
    \mu"~\text{fast-überall}\Leftrightarrow
    f(\omega_0)=g(\omega_0)$ für $\omega_{0}\in\Omega$. Denn sei
    $f(\omega)\neq g(\omega), \omega\neq \omega_{0}\Rightarrow
    \mu(\{\omega\in\Omega | f(\omega)\neq g(\omega)\})= \mu(\{\omega\in\Omega
    | f(\omega_{0})\neq g(\omega_{0})\}) \Rightarrow f=_{\delta_{\omega_0}}
    g\Leftrightarrow f(\omega_{0})= g(\omega_{0})$.
  \item $\ofmu=\rbel, f=\chi_\Q$ Da die rationalen Zahlen abzählbar
    ...
    Allgemeiner gilt: $f=_{\ell}g\Leftrightarrow f\chi_{A^{c}} =_{\ell}
    g\chi_{A^{c}}$.
  \end{enumerate}
\end{beispiel}
...
\begin{satz}
  ...
  \[\int fd\mu=\int gd\mu\]
\begin{proof}
  $f=_\mu g\Leftrightarrow f-g=_\mu 0=_{\mu}h$ durch "`Reduktion"' der
  Allgemeinheit
  \begin{itemize}
  \item Wegen $\int(f-g)d\mu=\int fd\mu-\int gd\mu$ genügt es zu
    zeigen, dass $h=0$ messbar und aus $h=_\mu 0$ folgt $\int hd\mu=0$.
   \item Es genügt, den Fall $h\geq0$ zu zeigen. Denn wir haben
    $h=h^{+}+h^{-}. h^{+}, h^{-}\geq0, \int h\,d\mu= \int h^{+}\,d\mu-
    \int(-h^{-})\,d\mu$ und $0= \mu(\{\omega\in\Omega | h(\omega)\neq=\})\geq
    \mu(\{\omega\in\Omega| h^{+}(\omega)\neq0\})\geq0$. Das gilt analog für
    $h^{-}$.
   \item Es reicht weiter, den Fall $h$ einfach und größer 0 zu betrachten.
    Denn sei $\int h\,d\mu=\lim\int f_{n}\,d\mu$ mit $0\leq f_{n}\leq f_{n+1},
    \lim f_{n}=h$. Dann wissen wir nach dem obigen Punkt: $0=
    \mu(\{\omega\in\Omega| h(\omega)\neq0\})= \mu(\{\omega\in\Omega|
    h(\omega)>0\})\geq \mu(\{\omega\in \Omega| f_{n}(\omega)>0\})\geq0$.
   \item Das Problem kann weiter auf die Betrachtung von $h=\chi_{A}$ mit
    $A\in\FP$ reduziert werden. Denn sei $h$ wie in der obigen Feststellung
    einfach. Dann gilt: $h=\sum_{j=1}^{m} a_{j} \chi_{A_{j}}$ für $a_{j}>0$
    und für alle $j=1,\dotsc,m$ ist $h\geq a_{j} \chi_{A_{j}}$. Weiter ist $0=
    \mu(\{\omega\in\Omega| h(\omega)>0\})\geq \mu(\{ \omega\in\Omega|
    \chi_{A_{j}}(\omega)>0\})$. Wir haben also: $\int h\,d\mu= \sum a_{j}
    \int\chi_{A_{j}}\,d\mu$. 
    \begin{itemize}
...
\begin{definition}
  ...
  f\colon[\Omega,\CF]\rightarrow[\R,\FB]$. Dann heißt $\nu$ \highl{absolut
...
\begin{remark}
  \begin{enumerate}
    \setcounter{enumi}{-1}
  \item Schreibweisen: $\int_A fd\mu\colon=\int f\chi_A\,d\mu, \nu\ll\mu$. In
    der Literatur ist auch die folgende Definition gebräuchlich: Seien $\mu$
    und $\nu$ Maße auf einer $\sigma$"~Algebra $\CF$. Dann heißt $\mu$ absolut
    stetig bezüglich $\nu$, wenn für alle $A\in\CF$ gilt:
    \begin{gather*}
      \nu(A)=0 \Rightarrow \mu(A)=0
    \end{gather*}
  \item $\mu f(A)=\int f\chi_{A}\,d\mu$ definiert ein Maß auf $[\Omega,\CF]$\\
    Beweis:
    \begin{itemize}
    \item Fall: $A=\emptyset\colon \mu f(\emptyset)=\int f\chi_\emptyset
      d\mu=\int 0d\mu=0$
    ...
    \end{itemize}
  \item $\mu f$ ist endlich. Es ist per Definition: $+\infty> \mu f(\Omega)
    =\int f\chi_\Omega d\mu=\int fd\mu\Leftrightarrow f$ ist
    $\mu$-integrierbar. 
  \end{enumerate}
...
\begin{beispiel}
  Sei $\Omega=\N_0, \CF=\FP(\N_0)$ und $\mu\colon=\sum_{i=0}^\infty \delta_i$
  das Zählmaß.\\
...
\begin{remark}
  ...
  \item["`$\Rightarrow$"'] Angenommen $\neg f=_\mu g$,
    ...
    die erste Gleichung und sei $A_n\colon=\{\omega\in\Omega|g(\omega)\geq
    f(\omega)+\frac{1}{n}\}\Rightarrow A_n\leq A_{n+1}$. Wegen
    der Stetigkeit von unten folgt: $\lim \mu(A_n)=\mu(\bigcup A_n)=
    \mu(\{\omega\in\Omega|f(\omega)<g(\omega)\})>0\Rightarrow \exists
    n_0\colon \mu(A_{n_0})>0=\mu(\{g(\omega)\geq f(\omega)+\frac{1}{n_0}\})=
    \mu(\{\omega\in\Omega|g(\omega)-f(\omega)\geq \frac{1}{n_0}\})
    \Rightarrow \int (g-f)\chi_{A_{n_0}} d\mu\geq \int \frac{1}{n_0}
...
\begin{satz}
  Seien $\mu$ ein Maß auf $[\Omega,\CF],f,g\colon[\Omega,\CF]\rightarrow
...
\begin{beispiel}
  ...
  $g\colon[\R,\FB]\rightarrow[\R,\FB]$ bezüglich $P$ integrierbar und
...
\begin{satz}[Satz von Fubini]
  Sei $f\colon\times_{i=1}^n [\Omega_i, \CF_i]\rightarrow[\R,\FB]$
  ...
  \omega_n) \mu_{i_1}(d\omega_{i_1})\ldots \mu_{i_n}(d\omega_{i_n})\]
\end{satz}
...
\begin{satz}
  ...
  Maßräume mit $\sigma$-endlichen Maßen. Weiterhin seien $f_i\colon
  ...
  Setzen $f(\omega_1,\ldots,\omega_n)\colon=\prod_{i=1}^n
...
anderen bevorzugt werden. Abstrakt hat man dann, endliches
...
$P(\Omega)=1 =P(\bigcup_{i=1}^n \{\omega_i\})=\sum_{i=1}^n 
P(\{\omega_k\}) \Rightarrow
...
\begin{remark}
  ...
  folgt dargestellt: Sei $\mu=\sum_{\omega\in \Omega} \delta_{\omega}$ ein
  Zählmaß auf $\Omega$. Dann kann man
  das \highl[Zählmaß!normiertes]{normierte Zählmaß} als
  $P(A)=\frac{1}{\mu(\Omega)} \mu$ darstellen. Allgemeiner gilt: Sei
  $\mu$ ein endliches Maß auf $[\Omega,\CF]$ mit $\mu(\Omega)\neq0$. Dann
  kann man $Q\colon=\frac{1}{\mu(\Omega)}\mu$ setzen.
\end{remark}
...
\item Auswahl \emph{mit} Berücksichtigung der Reihenfolge und
  ...
  $A_{k,n}^{mWmR}\colon= \{[\omega_1,\ldots, \omega_k]|
  \omega_i\in \{1,\ldots, n\}, i=1,\ldots, k\}= \{1,\ldots,n\}^k
  \Rightarrow \#A_{k,n}^{mWmR}=n^k$
\item Auswahl \emph{mit} Berücksichtigung der Reihenfolge und
  \emph{ohne} Zurücklegen $(k\leq n)$\\
  $A_{k,n}^{oWmR}\colon=\{[\omega_1,\ldots,\omega_k]| \{\omega_1,\ldots,
  \omega_k\}\in \{1,\ldots,n\}\}=n(n-1)(n-2)\ldots (n-k+1)\Rightarrow
  \#A^{oWmR}_{k,n}=  \frac{n!}{(n-k)!}$\\
  Für den Spezialfall $k=n$ folgt dann: $\#A=n!$. Dies entspricht den
  Permutationen von $\{1,\dotsc,n\}$.
\item Auswahl \emph{ohne} Berücksichtigung der Reihenfolge und
  \emph{ohne} Zurücklegen\\
  $A_{k,n}^{oWoR}\colon=\{\{\omega_1,\ldots,\omega_k\}| \{\omega_1, \ldots,
  \omega_k\} \subseteq \{1,\ldots,n\}\}$\\
  $\Rightarrow\#A_{k,n}^{oWmR}\frac{1}{k!}=
  \frac{n!}{k!(n-k)!} =\binom{n}{k}=\#A^{oWoR}_{k,n}$
\item Auswahl \emph{ohne} Berücksichtigung der Reihenfolge und
  \emph{mit} Zurücklegen\\
  $A_{k,n}^{mWoR}=\binom{k+n}{n}=\frac{(k+n)!}{n!k!}$
\end{enumerate}
...
\begin{beispiel}
  ...
  k=\sum_{i=1}^n \omega_i\}=A_{k,n}^{oWoR}=\binom{n}{k}\Rightarrow
  P(\text{"`$A_k$"'})=\frac{\#A_k}{\#\Omega}=\frac{\binom{n}{k}}{2^n}$.

  Das Ereignis $B_k$ entspricht der Aussage "`beim $k$-ten Wurf wird
  erstmalig Zahl geworfen"':
  \begin{align*}
    B_k & =\{[\omega_1,\ldots,\omega_n]\in\Omega| \underbrace{\omega_1, \ldots,
  \omega_{k-1}}_{=0}, \omega_k=1\}\\
    &= \{[0,\dotsc,0,1]\}\times\{[\omega_{k+1},\dotsc,\omega_{n}]|
       \omega_{j}\in \{0,1\}\}\\
    P(\text{"`$B_k$"'}) &=\frac{\# B_k}{\#\Omega}=\frac{2^{n-k}}{2^n}=2^{-k}
  \end{align*}
...
\begin{align*}
  \Omega &\colon= \{\{\varepsilon_1,\ldots,\varepsilon_n\}| \{\varepsilon_1,
  \ldots, \varepsilon_n\}\subseteq \{1, \ldots, N\}\}=A^{oRoW}_{n,N}\\
  \CF &=\FP(\Omega)\qquad P(A_{k})\colon=\frac{\# A_{k}}{\# \Omega}=\frac{\#
     A_{k}}{\binom{N}{n}}\\
  A_k &= \{\{\varepsilon_1, \ldots, \varepsilon_n\}\subseteq\Omega|\# (\{\varepsilon_1,
  \ldots, \varepsilon_n\}\cap \{1, \ldots, M\})=k\text{ und} \\
...
\binom{N}{n}\] 
Zum Beweis dieser Aussage wissen wir, dass $A_k\cap A_j=\emptyset$ für $k\neq
j$ und
...
\[H_{N,M,n}\colon= \sum_{k=\max(0,n+M-N)}^{\min(M,n)}
...
\begin{remark}
  ...
  \item $\Omega= \{[\omega_1,\ldots, \omega_n]|\omega_i\in \{0,1\},
    i=1,\ldots,n\}=\{0,1\}^{n\times}$
  \item $\CF=\FP(\Omega)$
  \item $P(\{[\omega_1,\ldots,\omega_n]\}) =P(\times_{i=1}^{n} \{\omega_{i}\})
    = (p\delta_1+ (1-p) \delta_0)^{n\times} (\times_{i=1}^n \{\omega_i\})=
    \prod_{i=1}^n (p\delta_1+ (1-p)\delta_0)(\{\omega_i\})= \prod_{i=1}^{n}
    (p\delta_{1})(\{\omega_{i}\})+(1-p)\delta_{0}(\{\omega_{i}\})= \prod_{i=1}^n
...
\begin{beispiel}
  ...
  Erfolgswahrscheinlichkeit von $p=\nicefrac{1}{2}$ nach dem klassichen Modell
  dienen. Wir
  ...
  \begin{align*}
    A_k &= \{[\omega_1,\ldots, \omega_n]\in\Omega| \sum \omega_i=k\}\\
    P(A_k) &= \sum_{[\omega_1,\ldots,\omega_n]\in A_k}
    p^{\sum\omega_i} (1-p)^{n-\sum\omega_i}= \sum_{\omega\in A_{k}}
       P(\{\omega\})\\
    &= p^{k} (1-p)^{n-k} \sum_{\omega\in A_{k}} 1= \sum p^k (1-p)^{n-k}= p^k(1-p)^{n-k} \#A_k\\
    &=
       \begin{cases}
       p^k(1-p)^{n-k}\binom{n}{k} & k=0,\dotsc,n\\
	 0 & \text{sonst}
       \end{cases}
  \end{align*}
  Damit folgt, dass $\sum_{k=0}^n p^k(1-p)^{n-k}\binom{n}{k}=1$. Denn
...
\[B_{n,p} \colon= \sum_{k=0}^n \binom{n}{k} p^k (1-p)^{n-k} \delta_k\]
\begin{beispiel}
  ...
  \[G_p \colon= \sum_{k=1}^\infty p^k (1-p)\delta_k\]
...
\begin{beispiel}
  ...
  auf Probleme stoßen. So ist im Beispiel der Genetik
  $P(\text{"`}k\text{-mal Erfolg}\text{"'})=\binom{n}{k} p^k (1-p)^{n-k}= 
...
Wir betrachten die Binomialverteilung $B_{n,p}$ für sehr große $n$ und kleine
$p$, d.\,h.
\[B_{n,p}(\{k\})=
\begin{cases}
...
und weiter die Poissonverteilung zum Parameter $\lambda$:
\[\Pi_\lambda(\{k\})=\frac{\lambda^k}{k!}e^{-\lambda}\qquad(\lambda>0)\]

\begin{satz}[Poissonsche Grenzwertsatz]
  Sei $(p_n)_{n=1}^\infty$ eine Folge mit $0\leq p_{n}\leq 1$ für alle
  $n=1,2,\ldots$ und $0<\lambda=\lim_{n\rightarrow\infty} np_n$.
  Dann gilt:
  \[\lim_{n\rightarrow\infty} B_{n,p_{n}} (\{k\})=\Pi_\lambda(\{k\})\quad
  (k=0,1,\ldots)\]
\begin{proof}
  in der Übung
\end{proof}
...
\begin{beispiel}
  Seien $n=10^{11}, p=c\cdot 10^{-11}\Rightarrow np=c, \lambda=1$. Es gilt die
  Annahme, $B_{n,p}(\{k\})\approx \Pi_c(\{k\})$. Wir betrachten
  $k=0,1,2,3,4,5$ für $c=1$:
...
Im kontinuierlichen Modell ist $\Omega\subseteq \R^n, 0\leq \ell^{(n)}
(\Omega)< +\infty$:
\[\Rightarrow Gl_\Omega \colon=\frac{\ell^{(n)}}{\ell^{(n)}(\Omega)}\]
...
\begin{definition}
  ...
  \[P(A|B)\colon=\frac{P(A\cap B)}{P(B)}\]
...
\begin{proof}
  ...
  \item Nach der Definition ist klar, dass $P(A|B)\geq 0$
    ...
    erhält man nun $\frac{P(A\cap B)}{P(B)}\leq 1$. Es ist klar, dass
    $P(\emptyset|B)=\frac{P(\emptyset\cap B)}{P(B)}=\frac{P(\emptyset)}{P(B)}
    = 0$ ist.
...
\begin{remark}
  Durch $A\rightarrow P(A|B)$ ist ein Wahrscheinlichkeitsmaß auf
  $[\Omega,\CF]$ gegeben. Man bezeichnet $P(\cdot|B)$ als \highl{bedingte
  Verteilung} unter der Bedingung $B$.
\end{remark}
...
\label{satz:totalWahr}
  ...
  $\Omega$, d.\,h. $B_{i}\in\CF, i\in I$ abzählbar, $B_{i}\cap
  B_{j}=\emptyset$ für $i\neq j, \bigcup B_{i}=\Omega$. Dann gilt:
\[P(A)=\sum_{i\in I} P(A|B_i)P(B_i)\qquad A\in\CF, P(B_i)>0\]
\begin{proof}
  \begin{align*}
...
\paragraph{Informationsübertragungsmodell}

Seien $S_{1}=S_{2}=S$ ein endliches Alphabet. $S_{1}$ bezeichnet das Alphabet
des Senders, $S_{2}$ das des Empfängers. Es ist $\CF=\FP(\Omega)$ und $\Omega=
S_{1}\times S_{2}=S\times S= \{[s_{1}, s_{2}]| s_{1}, s_{2}\in S\}$. $A_{s}$
entspricht dem Ereignis, dass $S$ empfangen und $B_{s}$ das $S$ gesendet wird.
Dabei ist $s\in S$. Weiter ist $A_{s_{2}}=S\times\{s_{2}\}$,
$B_{s_{1}}=\{s_{1}\}\times S$, $A_{s_{2}}\cap B_{s_{1}}=\{[s_{1}, s_{2}]\}$
und $q(s_{1}, s_{2})\colon= P(A_{s_{2}}|B_{s_{1}})$. Die Wahrscheinlichkeit
$p_{s_{1}}$ bezeichnet die a"~priori"=Wahrscheinlichkeit $P(B_{s_{1}})$. Damit
folgt, $P(A_{s_{2}})=\sum_{s_{1}\in S} P(A_{s_{2}}|B_{s_{1}})P(B_{s_{1}})=
\sum_{s_{1}\in S} p_{s_{1}} q(s_{1},s_{2})$. Ideal ist
$q(s_{1},s_{2})=\delta_{s_{1}, s_{2}}$.
...
\begin{definition}
  Ein Wahrscheinlichkeitsmaß $Q$ auf $[\Omega, \CF]$ heißt
  \highl{Lebensdauerverteilung}.
  \begin{itemize}
  \item Sei $Q=\sum_{i=1}^\infty q_i \delta_i$ ($Q$ ist bezüglich des
    Zählmaßes absolut stetig.). Dann heißt
    \[r_Q(i) \colon= \frac{q_i}{\sum_{j\geq i}^\infty q_{i}} \qquad
    q_{i}\geq0, \sum q_{i}=1,  i=1,2, \ldots\]
    \highl{Sterberate} von $Q$ zum Zeitpunkt $i$.
  \item Sei $Q=\ell f$ mit $f\geq 0, \int fd\ell =1$ ($Q$ ist absolut
    stetig mit Dichte $f$.). Dann heißt
    \[r_Q(x) \colon= \frac{f(x)}{\int_{[x,\infty)} f(y)\ell (dy)} \qquad
      x\in\R_+\]
    \highl{Sterberate} vom $Q$ zur Zeit $x\in\R_{+}$.
  \end{itemize}
\end{definition}
...
\begin{proof}
  \begin{enumerate}[(a)]
  \item $Q(\{i\}|\{i, i+1,\dotsc\})= \frac{Q(\{i\}\cap
    \{i,i+1,\dotsc\})}{Q(\{i, i+1,\dotsc\})}=
    \frac{Q(\{i\})}{Q(\{i,i+1,\dotsc\})}= \frac{Q(\{i\})}{\sum_{j\geq i} Q(\{j\})}=
...
\begin{beispiel}
  \begin{enumerate}
   \item Sei $Q=G_{p}=(1-p) \sum_{i=1}^{\infty} p^{i-1}\delta_{i}\Rightarrow
    r_{Q}(i)=1-p$ für $i=1,2,\dotsc$.
   \item Sei $Q=\Ex_{\lambda}=\ell f_{\lambda}$ mit
    $f_{\lambda}=\begin{cases}0 & x\leq0\\ \lambda e^{-\lambda x} &
		   x>0\end{cases}$. Dabei ist $\lambda>0$ gegeben. Sei
    $F(x)=\Ex_{\lambda}((-\infty, x))=\Ex([0,x))= \int_{[0,x)} f_{\lambda}(y)
    \ell(dy)=\int_{0}^{x} f(x)\,dy$. $F$ ist Verteilungsfunktion zu
    $Q\rightarrow \int_{[x,+\infty)} f(y)\ell(dy)=1-Fx)\Rightarrow r_{Q}(x)=
    \frac{f(x)}{1-F(x)}$. Es ist $Q=\Ex_{\lambda}\rightarrow
    F(x)=1-e^{-\lambda x}\Rightarrow r_{Q}(x)= \frac{\lambda e^{-\lambda
    x}}{e^{-\lambda x}}=\lambda$. Also hängt die Sterberate nicht vom
    augenblicklichen Alter ab.
  \end{enumerate}
\end{beispiel}
...
\begin{remark}
  \begin{enumerate}
  \item Sei $P(B)>0$. Dann gilt $A,B$ unabhängig $\Leftrightarrow P(A|B)
...
\begin{remark}
  ...
  \item $\FA$ ist genau dann vollständig unabhängig, wenn $\forall
    \{A_1, \ldots, A_n\}\subseteq \FA$ gilt: $A_1, \bigcap_{k=2}^n A_k$
    unabhängig. Beweisidee: Die betrachtete Unabhängigkeit heißt:
    $P(A_1\cap \bigcap_{k=2}^n A_k)= \prod_{j=1}^{n} P(A_{j})=
    P(A_1)P(\bigcap_{k=2}^n A_k)$. Die andere Richtung wird durch vollständige
    Induktion gezeigt: $k=2 \xrightarrow{n=1} P(A_{1}\cap
    A_{2})=P(A_{1})P(A_{2})$.
  \item $\FA$ ist genau dann vollständig unabhängig, wenn $\forall
    \{A_1, \ldots, A_n\}\subseteq \FA$ gilt, dass $\{A_1, \ldots, A_n\}$
...
\begin{beispiel}
  Sei $A\in\CF, 0<P(A)<1$. Setzen $A_1=A_2=A, A_3=\emptyset
...
\label{satz:vollun-432}
  Wenn $\FA$ vollständig unabhängig ist, dann ist $\tilde{\FA}\colon=
...
\label{satz:unendlEreignisse}
  ...
  Interpretation: $\omega\in\bigcap_{n=1}^{\infty}\bigcup_{i\geq n}
  A_{i}\Leftrightarrow \omega\in\bigcup_{i\geq n} A_{i}$ für $n=1,2,\dotsc
  \Leftrightarrow \#\{i| \omega\in A_{i}\}=+\infty\Rightarrow
  \bigcap_{n=1}^{\infty} \bigcup_{i\geq n} A_{i}$ entspricht dass unendlich
  viele Ereignisse $A_{i}$ eintreten.
\begin{proof}
  Es genügt zu zeigen, dass $P((\bigcap_{n=1}^\infty \bigcup_{i\geq n}
  A_i)^c) =0$. Es ist klar, dass $P((\bigcap_{n=1}^\infty \bigcup_{i\geq n}
  A_i)^c) = P(\bigcup_{n= 1}^{\infty} \bigcap_{i\geq n} A_i^c)\leq
  \sum_{n= 1}^{\infty} P(\bigcap_{i\geq n} A_i^c)$. Damit genügt zu zeigen,
  dass $P(\bigcap_{i\geq n} A_i^c)=0= P(\bigcap_{k=n}^\infty
  \bigcap_{i=n}^k A_i^c)\Rightarrow P(\bigcap_{i\geq n} A_i^c)=
  P(\bigcap_{k=n}^\infty \bigcap_{i=n}^k A_i^c)$.  Wegen der
  Stetigkeit von oben ist $\lim_{k\rightarrow\infty} P(\bigcap_{i=n}^k
  A_i^c)$. Da $A_i$ unabhängig ist, ist auch $A_i^c$ unabhängig. Daraus
  folgt, dass $\lim_{k\rightarrow\infty} \prod_{i=n}^k P(A_i^c)=
  \lim_{k\rightarrow\infty} \prod_{i=n}^k (1-P(A_i))=0$. Somit genügt es
  noch zeigen, dass $\ln (\prod_{i=n}^{k} (1-P(A_{i}))= \sum_{i=n}^{\infty}
  \ln(1-P(A_{i}))= -\infty= \sum_{i=n}^\infty
...
\begin{satz}
  ...
  \[P\left(\bigcap_{n= 1}^{\infty} \bigcup_{i\geq n} A_i\right) =0\]
  Hierbei wurde die Unabhängigkeit \emph{nicht} vorausgesetzt.
\begin{proof}
  Trivialerweise ist: $\bigcap_{n\geq 1} \bigcup_{i\geq n}
  A_i\subseteq \bigcup_{i\geq k} A_i$ und $0\leq P(\bigcap\bigcup A_{i})\leq
  P(\bigcup_{i\geq k} A_{i})$ für $k=1,2,\dotsc$. Es genügt zu zeigen, dass
  $\lim_{k\rightarrow\infty} P(\bigcup_{i\geq k}A_{i})=0$ ist. 
  Es ist, $P(\bigcup_{i\geq k} A_i) \leq \sum_{i\geq k} P(A_i)
    \xrightarrow{k\rightarrow\infty} 0$ und es reicht zu zeigen, dass
  $\lim_{k\rightarrow\infty} \sum_{i\geq k} P(A_{i})=0$. Das folgt aber aus
  $\sum_{i\geq k} P(A_{i})<+\infty$.
\end{proof}
\end{satz}
...
\begin{remark}
  ...
  \begin{inparaenum}[a)]
  \item $P(\bigcap_{n= 1}^{\infty} \bigcup_{i\geq n} A_i)\in \{0,1\}$ ($0,1$-Gesetz)
...
\begin{definition}
  ...
  messbarer Raum.
  \begin{itemize}
  \item Eine messbare Abbildung $\xi\colon [\Omega,\CF]\rightarrow[X,\FX]$
    heißt \highl{Zufallsvariable} über $[\Omega,
    \CF, P]$ mit Werten in $[X,\FX]$.
  \item $P_\xi\colon=\pxi$ heißt
...
\begin{beispiel}[Bernoulli"=Schema]
  \begin{align*}
    \ofp &= [\{0,1\}, \FP(\{0,1\}), p\delta_1+ (1-p)
    \delta_0]^{n\times}\\
    &=[\{0,1\}^{n}, \FP(\{0,1\}^{n}), (p\delta_{1}+
       (1-p)\delta_{0})nx]\\
    \Omega &= \{[\varepsilon_1, \ldots, \varepsilon_n]| \varepsilon_i
    \in \{0,1\}, i=1,\ldots,n\}\\
    \xi_{k}&\colon \Omega\rightarrow\R,
       \xi_{k}([\omega_{1},\dotsc,\omega_{n}]) \colon= \omega_{k}
  \end{align*}
...
Wir setzen $\xi\colon= \sum_{k=1}^{n} \xi_{k}$. Die Frage ist nun, was
$P_{\xi}$ ist. Anzunehmen ist, dass sich die Binomialverteilung ergibt. Dazu
genügt es, statt $P_{\xi}(A)$ nur $P_{\xi}(\{j\})$ zu betrachten
\begin{align*}
  P_{\xi}(\{j\}) &= P(\xi^{-1}(\{j\})) =
     P(\{[\omega_{1},\dotsc,\omega_{n}]\colon
     \xi([\omega_{1},\dotsc,\omega_{n}]) = j \})\\
  &=
     P(\{[\omega_{1},\dotsc,\omega_{n}]\colon \sum_{k=1}^{n}
     \underbrace{\xi_{k}([\omega_{1},\dotsc,\omega_{n}])}_{=\omega_{k}}=j\})\\
  &= P(\{[\omega_{1},\dotsc,\omega_{n}]\colon \sum_{k=1}^{n} \omega_{k}=j\}) =
     \binom{n}{j} p^{j} (1-p)^{n-j}= B_{n,p}(\{j\})\\
  &\Rightarrow P_{\xi}=B_{n,p}
\end{align*}

\begin{satz}
  Sei $Q$ ein Wahrscheinlichkeitsmaß auf $[\R, \FB]$ mit stetiger und
  streng monoton wachsender Verteilungsfunktion $F_Q$. Dann ist
  $\xi=F_Q^{-1}$ eine \highl{Zufallsgröße} über dem
...
\begin{proof}
  ...
  Es gilt, $F_{Q}(x)\in[0,1]$ mit $x\in\R$. Aber $F_{Q}$ kann nie die Werte 0
  oder 1 wegen der strengen Monotonie annehmen $\Rightarrow F_{Q}(x)\in(0,1)$.
  Somit existiert $F_{Q}^{-1}$ auf $(0,1)$ und $F_{Q}^{-1}$ ist stetig
  $\Rightarrow$ $F_{Q}^{-1}$ messbar. Also ist $\xi=F_{Q}^{-1}$ eine
  Zufallsgröße. (Genauere Betrachtungen zu oben.)

  Dazu ist nach Definition $F_{P_\xi}(x)=P_\xi((-\infty,
  x))=P(\xi^{-1}((-\infty,x)))$. Weiterhin ist:
  \begin{align*}
    P_\xi((-\infty, x)) &=  P(\xi^{-1}((-\infty,
      x)))= P(\{y\in (0,1)|\xi (y)\in(-\infty, x)\})\\
    &= P(\{y\in (0,1)|\xi(y)<x\})\\
...
\begin{remark}
  Schreibweise: $F_\xi\colon= F_{P_\xi}$ und $P(\xi^{-1}(A))=\colon P(\text{"`$\xi\in
  A$"'}) \colon= P(\{\omega\in\Omega| \xi(\omega)\in A\}) \Rightarrow
  F_\xi(x) = P(\text{"`$\xi\in(-\infty,x)$"'})=P(\text{"`$\xi< x$"'})$

  Es reicht die Forderung, $F_{Q}$ ist streng monoton wachsend auf $(a,b)$ mit
  $Q((a,b))=1$. Dabei ist $a=\infty, b=-\infty$ zulässig. Ohne Stetigkeit gilt
  die Aussage nicht.
\end{remark}

\begin{beispiel}
  Betrachten die Exponentialverteilung $Q=\Ex_{\lambda}\rightarrow
  Q((0,+\infty))=1, F_{Q}(x)=1-e^{-\lambda x}$ für $x\in(0,+\infty)$. Weitere
  Betrachtung in der Übung.
\end{beispiel}


\section{Unabhängige Zufallsgrößen}

Im folgenden Abschnitt gilt immer, dass $\ofp$ ein Wahrscheinlichkeitsraum und
eine Familie von Zufallsvariablen $(\xi_i)_{i\in I}$ mit Werten in $[X_i,
\FX_i]$ ist.

Spezialfall: $[X_{i}, \FX_{i}]=[\R, \FB]$.

\begin{definition}
  Die Familie $(\xi_i)_{i\in I}$ heißt \highl{unabhängig}, wenn für alle
  Folgen $B_i\in\FX_i\,(i\in I)$ die Familie $\{\xi_i^{-1}(B_{i})|i\in
  I\}\subseteq \CF$ (vollständig) unabhängig ist.
\end{definition}

\begin{satz}\label{satz:chiiunab-521}
  Wegen der Bemerkung h nach der Definition von unabhängigen
  ...
  Andere Formulierung: Die $(\xi_{i})_{i\in I}$ sind genau dann unabhängig,
  wenn für jedes $n\geq2$ und $i_{1},\dotsc, i_{n}\subseteq I$ die Folge
  $\xi_{i_{1}},\dotsc,\xi_{i_{n}}$ unabhängig ist.
\end{satz}
...
\begin{remark}
  ...
  \begin{itemize}
  \item Schreibweise: $\overline{\xi}(\omega)\colon= [\xi_1(\omega),
    \ldots, \xi_n(\omega)]\Rightarrow \overline{\xi}$ ist Zufallsvariable mit
    Werten in $[\R^{n},\FB^{n}]$. \\
...
\begin{proof}
  ...
  \item["`$\Rightarrow$"'] Wir haben die Unabhängigkeit von $\xi_1,
    ...
    P_{\xi_i}(B_i)$ mit $B_{1},\dotsc, B_{n}\in\FB$. Dazu wissen wir:
    \begin{align*}
      P_{\overline{\xi}}(\times_{i=1}^n B_i) &=
      P(\overline{\xi_1^{-1}}(\times_{i=1}^n B_i)) = P(\{\omega\in\Omega|
	 \overline{\xi}(\omega)\in\times_{i=1}^{n} B_{i}\}) \\
      & = P(\text{"`}[\xi_1, \ldots, \xi_n]\in\times_{i=1}^n B_i\text{"'})=
	 P(\text{"`$\xi_i\in B_i$"'}) = P(\bigcap_{i=i}^{n} \{\omega\in\Omega|
	 \xi_{i}(\omega)\in B_{i}\})\\
  ...
  \item["`$\Leftarrow$"'] Wir haben die Aussage,
    ...
    heißt, für $\{i_1, \ldots, i_k\}\subseteq\{i_1, \ldots, i_n\}$ und
    $B_{ij}\in\FB$
    muss $P(\bigcap_{j=1}^k \xi_{ij}^{-1}(B_{ij}))= \prod_{j=1}^k
    P(\xi_{ij}^{-1}(B_{ij}))$ gelten. Wir setzen, $A_j\colon= \R$ mit
    $j\notin\{i_1, \ldots, i_k\}$ und $A_{ij}\colon=B_{ij}$ mit $j=1,
    \ldots, k$.
    \begin{align}
      \label{eq:5231}
...
\begin{beispiel}[Bernoullischema]
  Sei $\ofp= [\{0,1\}, \FP(\{0,1\}),
  B_{1,p}]^{n\times}= [\{0,1\}, \FP(\{0,1\}), p\delta_{1}+
  (1-p)\delta_{0}]^{n\times}= [\{0,1\}^{n\times}, \FP(\{0,1\})^{n\times},
  [p\delta_{1}+ (1-p)\delta_{0}]^{n\times}]$. Wir betrachten $\xi_k([\varepsilon_1, \ldots,
  ...
  Abbildung). Somit folgt, $P_{\overline{\xi}}=(p\delta_{1}+ (1-p)\delta_{0})^{n\times}=\times_{i=1}^n
...
\begin{definition}
  ...
  \[P_1*P_2 \colon= \int P_1(\underbrace{B-x}_{\{y-x|y\in B\}})
...
\[P_{\xi_1+\xi_2} = P_{\xi_1}*P_{\xi_2}\]
\begin{proof}
  Wir setzen $h(x_1, x_2)=x_1+x_2$. Die $x_1, x_2$ sind aus $\R$ und
  somit ist die Abbildung $h$ eine Abbildung von $\R^2$ nach $\R$
  ($h\colon\R^2\rightarrow\R$), die stetig und damit messbar ist.
  \begin{align*}
    \xi_1+\xi_2&= h([\xi_{1}, \xi_{2}])= h([\overline{\xi}])= h\circ \overline{\xi}\Rightarrow
    (\xi_1+\xi_2)^{-1}(B)= \overline{\xi^{-1}} (\hmineins(B))\\
    &\Rightarrow P_{\xi_1+\xi_2}(B)= P\circ(\xi_{1}+\xi_{2})^{-1}=
       P((\xi_1+\xi_2)^{-1}(B))= P(\overline{\xi^{-1}} (\hmineins(B))) =
       P\circ\overline{\xi}^{-1}\circ h^{-1}\\
    &= P_{\overline{\xi}}(\hmineins(B))= P_{\overline{\xi}}\circ h^{-1}(B)=
       P_{\xi_{1}}\times P_{\xi_{2}}(h^{-1}(B))=P_{\overline{\xi}} (\{[x_1,
    x_2]|x_1+x_2\in B\})\\
    &= \int \chi_{\{[x_1, x_2]|x_1+x_2\in B\}}(y_1, y_2)
    P_{\overline{\xi}}(d[y_1, y_2])= \int \chi_{B-y_2} (y_1)
    P_{\overline{\xi}} ([y_1, y_2])= (P_{\xi_{1}}\times P_{\xi_{2}})\circ h^{-1}\\
...
\begin{proof}
  ...
  Behauptung nach \autoref{satz:pchiadd-531} richtig und wir nehmen an, dass die
...
\begin{satz}
  Seien $\xi_1, \xi_2$ unabhängige Zufallsgrößen. Dann gilt:
  \[F_{\xi_1+\xi_2} (y)=\int F_{\xi_1}(y-x) P_{\xi_2}(dx)\]
\begin{proof}
  Wir wenden den \autoref{satz:pchiadd-531} mit $B=(-\infty, y)$ an
  und haben:
  \begin{align*}
    F_{\xi_1+\xi_2} &= P_{\xi_1+\xi_2}((-\infty, y))=P_{\xi_1}*P_{\xi_2}
    ((-\infty, y))\\
...
\begin{satz}
  Seien $\xi_1, \xi_2$ unabhängige Zufallsgrößen mit $P_{\xi_{i}}=\ell f_i$
  ($i=1,2$) und $f_{1}, f_{2}$ stetig. Dann gilt:
  \[P_{\xi_1+\xi_2}=\ell (f_1*f_2)\]
  wobei $f_1*f_2(y)\colon= \int_{-\infty}^{\infty} f_1(y-x)f_2(x)\ell(dx)$.
\begin{proof}
  ...
  aber $\int f_1(x-y)f_2(y)\ell(dy)=f_1*f_2(x)$
\end{proof}
\end{satz}
...
\begin{beispiel}
  ...
  $i=1,2$. Zur Bestimmung von $P_{\xi_1+\xi_2}$ wäre es jetzt möglich,
  ...
  $n_1+n_2,p$ und haben durch den Wahrscheinlichkeitsraum $[\{0,1\},\FP(\{0,1\}),
  B_{1,p}]^{n_1+n_2}$ ein Bernoullischema gegeben. Dabei stellen
  $\hat{\xi_1}\colon= \sum_{i=1}^{n_1} \pi_i$ und $\hat{\xi_2}\colon=
...
\begin{definition}
  Sei $\xi$ eine Zufallsgröße, so dass $E\xi\colon=
...
\begin{remark}
  ...
  \item In älteren Büchern wird $E\xi$ auch manchmal als $M\xi$ bezeichnet.
   \item Ist $\xi$ bezüglich $P$ integrierbar, so ist $E\xi$ endlich. Man
    sagt, $\xi$ hat einen endlichen Erwartungswert.
...
\begin{proof}
  ...
  $[\Omega_1, \CF_1, \mu]\colon= \ofp, [\Omega_2,
  \CF_2]=[\R,\FB], h\colon[\Omega_1,\CF_1]\rightarrow [\Omega_2,\CF_2],
  f\colon[\Omega_2,\CF_2]\rightarrow [\R,\FB]$. Speziell sind $h=\xi$ und
  $f=\iota$. Somit folgt, $\int f\circ hd\mu= \int f d\mu\underbrace{\circ
  \hmineins}_{P\circ\xi^{-1}=P_{\xi}}$
...
\begin{satz}
  ...
  Sei $P_\xi(\Z)=1$\footnote{diskrete Verteilung, $\xi$ nimmt nur
    ganze Zahlen an.} und $\xi$ eine Zufallsgröße mit $P_{\xi}= \sum_{k\in\Z}
  a_{k} \delta_{k}$. Falls $E\xi$ absolut konvergiert, gilt:
...
\begin{satz}
  Sei $P_\xi=\ell f$ mit $f$ stetig. Falls der Erwartungswert $E\xi$
  existiert, gilt:
...
\begin{satz}
  Sei $\xi$ eine nichtnegative Zufallsgröße, d.\,h. $P(\text{"`$\xi\geq 0$"'}) =1 
  \Leftrightarrow P_\xi([0, +\infty))=1$. Dann gilt:
  \[E\xi = \int_{[0, +\infty)} (1-F_\xi(x)) \ell dx\]
\begin{proof}
  Nach \autoref{satz:zgreew-41} ist $E\xi= \int_{[0,x]} 1\ell(dy)=
  \int_{[0,+\infty)} xP_{\xi}(dx)= \int_{[0,+\infty)} (\int_{[0,x)} 1\ell(dy))
  P_{\xi}(dx)= \int \chi_{[0, +\infty)} (x) \int \chi_{[0, x)} (y)\ell(dy)
  P_{\xi}(dx)= \iint \chi_{\{[z_{1}, z_{2}]\in\R^{2}| 0\leq z_{2}<z_{1}\}}(x,y)
  \ell(dy) P_{\xi}(dx)= \iint \chi_{\{[z_{1}, z_{2}]\in\R^{2}| 0\leq z_{2}<z_{1}\}}(x,y)
  P_{\xi}(dx) \ell(dy)= \iint \chi_{[0,\infty)}(y) \chi_{[y,\infty)}(x)
  P_{\xi}(dx) \ell(dy)= \int \chi_{[0,\infty)} (y) (\int \chi_{[y,\infty)}(x)
  P_{\xi}(dx)) \ell(dy)= \int \chi_{[0,\infty)}(y) P_{\xi}([y,\infty))
  \ell(dy)= \int\chi_{[0,\infty)}(y) (1-F_{\xi}(y))\ell(dy)= \int_{[0,\infty)}
  (1-F_{\xi}(y))\ell(dy)$.
\end{proof}
\end{satz}

\begin{definition}
  Sei $\xi$ eine Zufallsgröße, so dass ein endlicher $E\xi^k$ existiert. Dann heißt
...
$E\xi^k \not\Rightarrow P_\xi$. Eine positive Antwort auf die obige
Frage ist nur unter gewissen Beschränktheitsforderungen möglich.

\begin{satz}[Elementare Eigenschaften des Erwartungswerts]
  \begin{enumerate}[(E1)]
  ...
  \item Sei $P(\text{"`$\xi=c$"'})=1\Leftrightarrow P_{\xi}=\delta_{c}\Rightarrow E\xi=c$ 
  \item Sei $P(\text{"`$a\leq \xi \leq b$"'})=1 \Rightarrow a\leq E\xi\leq b$ 
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[(E1)]
  ...
  \item $P(\text{"`$\xi=c$"'})=1\Rightarrow P_\xi=\delta_c$\\ 
    ...
    allgemeinen Fall gilt $\int f(x)\delta_c (dx)= f(x)$. \help{muss das nicht
    eigentlich f(\emph{c}) sein?}
    Noch allgemeiner formuliert: $\int f(x) \sum_{k}
    a_k\delta_{\omega_k} (dx)= \sum_k a_k f(\omega_k)$.
  \item $P(\text{"`$a\leq \xi\leq b$"'})=1 \Rightarrow P_\xi([a,b])=1 
...
\begin{beispiel}
  ...
  \item Sei $P\xi=\ell f$ mit $f(x)=\frac{1}{\pi(1+x)^2}$ für
   ...
   \item $P_{\xi}=\prod_{\lambda}\Rightarrow E\xi=\lambda$
   \item $P_{\xi}=\Ex_{\lambda}\Rightarrow E\xi=\nicefrac{1}{\lambda}$
  \end{enumerate}
\end{beispiel}
...
\begin{proof} Spezialisierung des Übertragungssatzes:
  \begin{align*}
    [\Omega_1, \CF_1, \mu] &:= \ofp & [\Omega_2, \CF_2] &\colon= [\R^2,
    \FB_2]\\
    h &\colon \Omega_1\rightarrow \Omega_2 & h &= [\xi, \eta]\\
    f &\colon \Omega_2\rightarrow \R & f(x,y) &= xy
  \end{align*}
  \begin{align*}
    \Rightarrow \int f\circ h d\mu &= \int f d\mu\circ \hmineins\\
    \int \xi\cdot \eta dP &= \int xy dP_{[\xi,\eta]}d([x,y])\\
    \Rightarrow f\circ h &= \xi\cdot\eta, \mu\circ h^{-1}=P\circ
       h^{-1}=P_{[\xi,\eta]} =P_{\xi}\times P_{\eta}\\ 
...
\begin{remark}
  Ohne die Forderung der Unabhängigkeit von $\xi$ und $\eta$ ist die
  Behauptung im obigen
...
\begin{definition}
  ...
  \[D\xi \colon= E(\xi-E\xi)^2=\int(\xi-E\xi)^{2}\,dP\]
  heißt \highl{Varianz} von $\xi$. Weiterhin heißt
  $\sqrt{D\xi}$ \highl{Standardabweichung} oder \highl{Streuung} von $\xi$.
\end{definition}

\begin{satz}[Elementare Eigenschaften der Varianz]
  \begin{enumerate}[a)]
  \item $D\xi\geq 0$
  \item $D\xi=0\Leftrightarrow
    P_\xi=\delta_{E\xi}$. d.\,h. $P(\text{"`$\xi=c$"'}) =1$, 
    also Determinismus.
  \item $D\xi$ endlich $\Leftrightarrow E\xi^2$ endlich. In diesem
    Fall gilt $D\xi=E\xi^2-(E\xi)^2$
  \item $D(c_1\xi+c_2)=c_1^2D\xi$ mit $c_1, c_2\in\R$ (Varianz einer linearen
    Transformation)
...
\begin{proof}
  ...
  \item
    \begin{itemize}
    \item["`$\Leftarrow$"'] $P_\xi= \delta_{E\xi}\Rightarrow D\xi= \int(x-E\xi)^2 P_\xi
      (dx)= (E\xi-E\xi)^2=0$
    \item["`$\Rightarrow$"'] Sei $\eta\geq0, E\eta=0\Rightarrow
      P(\text{"`$\eta=0$"'})=1$.
    \end{itemize}
  \item $D\xi= E(\xi-E\xi)^2= E(\xi)^2-2e\xi E\xi+(E\xi)^2)= E\xi^2 -
    2E\xi E\xi + (E\xi)^2= E\xi^2- 2(E\xi)^2+(E\xi)^2= E\xi^2-(E\xi)^2$
  \item $D(c_1\xi+c_2)= E(c_1\xi+c_2-E(c_1\xi+c_2))^2=
    E(c_1\xi-c_1E\xi)^2 =E(c_1(\xi-E\xi))^2= c_1^2 E(\xi-E\xi)^2= c_1^{2} D\xi$
...
\begin{beispiel}[für $E\xi\cdot\eta\neq E\xi\cdot E\eta$]
  ...
  Konstante, und $\eta \colon= \xi$. Dann folgt, $E\xi\cdot\eta=E\xi^2$. Da
...
\begin{satz}
  ...
  \[D(\xi+\eta)=D\xi+D\eta\]
\begin{proof}
  Es ist klar, dass aus $\xi,\eta$ unabhängig auch $\xi-E\xi, \eta-E\eta$
  unabhängig folgt.
  \begin{align*}
...
\begin{satz}
  ...
  \item Sei $P_\xi= \sum_{k\in\Z} a_k \delta_k$ und $E\xi$
    endlich. Dann gilt:
    \[D\xi= \sum_{k\in\Z} (k-E\xi)^2 a_k= \sum_{k\in\Z} (k-\sum_{j\in\Z}
    ja_{j})^{2} a_{k} = \sum_{k\in\Z} k^{2}a_{k}- (\sum_{k\in\Z} ka_{k})^{2} \]
  \item Sei $P_\xi=\ell f$ mit $f$ stetig und $E\xi$ endlich. Dann
    ist:
    \[D\xi= \int_{-\infty}^{+\infty} (x-E\xi)^2 f(x)dx=
    \int_{-\infty}^{+\infty} x^{2} f(x)dx- \left(\int_{-\infty}^{+\infty}
    x f(x)dx\right)^{2}\]
  \end{enumerate}
\end{satz}
...
\begin{beispiel}
  \begin{enumerate}
  \item $P_\xi=B_{1,p} =p\delta_1+(1-p)\delta_0\Rightarrow D\xi=
  ...
  \item $P_\xi=B_{n,p}$\\
    ...
    P_{\sum \xi_k}\Rightarrow D\xi=D\sum_{k=1}^{n} \xi_k= \sum_{k=1}^n D\xi_k=
    np(1-p)$. Andererseits gilt auch: $D\xi=\sum_{k=0}^n (k-np)^2p^k
    (1-p)^{n-k}\binom{n}{k} = np(1-p)$
  \end{enumerate}
\end{beispiel}
...
\begin{proof}
  \begin{align*}
    ...
    &= E((\xi-E\xi)^2 + E(\xi-E\xi)(E\xi-a) + (E\xi-a)^2)\\
    &= D\xi+ E(E\xi-a)\underbrace{E(\xi-E\xi)}_{=0} +(E\xi-a)^2\\
    &= D\xi+ (E\xi-a)^2\\
...
\begin{remark}
  ...
  \item Wir betrachten Normierung und Zentrierung: $\eta\colon=
...
\begin{satz}[Markoffsche Ungleichung]
  \label{satz:markoff}
  Sei $\xi\geq 0$ eine nichtnegative Zufallsgröße\footnote{Damit existiert $\int\xi
    d\mu$ nach der Definition zweiter Stufe immer.} und $a\geq0$ eine
  reelle Zahl. Dann gilt:
  \[E\xi\geq aP(\text{"`$\xi\geq a$"'})= aP_\xi([a, +\infty])=a(1-F_\xi(a))\] 
\begin{proof}
  ...
  das Integral monoton ist, folgt $E\xi= \int\xi(\omega)P(d\omega)\geq
  \int a\chi_{\{\hat{\omega} | \xi(\hat{\omega})\geq a\}}dP= a\int
  \chi_{\{\hat{\omega} | \xi(\hat{\omega})\geq a\}}dP= a P(\{\hat{\omega}|
  \xi(\hat{\xi})\geq a\})= aP(\text{"`$\xi\geq a$"'})$. 
\end{proof}
\end{satz}
...
\begin{satz}[Tschebyscheffsche Ungleichung]
  ...
  \[P(\text{"`$\lvert\eta-E\eta\rvert\geq\varepsilon$"'})\leq\frac{D\eta}{\varepsilon^2} 
  \qquad \varepsilon> 0\]
\begin{proof}
  Zum Beweis wird der \autoref{satz:markoff} für folgenden
  Spezialfall angewendet. Wir setzen $\xi:=(\eta-E\eta)^2\geq 0$ und
  $a\colon=\varepsilon^2$. Dann ist klar, dass $E\xi=D\eta$ gilt. Dann
  ergibt sich: $D\eta=E\xi\geq aP(\text{"`$\xi\geq a$"'})= \varepsilon^2
  P(\text{"`}(\eta-E\eta)^2 \geq\varepsilon^2\text{"'})= \varepsilon^2
  P(\text{"`}\lvert\eta-E\eta\rvert \geq\varepsilon\text{"'})$. 
\end{proof}
\end{satz}
...
\label{satz:563}
  ...
  $E\xi_i=a, D\xi_i=\sigma^2$ für $i=1,\ldots, n$. Dann gilt:
  \[P\left(\text{"`}\lvert\frac{1}{n}\sum_{i=1}^n \xi_i-a\rvert
    \geq\varepsilon\text{"'}\right)\leq \frac{\sigma^2}{n\varepsilon^2}\qquad 
    \varepsilon > 0\]
\begin{proof}
  Für den Beweis wird \autoref{satz:tscheby} angewendet und wir
  setzen: $\eta\colon=\frac{1}{n} \sum_{i=1}^n \xi_i$
  \begin{gather*}
    \Rightarrow E\eta = E\left(\nicefrac{1}{n}\sum_i \xi_i\right)=
    \nicefrac{1}{n} \sum_i \underbrace{E\xi_i}_{=a} = a\nicefrac{1}{n}n=a\\
    D\eta= D\left(\nicefrac{1}{n}\sum_i \xi_i\right) =
    \nicefrac{1}{n^2} \sum_i \underbrace{D\xi_i}_{=\sigma^{2}} = \nicefrac{1}{n^2} n\sigma^2
    = \frac{\sigma^2}{n}\\
    \Rightarrow P\left(\text{"`}\lvert\nicefrac{1}{n}\sum\xi_i-a\rvert\geq
      \varepsilon\text{"'}\right) =P(\text{"`}\lvert\eta-E\eta\rvert\geq
    \varepsilon\text{"'}) 
...
\begin{itemize}
\item $n=250$ Münzwürfe mit $\xi_1,\ldots,\xi_{250}$, Bernoullischema mit
  $n=250, p=\nicefrac{1}{2}$.
\item $\xi_i=1$ entspricht der Aussage, dass der $i$-te Wurf Wappen zeigt.
\item Damit sind die $\xi_{1},\ldots,\xi_n$ unabhängig
\item $P_{\xi_i}=B_{1,p}=\nicefrac{1}{2}(\delta_1+\delta_0)$ mit
  $p=\nicefrac{1}{2}\Rightarrow E\xi_i=\nicefrac{1}{2},
  D\xi_i=\nicefrac{1}{2} (1-\nicefrac{1}{2})=\nicefrac{1}{4}$
\item $\lvert\sum_{i=1}^{250} \xi_i-125\rvert < 25=\colon A$ entspricht der
  Aussage, dass ich gewinne.
\end{itemize}
Aus \autoref{satz:563} folgt nun: $P(A)=P(\text{"`}\lvert \nicefrac{1}{n}
\sum_{i=1}^{n} \xi_i-a\rvert\geq\varepsilon\text{"'})<
\frac{\sigma^2}{n\varepsilon^2}=1-P(\text{"`$\abs{\nicefrac{1}{2}
\sum\xi_{i}-a}\geq\epsilon$"'})=1-\nicefrac{1}{10}=0,9$
...
Messungen durchgeführt werden und man so viele $\xi_{1},\ldots,\xi_n$
...
\item Vorgabe eines Sicherheitsniveaus $s<1$ mit
  \begin{align}
    \label{eq:1}
    P(\text{"`}\lvert\eta_n-a\rvert<\varepsilon\text{"'})\geq s 
  \end{align}
\end{enumerate}
...
\begin{proof}
  ...
  $P(\text{"`}\lvert \eta_n-a\rvert <\varepsilon\text{"'})=1- P(\text{"`}\lvert
  \underbrace{\eta_n}_{\nicefrac{1}{n} \sum\xi_i} -a\rvert\geq
  \varepsilon\text{"'})$. 
...
\begin{definition}
  ...
  \[P(\text{"`}\lvert\xi_n-\xi\rvert\geq\varepsilon\text{"'})\xrightarrow{n\rightarrow
    \infty} 0\Leftrightarrow P(\text{"`}\lvert
  \xi_n-\xi\rvert<\varepsilon\text{"'}) \xrightarrow{n\rightarrow\infty} 1\]
\end{definition}
...
\begin{proof}
  Aus \autoref{satz:563} folgt: \help{Beweis unklar}
\end{proof}
\end{satz}
...
$(\eta_i)_{i\geq 1}$ iid sind. Nun betrachten wir die $\xi_i\colon=\xi_i^A
\colon=\chi_A(\eta_i)=\chi_A\circ\eta_i$ für $i=1,2,\ldots$. Diese sind
...
\frac{1}{n\varepsilon^2} \Rightarrow \sup_A P(\text{"`}\lvert \nicefrac{1}{n}
\sum\xi_i -P_{\eta_1}(A)\rvert \geq\varepsilon\text{"'})\xrightarrow{n
  \rightarrow \infty} 0$ 
...
\begin{remark}
  ...
  $P(\text{"`}\lvert \eta_n-\eta\rvert\geq\varepsilon\text{"'})$
  ist konstant und ungleich 0. Außer für den Fall, dass $P_\eta,
...
\begin{definition}
  Sei $(\eta)_{n=1}^\infty, \eta$ eine zufällige Größe. Dann konvergiert
  $(\eta_n)_{n=1}^\infty$ mit Wahrscheinlichkeit 1 (fast sicher) gegen $\eta$,
  wenn gilt\footnote{Schreibweise: $\eta_n\xrightarrow{P=1} \eta$}:
  \[P(\text{"`}\lim \eta_n =\eta\text{"'})=1=P(\{\omega\in\Omega\colon
  \eta(\omega)=\lim_{n\rightarrow\infty} \eta_{n}(\omega)\})\]
\end{definition}

\begin{satz*}
  \[\eta_n\xrightarrow{P=1} \eta \Rightarrow \eta_n\xrightarrow{P} \eta\]
  Die Umkehrung dieses Satzes ist im Allgemeinen falsch.
\end{satz*}

\begin{remark}
  Andere Formulierung:\\
   Wenn zu jeder Folge eine Teilfolge existiert, so dass man immer
   Konvergenz in Wahrscheinlichkeit hat, hat man auch fast sichere Konvergenz.
\end{remark}

\begin{satz}[Starkes Gesetz der großen Zahlen nach Kolmogoroff]
  Sei $(\xi_k)_{k=1}^n$ eine Folge unabhängiger identisch verteilter
  Zufallsgrößen mit endlichem Erwartungswert $a$. Dann gilt:
  \[\frac{1}{n} \sum_{k=1}^n \xi_k \xrightarrow{P=1} a\]
\end{satz}

\begin{remark}
  Betrachten Zufallsgrößen mit $E\abs{\xi}^{p}<+\infty$  und führen hier eine
  Konvergenz ein. Diese Räume werden als $L_{p}$"~Räume bezeichnet. Es gilt
  dann $L_{p}(\Omega,\CF,P)$ und man hat die Spezialfälle:
  \begin{enumerate}
   \item $p=1\rightarrow \rho(\eta,\xi)\colon=E\abs{\xi-\eta}$
   \item $p=2\rightarrow\langle f,g\rangle\colon= \int\overline{f} gdP$. Der
    Raum $L_{2}$ ist der \highl{Hilbertraum} mit $\norm{f}^{2}= \langle
    f,f\rangle$.
  \end{enumerate}
\end{remark}


\begin{satz}[Spezialfall des Satz von Weierstraß]
  Sei $f\colon[0,1]\rightarrow \R$ stetig. Weiter sei $b_n(x):=
  \sum_{k=0}^n f(\frac{k}{n})\binom{n}{k}x^k(1-x)^{n-k}$ ein
  Bernoullipolynom mit $x\in[0,1]$. Dann gilt:
  \[\limn b_n(x)=f(x)\]
  \begin{proof}
    Der Beweis erfolgt durch die Anwendung des schwachen Gesetzes der großen
    Zahlen (\autoref{satz:schwachges-571}).
    Seien $\xi_{1},\dotsc,\xi_{n}$ unabhängige, identische verteilte
    Zufallsgrößen mit $P_{\xi_{i}}=B_{1,x}\Rightarrow E\xi_{k}=x,
    D\xi_{k}=x(1-x)$. Aus dem obigen Gesetz folgt $\nicefrac{1}{n}
    \sum_{k=1}^{n} \xi_{k}\xrightarrow{P} x$. Dies ist gleichwertig zu
    $P_{\nicefrac{1}{n} \sum_{k=1}^{n} \xi_{k}}
    \Rightarrow\delta_{x}\Leftrightarrow \int f(y)P_{\nicefrac{1}{n}
    \sum_{k=1}^{n} \xi_{k}}(dy)\Rightarrow\int f(y)\delta_{x}(dy)=f(x)$. Nach
    dem Übertragungssatz ist $\int f(y)P_{\nicefrac{1}{n}
    \sum_{k=1}^{n} \xi_{k}}(dy)= \int f(\nicefrac{y}{n})P_{\sum_{k=1}^{n}
    \xi_{k}} (dy)= \sum_{k=1}^{n} f(\nicefrac{k}{n}) a_{k}= \sum_{k=1}^{n}
    f(\nicefrac{k}{n}) \binom{n}{k} x^{k} (1-x)^{n-k}$.
  \end{proof}
\end{satz}
...
\begin{enumerate}
  \item Sei $\Omega$ eine nichtleere Menge und es gelte $A_{i}\subseteq
  \Omega, i= 1,2,\dotsc,$ Wir definieren:
  \begin{align*}
    \limsup_{n\rightarrow\infty} A_{n} &\colon= \bigcap_{n=1}^{\infty}
       \bigcup_{k=n}^{\infty} A_{k}\\
    \liminf_{n\rightarrow\infty} A_{n} &\colon= \bigcup_{n=1}^{\infty}
       \bigcap_{k=n}^{\infty} A_{k}
  \end{align*}
  Zeigen Sie:
  \begin{enumerate}
    \item Berechnen Sie $\limsup_{n\rightarrow\infty} A_{n}$ und
    $\liminf_{n\rightarrow\infty} A_{n}$ für die Mengen
    \begin{gather*}
      A_{n} \colon= (\nicefrac{1}{n}, 1+\nicefrac{1}{n}]
    \end{gather*}
   \item Beweisen Sie:
    \begin{gather*}
      \liminf_{n\rightarrow\infty} A_{n} = \{\omega\in\Omega\colon \omega\in
	 A_{n} \forall n\in\N \text{ bis auf endlich viele}\}
    \end{gather*}
   \item Geben Sie eine zu (b) analoge Beschreibung von
    $\limsup_{n\rightarrow\infty} A_{n}$.
   \item Zeigen Sie:
    \begin{gather*}
      \liminf_{n\rightarrow\infty} A_{n}^{c} = \left(
	 \limsup_{n\rightarrow\infty} A_{n}\right)^{c}
    \end{gather*}
    \begin{itemize}
      \item["`$\Leftarrow$"'] $x\in (\bigcap_{n=1}^{\infty}
      \bigcup_{k=n}^{\infty} A_{k})^{c}\Rightarrow x\notin
      \bigcup_{n=1}^{\infty} (\bigcup_{k=n}^{\infty}A_{k}) \Rightarrow \exists
      n_{0}\colon x\notin \bigcap_{k=n_{0}}^{\infty} A_{k} \Rightarrow x\notin
      A_{k}$ für $k=1,2,\dotsc\Rightarrow x\in A_{k}^{c}$ für $k=n_{0},
      n_{0}+1,\dotsc \Rightarrow x\in\bigcap_{k=n_{0}}^{\infty} A_{k}^{c}
      \Rightarrow x\in \bigcup_{n=1}^{\infty} \bigcup_{k=n}^{\infty} A_{k}^{c}
      = \liminf_{n\rightarrow\infty} A_{n}^{c}$.
     \item["`$\Rightarrow$"'] selbst
    \end{itemize}
  \end{enumerate}
 \item Sei $\Omega$ eine nichtleere Menge und $\FE \in \FP(\Omega)$. Die von
  $\FE$ erzeugte $\sigma$-Algebra $\sigma(\FE)$ ist definiert durch
  \begin{gather*}
    \sigma(\FE)=\bigcap \{\CF\colon \CF\supseteq \FE, \CF \text{ ist
       $\sigma$-Algebra über } \Omega\}
  \end{gather*}
  Es sei $\Omega=\{1,2,3,4,5,6\}$ und $\FE=\{\{1,2,3\},\{1,3\}\}$. Ermitteln
  Sie $\sigma(\FE)$.\\
  \begin{tabular}{c|c|c|c|c|c|c|c|c}
    $i$ & 1 & 2 & 3 & 4 & 5 & 6 & 7 & 8\\
    \toprule
    $A_{i}$ & $\emptyset$ & $\{2\}$ & $\{1,3\}$ & $\{1,2,3\}$ & $\{4,5,6\}$ &
       $\{2,4,5,6\}$ & $\{1,3,4,5,6\}$ & $\Omega$
  \end{tabular}

  \begin{tabular}{c|c|c|c|c|c|c|c|c}
    & $A_{1}$ &$A_{2}$ &$A_{3}$ &$A_{4}$ &$A_{5}$ &$A_{6}$ &$A_{7}$ &$A_{8}$\\
    \toprule
    $A_{1}$ & 1 & 2 & 3 & 4 & 5 & 6 & 7 & 8\\
    $A_{2}$ & x & 2 & 4 & 4 & 6 & 6 & 8 & 8\\
    $A_{3}$ & x & x & 3 & 4 & 7 & 8 & 7 & 8\\
    $A_{4}$ & x & x & x & 4 & 8 & 8 & 8 & 8\\
    $A_{5}$ & x & x & x & x & 5 & 6 & 7 & 8\\
    $A_{6}$ & x & x & x & x & x & 6 & 8 & 8\\
    $A_{7}$ & x & x & x & x & x & x & 7 & 8\\
    $A_{8}$ & x & x & x & x & x & x & x & 8\\
  \end{tabular}
 \item Sei $\ofmu$ ein Maßraum mit $\mu(\Omega)<\infty$. Zeigen Sie für
  beliebige meßbare Mengen $A,B\in\CF$ gilt:
  \begin{gather*}
    \mu(A\cup B)= \mu(A)+\mu(B)-\mu(A\cap B)
  \end{gather*}
  Wir können $A\cup B$ auch als $A\cup(B\setminus A)$ aufschreiben und
  erhalten so zwei disjunkte Mengen. Also folgt $\mu(A\cup B) = \mu(A)+
  \mu(B\setminus A)$. Also müssen wir noch zeigen, dass $\mu(B\setminus A)=
  \mu(B) - \mu(A\cap B)$ gilt. Hierzu ist wichtig zu wissen, dass es sich um
  ein endliches Maß handelt. Wir können die Menge $B\setminus A$ zu $B\setminus
  (A\cap B)$ umformen und können einen Satz aus der Vorlesung anwenden:
  $\mu(A\cap B)=\mu(B\setminus (A\cap B))= \mu(B)- \mu(B\cap A)$.
 \item Wir betrachten die messbaren Räume $[\Omega_{k}, \CF_{k}]$ mit
  $k=1,2,\dotsc$. Dabei sei $\Omega_{k}$ endlich oder abzählbar unendlich und
  es gelte $\CF_{k}=\FP(\Omega_{k})$.
  \begin{enumerate}
    \item Zeigen Sie
    \begin{gather*}
      \otimes_{k=1}^{n} \CF_{k} = \FP(\times_{k=1}^{n} \Omega_{k})
    \end{gather*}
    Hierfür müssen wir zeigen, dass $\otimes_{k=1}^{n} \CF_{k} \subseteq
    \FP(\Omega_{k})$ sowie $\FP(\Omega_{k})\subseteq \otimes_{k=1}^{n}
    \CF_{k}$ gilt.

    Es ist $\times_{k=1}^{n} \Omega_{k}=
    \{[\omega_{1},\dotsc,\omega_{n}]\colon \omega_{k}\in\Omega_{k}\}$.
    Weiterhin wissen wir, dass $\otimes_{k=1}^{n} \CF_{k} =
    \sigma(\underbrace{\{\times_{k=1}^{n} A_{k}, A_{k}\in\CF_{k}\}}_{=\FE})$
    und es gilt, $\sigma(\FE)=\bigcap \{\CF \text{ ist $\sigma$-Algebra},
    \FE\subseteq \CF \}$. Somit folgt $\{\times_{k=1}^{n} A_{k},
    A_{k}\in\CF_{k}\} \subseteq \FP(\times_{k=1}^{n} \Omega_{k}) \Rightarrow
    \sigma(\{\times_{k=1}^{n} A_{k}, A_{k}\in\CF_{k}\})\subseteq
    \FP(\times_{k=1}^{n} \Omega_{k})$.
  \end{enumerate}
 \item Zeigen Sie, dass durch nachfolgende Mengenfunktionen
  Wahrscheinlichkeitsmaße auf $[\R, \FB]$ gegeben sind und skizzieren Sie
  jeweils deren Verteilungsfunktion:
  \begin{enumerate}
    \item Das Maß heißt \highl{Binomialverteilung} mit den Parametern $n$ und
    $p$:
    \begin{gather*}
      B_{np} \colon= \sum_{k=0}^{n} \binom{n}{k} p^{k} (1-p)^{n-k} \delta_{k},
	 \qquad p\in[0,1], n\in\N
    \end{gather*}
    Wir betrachten zunächst die allgemeine Formulierung:\\
    Sei $(\mu_{i})_{i\in I}$ eine abzählbare Familie von Maßen auf $[\R,\FB]$
    und $(\alpha_{i})_{i\in I}$ eine Familie nichtnegativer reeller Zahlen.
    Falls gilt, $\sum_{i\in I} \alpha_{i}=1$ und $\mu(\R)=1$, dann ist
    $\mu=\sum_{i\in I} \alpha_{i} \mu_{i}(\R)$ ein Wahrscheinlichkeitsmaß. In
    unserem Beispiel ist $I=\{0,1,2,\dotsc,n\}\subseteq \N_{0}$. Weiter ist
    $\alpha_{i}= \binom{n}{i}p^{i} (1-p)^{n-i}$. Für $0<p<1$ ist $\sum_{i\in
    I}\alpha_{i} = \sum_{i=0}^{n} \binom{n}{i} p^{i} (1-p)^{n-i}=
    (p+(1-p))^{n}= 1^{n}=1$. Es ist klar, dass alle $\alpha_{i}>0$.

    Für die Verteilungsfunktion gilt: $F_{np}(x)= B_{np}((-\infty,x))=
    \sum_{i=0}^{n} \binom{n}{i} p^{i} (1-p)^{n-i}\delta_{i}((-\infty,x))$.
    Dabei bildet $F$ von $\R$ nach $[0,1]$ ab ($F\colon\R\rightarrow [0,1]$).
    \begin{enumerate}[1. F{a}ll]
      \item Falls $-\infty<x\leq0$, dann ist $F_{np}(x)=0$.
     \item Falls $0<x\leq 1$, dann ist $F_{np}(x)=\alpha_{0}$.
     \item Falls $1<x\leq 2$, dann ist $F_{np}(x)=\alpha_{0}+\alpha_{1}$.
     \item Falls $n<x<\infty$, dann ist $F_{np}(x)=\sum_{i=0}^{n}
      \alpha_{i}=1$.
    \end{enumerate}
    Allgemein gilt:
    \begin{gather*}
      P((-\infty,x))=F_{np}(x)= \sum_{i=0}^{\lceil x\rceil-1} \alpha_{i}
    \end{gather*}
   \item Diese Verteilung heißt \highl{Poissonverteilung}:
    \begin{gather*} 
      \pi_{\lambda} \colon= \sum_{k=0}^{\infty} \frac{\lambda^{k}}{k!}
	 e^{-\lambda} \delta_{k}, \qquad \lambda>0
    \end{gather*}
   \item
    \begin{gather*}
      G_{q} \colon= \sum_{k=1}^{\infty} (1-q)q^{k-1}\delta_{k}, \qquad 0\leq
	 q\leq 1
    \end{gather*}
  \end{enumerate}
 \item Es sei $A\in\FB_{n}$ mit $0<\ell^{(n)}(A)<+\infty$ gegeben. Zeigen Sie,
  dass durch:
  \begin{gather*}
    \Gl_{A}(B) \colon= \frac{\ell^{(n)} (A\cap B)}{\ell^{(n)}(A)}, \qquad
       (B\in\FB_{n})
  \end{gather*}
  ein Wahrscheinlichkeitsmaß auf $[\R^{n},\FB_{n}]$ gegeben ist.

  Es ist $\Gl_{A}(\emptyset)=\frac{\ell^{(n)}(A\cap\emptyset)}{\ell^{(n)}(A)}=
  \frac{\ell^{(n)}(\emptyset)}{\ell^{(n)}(A)}=0$. Weiter gilt
  $\Gl_{A}(\R^{n})= \frac{\ell^{(n)}(A\cap\R^{n})}{\ell^{(n)}(A)}=
  \frac{\ell^{(n)}(A)}{\ell^{(n)}(A)}=1$.
 \item $\ofp \xrightarrow{\xi_{i}} [\R, \FB, P_{\xi_{i}}],
  P_{\xi_{i}}=Ex_{\lambda_{i}}, \lambda_{i}>0$. Weiter ist $P_{\xi_{i}}\colon
  F_{i}(x)= (1-e^{-\lambda_{i}x})\chi_{(0,\infty)}(x)=
  \begin{cases}
0 & x \leq0\\
    1-e^{-\lambda_{i}x} & x>0
  \end{cases}
$. Gesucht ist die Verteilungsfunktion der Zufallsgröße $\xi=
  \min\{\xi_{1},\dotsc,\xi_{n}\}$. Wie ist $F(x)$ definiert? Es gilt,
  $F(x)=P_{\xi}((-\infty,x))= P(\min\{\xi_{1},\dotsc,\xi_{n}\}<x)= 1-
  P(\min\{\xi_{1},\dotsc,\xi_{n}\}\geq x)= 1- P(\xi_{1}\geq
  x,\dotsc,\xi_{n}\geq x)= 1- P(\{\omega\in\Omega\colon \xi_{1}(\omega)\geq
  x,\dotsc, \xi_{n}(\omega)\geq x\}= 1- P(\bigcap_{i=1}^{n}
  \{\omega\in\Omega\colon \xi_{i}\geq x\})$ mit $x\in\R$. Dabei sind die
  Ereignisse unabhängig und man kann das schreiben als $1-\prod_{i=1}^{n}
  P(\{\omega\in\Omega\colon \xi_{i}\geq x\})= 1-\prod_{i=1}^{n} P(\xi_{i}\geq
  x)= 1- \prod_{i=1}^{n} (1-F_{i}(x))$. Die weitere Betrachtungen erfordern
  eine Fallunterscheidung. Im Fall $x\leq0$ ist $F(x)= 0$. Für $x>0$ gilt
  $F(x)= 1- \prod_{i=1}^{n} (1-(1-e^{-\lambda_{i}x}))= 1-\prod_{i=1}^{n}
  e^{-\lambda_{i}x}= 1-e^{-\lambda x}$ für $\lambda=\sum_{i=1}^{n}
  \lambda_{i}$. Also folgt, $F(x)= (1-e^{-\lambda x})\chi_{(0,\infty)}(x)$.
  Somit ist $P_{\xi}= \Ex_{\lambda}$.$)$
 \item Es sind $n\geq1$ unabhängige Zufallsgrößen $\xi_{1},\dotsc,\xi_{n}$
  über einem Wahrscheinlichkeitsraum $\ofp$ mit $P_{\xi_{i}}=\Ex_{\lambda},
  \lambda>0$ gegeben. Weiterhin ist $\eta=\xi_{1}+\dotsb+\xi_{n}$. Zeigen Sie:
  $P_{\eta}$ ist absolut stetig bezüglich des \textsc{Lebesgue}-Maßes $\ell$
  auf $[\R, \FB]$ mit der Dichte
  \begin{gather*}
    f_{n}(x)=
       \begin{cases}
	 0 & x\leq0\\
	 \frac{1}{(n-1)!} \lambda^{n} e^{-\lambda x} x^{n-1} & x\geq0
       \end{cases}
  \end{gather*}
  Das Wahrscheinlichkeitsmaß $P_{\eta}$ heißt \textsc{Erlang}-Verteilung mit
  $n$ Freiheitsgraden und dem Parameter $\lambda>0$. Speziell ist $f_{1}$ die
  Dichte der Exponentialverteilung.\\ Wir wissen:
  $\eta=\xi_{1}+\dotsb+\xi_{n}, P_{\eta}= \ell f_{n}$ und müssen eine
  Induktion über $n$ durchführen: Sei $n=2, \eta=\xi_{1}+\xi_{2}$. Dabei sind
  die $\xi_{1}, \xi_{2}$ unabhägig und gleich verteilt. Nach der Vorlesung ist
  $P_{\eta}= P_{\xi_{1}}* P_{\xi_{2}}$. Wir wissen weiter, $f_{\xi_{1}}=
  f_{\xi_{2}}=f_{1}$. Die Dichte ist $(f_{\xi_{1}}* f_{\xi_{2}})(x)=
  \int_{0}^{y} \underbrace{f_{\xi_{1}}(y-x)}_{>0\text{ für } y-x>0}
  \underbrace{f_{\xi_{2}}(x)}_{>0} \ell(dx)= \lambda^{2}\int_{0}^{y}
  e^{-\lambda(y-x)} e^{-\lambda x}\,dx= \lambda^{2} e^{-\lambda y}
  \int_{0}^{y}\,dx= \lambda^{2} e^{-\lambda y}y$. Denn $e^{-\lambda
  x}e^{-\lambda(-x)}=1$. Also folgt, dass $f_{\xi_{1}}*f_{\xi_{2}}=f_{2}$. Es
  gelte nun die Behauptung für $n-1$. Nun ist $P_{\eta_{n}}= *_{i=1}^{n}
  P_{\xi_{i}}= P_{\eta_{n-1}}* P_{\xi_{n}}$.  Es gilt $(f_{n-1}*f_{n})(y)=
  \int_{0}^{y} f_{n-1}(y-x) f_{n}(x)\ell(dx)= \frac{1}{(n-2)!}\lambda^{n-1+1}
  \int_{(0,y)} e^{-\lambda(y-x)} (y-x)^{n-2} e^{-\lambda x}\,\ell(dx)=
  \frac{1}{(n-2)!} \lambda^{n} e^{-\lambda y} \int_{0}^{y} (y-x)^{n-2}\,dx$
  Nun substituieren wir $t=y-x, dt=-dx$ und schreiben das Integral
  $\frac{1}{(n-2)!} \lambda^{n} e^{-\lambda y} (-\int_{y}^{0} t^{n-2}\,dt)=
  \frac{1}{(n-2)!} \lambda^{n} e^{-\lambda y} \int_{0}^{y} t^{n-2}\,dt=
  \frac{1}{(n-2)!(n-1)} \lambda^{n} e^{-\lambda y} t^{n-1}|^{y}_{0}=
  \frac{1}{(n-1)!} \lambda^{n} e^{-\lambda y}y^{n-1}$. Somit folgt die
  Behauptung.
 \item Sei $\ofp$ ein Wahrscheinlichkeitsraum. Berechnen Sie das erste Moment,
  das zweite Moment und die Varianz folgender Zufallsgrößen: $\xi_{2}\colon
  \Omega\rightarrow\N_{0}$ mit $P_{\xi_{2}}= G_{q}, 0<q<1$.\\
  $m_{2}= (1-q) \sum_{k=1}^{\infty} k^{2} q^{k-1}= 2m_{1}\frac{1}{1-q}$
\end{enumerate}
