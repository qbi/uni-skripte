...
\begin{proof}
  ...
  \item[zu (b) und (c)] $A\subseteq B\Rightarrow B=A\cup(B\setminus
    ...
    A)) \overset{\text{\ref{satz:folgenmass}}}{=} \mu(A)+\mu(B\setminus A)$
  ...
  \item[ zu (a)] $A\cup B=A\cup (B\setminus A)\Rightarrow \mu(A\cup
    ...
    \mu(B\setminus A)\subseteq\mu(B)\Rightarrow\mu(A\cup B)\leq
    \mu(A) +\mu(B)$
  \end{itemize}
\end{proof}
...
\begin{proof}
  ...
  \item[zu b)] $A_n\supseteq A_{n+1}\Rightarrow A_n^c\subseteq
    ...
    A_n)^c)= \mu(\Omega\setminus(\bigcap_{n=1}^\infty A_n))
     \overset{\text{\ref{satz:elemEigen} c}}{=} \mu(\Omega)-\mu(\bigcap A_n)$\\
