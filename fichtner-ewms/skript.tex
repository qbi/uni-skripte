...
$[\Omega, \CF, P]$. Diese haben folgende Bedeutung:
\begin{enumerate}[1)]
...
\begin{proof}
  ...
  \item[zu $\FE_6$] $\forall\beta ((-\infty,\beta]^c=(\beta,\infty)
    ...
    Beweis fuer $\sigma(\FE)\subseteq\sigma(\FE_6)$ fehlt noch.
  \end{itemize}
\end{proof}
...
\begin{definition}
  ...
  gilt: \[P(\Omega)=1\]
  In diesem Fall heißt $[\Omega,\CF,P]$
...
\begin{proof}
  ...
  \begin{align*}
    h^{-1} (A\times B) &= \{\omega\in\Omega|[f(\omega),g(\omega)]\in
    A\times B\}\\
  ...
  g^{-1}(B)\in\CF$) gilt für alle $A,B\in\FB: h^{-1}(A\times B)\in
...
\begin{definition}[2. Stufe]
  Sei $f\geq 0$ und messbar. Dann existieren nach Satz
  \ref{satz:limfolge-2110} $(f_n)_{n=1}^\infty, 0\leq f_n\leq
  f_{n+1}, f=\lim f_n, f_n$ einfach. Wir setzen:
...
\begin{definition}
  Ein Wahrscheinlichkeitsraum $[\Omega,\CF, P]$ heißt
...
Die kombinatorischen Formeln drücken die Auswahl von $k$ Elementen
einer $n$-elementigen Menge aus. Dabei kann man sich vorstellen, dass
aus eine Urne mit insgesamt $n$ numerierten Kugeln nach bestimmten
Kriterien Kugeln gezogen werden.
\begin{enumerate}[1. V{a}r{i}{a}nte]
\item Auswahl \emph{mit} Berücksichtigung der Reihenfolge und
  \emph{mit} Zurücklegen\\
  Menge der Auswahlen: $A_{k,n}^{mWmR}:= \{[\omega_1,\ldots, \omega_k]|
...
\item Auswahl \emph{ohne} Berücksichtigung der Reihenfolge und
  ...
  \omega_k\} \subseteq \{1,\ldots,n\}\}=\#A_{k,n}^{oWmR}\frac{1}{k!}=
...
\begin{definition}
  ...
  Wahrscheinlichkeitsraum $[\Omega,\CF,P]=[\{0,1\},\FP(\{0,1\}),
...
\begin{remark}
  \setcounter{enumi}{-1}
  \begin{enumerate}
  \item $[\Omega,\CF,P]=[\{0,1\}^{n\times},\FP(\{0,1\}^{n\times}),
...
Im folgenden gilt immer, dass $[\Omega, \CF, P]$ ein
...
\begin{proof}
  \begin{align*}
    ...
    P(\bigcup_{i\in I} (A\cap B_i))&= P(A\cap \bigcup_{i\in I} B_i) &=
    P(A\cap \Omega)=P(A)
  \end{align*}
\end{proof}
...
\begin{definition}
  Seien $[\Omega,\CF,P]$ ein Wahrscheinlichkeitsraum und $[X,\FX]$ ein
  messbarer Raum.
  \begin{itemize}
...
\begin{beispiel}
  \begin{align*}
    [\Omega, \CF, P] &= [\{0,1\}, \FP(\{0,1\}), p\delta_1+ (1-p)
    \delta_0]^{n\times}\\
...
Im folgenden Abschnitt gilt immer, dass $[\Omega, \CF, P]$ ein
...
%%Probleme mit dem Ausfuehrungszeichen, wird als 1 dargestellt.
...
\begin{proof}
  ...
  \begin{align*}
    ...
    (\xi_1+\xi_2)^{-1}(B)= \overline{\xi^{-1}} (h^{-1}(B))\\
    &\Rightarrow P_{\xi_1+\xi_2}(B)= P((\xi_1+\xi_2)^{-1}(B))=
    P(\overline{\xi^{-1}} (h^{-1}(B)))\\
    &= P_{\overline{\xi}}(h^{-1}(B))=P_{\overline{\xi}} (\{[x_1,
    x_2]|x_1+x_2\in B\})\\
...
Im folgenden sind immer $[\Omega, \CF, P]$ ein Wahrscheinlichkeitsraum
...
\begin{beispiel}
  ...
  \item $P_\xi= B_{n,p} = \sum_{k=0}^n \binom{n}{k} p^k (1-p)^{n-k}
    ...
    (1-p)^{n-k} =np$\\
    Ein anderer Weg, um dies zu ermitteln: Sei $[\Omega, \CF, P] =
...
\begin{proof}
  ...
  \end{align*}
  \begin{align*}
    \Rightarrow \int f\circ h d\mu &= \int f d\mu\circ h^{-1}\\
...
\begin{definition}
  Sie $\xi$ eine Zufallsgröße mit \emph{endlichem} Erwartungswert.
  \[D\xi := E(\xi-E\xi)^2\]
...
\begin{satz}
  ...
  \item $D\xi=0\Leftrightarrow
    P_\xi=\delta_{E\xi}$. D.h. $P("`\xi=c"') =1$, also Determinismus.
...
\begin{proof}
  ...
  \item $D\xi= E(\xi-E\xi)^2 = \int (\xi-E\xi)^2dP$. Nach dem
    ...
    h^{-1}=P_\xi$. Weiter folgt somit $E(\xi-E\xi)^2=\int f(x)P_\xi
