...
\begin{remark}
  ...
  \item Die Umkehrung von \autoref{satz:s-alg} ist in der Regel
    ...
    \[\Omega=\N; \CF\colon=\{A\subseteq\N| A\vee A^c\text{ endlich}\}\]
    Eigenschaften:
    \begin{enumerate}[(1)]
    \item klar
...
\begin{definition}
  ...
  \item Eine nichtnegative Funktion $\mu$ auf
    $\CF$\footnote{$\mu\colon\CF\rightarrow [0,+\infty]$} heißt
...
\begin{proof}
  \begin{itemize}
  \item[zu a)] Setzen $B_1\colon=A_1, B_{n+1}=A_{n+1}\setminus A_n$. Damit
...
\todo{Beweis muss ergänzt werden}
\end{satz}


\begin{satz}[Satz von Weierstraß]
  Sei $f\colon[0,1]\rightarrow \R$ stetig. Weiter sei $b_n^f(x):=
