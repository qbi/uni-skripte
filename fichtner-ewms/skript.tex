...
\todo{Informationsübertragungsmodell ausgelassen}
...
\begin{definition}
  ...
  \item Sei $Q=\sum_{i=1}^\infty q_i \delta_i$ ($Q$ ist bezüglich des
    ...
    \[r_Q(i) \colon= \frac{q_i}{\sum_{j=i}^\infty q_{i} \delta_{i}} \qquad i=1,2, \ldots\]
    \help{Ist die Formel oben korrekt?}
    \highl{Sterberate} von $Q$.
  \item Sei $Q=\ell f$ mit $F\geq 0, \int fd\ell =1$ ($Q$ ist absolut
    stetig mit Dichte $f$.). Dann heißt
    \[r_Q(x) \colon= \frac{f(x)}{\int_{[x,a)} f(y)\ell (dy)} \qquad
      x\in\R_+\]
...
\todo{Zwei Beispiele weggelassen.}
