...
\begin{beispiel}
  ...
  \item Sei $\CF=\FP(\Omega)$. Dann ist jede Abbildung
    $f\colon\Omega\rightarrow X$ messbar.
  \end{enumerate}
\end{beispiel}
...
\begin{satz}
  ...
  \item $f^+\colon=\max (f,0)$ ist messbar.
  \item $f^-\colon=\min (f,0)$ ist messbar.
  \item $|f|$ ist messbar.
  \end{enumerate}
\begin{proof}
  \begin{enumerate}[zu (1)]
...
\begin{satz}[Übertragungssatz]
  ...
  f\colon[\Omega_2, \CF_2]\rightarrow[\R,\FB]$, wobei $f$ bezüglich $\mu$
...
\begin{proof}
  ...
  \item Sei $f=\chi_A$ mit  $(A\in\CF_2)$ und damit messbar. Wir betrachten
    ...
    $\exists B\subseteq\Omega_1$ mit $\chi_A\circ h=\chi_B$\\
