...
\begin{beispiel}
  ...
  \item Sei $\Omega_1$ bis $\Omega_n$ abzählbar:
    ...
    $\{\underbrace{[\omega_1,\ldots,\omega_n]}_{\times_{i=1}^n
      \{\omega_i\}}\} \in
    \otimes_{i=1}^n \CF_i$ für $\omega_i\in\Omega_i, i=1,\ldots,n$ \todo{stimmt das omega-i hier?}. Der Rest
...
\begin{beispiel}
  ...
  dass $\Gl_{A}$ ein Wahrscheinlichkeitsmaß ist. Denn es gilt
  $\Gl_{A}(\emptyset)=\frac{\ell(\emptyset)}{\ell(A)}=0$ und $\Gl_{A}(\R)=
  \frac{\ell(A\cap \R)}{\ell(A)}= \frac{\ell(A)}{\ell(A)}=1$. Sei nun
  $(A_{k})_{k=1}^{\infty}$ eine Folge paarweiser disjunkter Mengen aus $\FB$
  und es folgt $\Gl_{A}(\bigcup A_{k})= \frac{\ell(A\cap \bigcup
  A_{k})}{\ell(A)} = \frac{\ell(\bigcup (A\cap A_{k}))}{\ell(A)}= \frac{\sum
  \ell(A\cap A_{k})}{\ell(A)}= \sum \Gl_{A} (A_{k})$. Für den Spezialfall
