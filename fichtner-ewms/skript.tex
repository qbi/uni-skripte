...
\usepackage{pst-plot}
\usepackage{wasysym}
\usepackage{booktabs}     % für den Blitz
...
\begin{beispiel}
  Sei $\lambda>0$. Wir setzen $F_{\lambda}(x):=
  \begin{cases}
    1-e^{-\lambda x} & x>0\\
    0 & x \leq 0
  ...
  \[\Gl_A(B):=\frac{\ell^{(n)}(A\cap B)}{\ell^{(n)} (A)}\qquad
  (B\in\FB_n)\]
...
\begin{beispiel}
  \begin{enumerate}
  \item $f(\omega)=c\in X\colon\fmineins(A)=
    \begin{cases}
      \emptyset & c\notin A\\
      \Omega & c\in A
...
\begin{satz}
  ...
  f_1\colon [\Omega_1,\CF_1]\rightarrow[\Omega_3,\CF_3]$.
\begin{proof}
...
\begin{proof}
  ...
  umgewandelt: $h:=[f,g]\colon\Omega\rightarrow\R^2, [f,g](\omega)=
...
\begin{beispiel}
  ...
  \item Wir betrachten die Reihe $\sum_{k=0}^m a_k$. Dabei ist auch
    ...
    $\mu(A)= \#A$. Setzen $f\colon\Omega\rightarrow\R$ mit $f(k):=a_k
...
\begin{remark}
  Schreibweise: $F_\xi:= F_{P_\xi}$ und $P(\xi^{-1}(A))=: P(\text{"`$\xi\in
