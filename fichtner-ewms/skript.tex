...
\begin{beispiel}
  ...
  \item Sei $\CF=\FP(\Omega)$. Dann ist jede Abbildung
    ...
    \fmineins(A)\in\CF\ (A\subseteq X)$. F"ur beliebige $\FX$ ist $f$
    $(\FP(\Omega),\FX)$"~messbar.
  \end{enumerate}
\end{beispiel}
...
\begin{proof}
  ...
  \item Sei $f\colon=\inf f_1\Rightarrow f\colon\Omega\rightarrow\R$. Wegen
    ...
    f_{i}(\omega) \geq x\}\in\FB$ f"ur $x\in\R$. Dann folgt nach
...
\begin{proof}
  ...
  \end{cases}\]
  F"ur die Gleichheit m"ussen wir beide Richtungen zeigen: Sei $\chi_{A}$
  messbar. Dann ist $\{1\}\in\FB\Rightarrow\chi_{A}^{-1}(\{1\})= A\in \CF$.\\
  Sei $A\in\CF$. Dann ist zu zeigen, dass $\chi_{A}$ messbar ist, d.\,h.
  $\chi_{A}^{-1}(B)\in\CF$ f"ur $B\in\FB$. Es folgt, dass $\chi_{A}^{-1}(B)\in
  \{\emptyset, \Omega, A,A^{c}\}$ und somit gen"ugt es zu zeigen, dass
...
\begin{proof}
  ...
  \item
    ...
    \item["`$\Leftarrow$"'] Setzen $\{b_1,\ldots,b_n\}=\{f(\omega)|
      \omega\in\Omega\}$ und $B_i=\fmineins(\{b_i\})$ f"ur $i=1,\dotsc,n$.
      Damit folgt
...
\begin{proof}
  ...
  \todo{Erg"anzung von Carolin einf"ugen}
  \begin{align*}
...
\begin{beispiel}
  \begin{enumerate}
  \item Sei $\mu=\delta_{\omega_0}\Rightarrow f=g\ \mu"~\text{fast-"uberall}\Leftrightarrow
    f(\omega_0)=g(\omega_0)$ f"ur $\omega_{0}\in\Omega$.
...
\begin{remark}
  ...
  ist konstant und ungleich 0. Außer f"ur den Fall, dass $P_\eta,
