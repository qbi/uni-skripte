...
\item
F"ur $B\in \Algeb$ mit $\WKM(B)>0$ ist die {\it bedingte
Wahrscheinlichkeit} $\WKM(\,.\,|B)$ durch
 \[\WKM(A|B)=\frac{\WKM(A\cap B)}{\WKM(B)}\quad\mbox{f"ur}\;\;A\in\Algeb\]
...
beachte, da"s $\WKM(\,.\,|B)$ ebenfalls ein Wahrscheinlichkeitsma"s
...
$\WKM(B_j)>0$ und f"ur $A\in \Algeb$ mit $A\subseteq\bigcup_1^n B_j$
folgt stets
\[\WKM(A)=\sum_{j=1}^n\WKM(B_j)\cdot\WKM(A|B_j)\;.\]
\item
Unter obigen Voraussetzungen an $B_1,\ldots,B_n$ und $A$ mit
$\WKM(A)>0$ lautet die {\it Formel von Bayes} wie folgt:
\[\WKM(B_k|A)=\frac{\WKM(B_k)\cdot\WKM(A|B_k)}{\sum_{j=1}^n
\WKM(B_j)\cdot\WKM(A|B_j)}\quad\mbox{f"ur}\;\;k=1,\ldots,n\;.\]
\item
Mengen $A_1,\ldots,A_n\in\Algeb$ hei"sen ({\it stochastisch}) {\it
unabh"angig}, wenn
\[\WKM(A_{i_1}\cap\cdots\cap A_{i_m})=\prod_{j=1}^m\WKM(A_{i_j})\quad
...
\item
Sei $\WKM$ ein Wahrscheinlichkeitsma"s auf $\Omega$ und existiert eine
h"ochstens abz"ahlbar unendliche Menge
$D=\{\omega_1,\omega_2,\ldots\} \subseteq\Omega$ mit $\WKM(D)=1$, so
nennt man $\WKM$ {\it diskret}. $\WKM$ wird dann eindeutig durch die
Folge $(p_i)_{i\ge 1}$ mit
$$
\WKM(\omega_i):=\WKM(\{\omega_i\})=p_i,\quad i=1,2,\ldots,
$$
bestimmt, denn man hat $\displaystyle\WKM(A)=\sum_{\omega_i\in A}
...
\[\WKM(A):=\int_A p(x)dx\]
ein Wahrscheinlichkeitsma"s $\WKM$ auf $\Omega$ (oder $\R^n$)
definiert. Ein so erzeugtes Wahrscheinlichkeitsma"s hei"st {\it
stetig} und $p$ nennt man die ({\it Wahrscheinlichkeits}--) {\it
Dichte} von $\WKM$.
\item
F"ur ein Wahrscheinlichkeitsma"s $\WKM$ auf $\R$ ist
\[\F(x):=\WKM((-\infty,x]),\quad x\in\R\]
die {\it Verteilungsfunktion} von $\WKM$. Sie bestimmt $\WKM$ eindeutig,
und es gilt
$$
p(x)=\F'(x)\;,
$$
sofern $\WKM$ die Dichte $p$ auf $\R$ besitzt.
\end{enumerate}
\item
{\large Zufallsvariable}
\begin{enumerate}
\item
Sei $(\Omega,\Algeb,\WKM)$ ein Wahrscheinlichkeitsraum. Eine Abbildung
...
\item
Sei $X$ eine auf $(\Omega,\Algeb,\WKM)$ definierte ZG. Dann wird durch
\[\WKM_X(B)=\WKM(X^{-1}(B))=\WKM(X\in B)\]
ein Wahrscheinlichkeitsma"s $\WKM_X$ auf $(\R,\BorelM(\R))$ definiert,
das man {\it Verteilungsgesetz} der ZG $X$ nennt.
\item
Zwei ZG $X$ und $Y$ hei"sen {\it identisch verteilt} (Schreibweise:
$X\stackrel{d}{=}Y$), wenn $\WKM_X=\WKM_Y$ gilt. Dabei m"ussen $X$ und
...
\item
Das Verteilungsgesetz $\WKM_{\vec X}$ eines zuf"alligen Vektors $\vec
X=(X_1,\ldots,X_n)$, definiert durch $\WKM_{\vec X}(B)=\WKM(\vec X\in
B)$ f"ur $B\in \BorelM(\R^n),$ nennt man auch {\it gemeinsame
Verteilung} der ZG $X_1,\ldots,X_n$, und $\WKM_{X_1},\ldots,\WKM_{X_n}$
sind dann die {\it Randverteilungen} von $\WKM_{\vec X}$. Man beachte,
da"s die gemeinsame Verteilung durch
\[\WKM_{\vec X}(B_1\times\cdots\times B_n)=\WKM(X_1\in B_1,\ldots,X_n\in B_n)\]
...
schlie"sen.
\item
Eine ZG $X$ hei"st {\it diskret}, wenn $\WKM_X$ diskret ist, und {\it
stetig} f"ur $\WKM_X$ mit Dichte $p$ ($p$ hei"st auch {\it
Verteilungsdichte} von $X$).
\item
F"ur eine ZG $X$ ist ihre {\it Verteilungsfunktion} $\F_X$ als
Abbildung von $\R$ nach $[0,1]$ durch
\[\F_X(x)=\WKM(X\le x)=\WKM(\omega\in\Omega:\;X(\omega)\le x)\]
definiert. $\F_X$ ist monoton wachsend und rechtsseitig stetig.
$\F_X$ ist genau dann stetig, wenn $\WKM(X=x)=0$ f"ur alle $x\in\R$
gilt. Ein Sprung von $\F_X$ der H"ohe $h$ in $x_0$ ist
gleichbedeutend mit $\WKM(X=x_0)=h$.
\item
...
\[ (\ast)\quad \WKM(X_1\in B_1,\ldots,X_n\in B_n)=\prod_{j=1}^n\WKM(X_j\in
...
(existierenden) Dichten der Randverteilungen $\WKM_{X_j}$
sind.\newline Im Fall von $\WKM_{\vec X}$ diskret, gen"ugt es $(\ast)$
f"ur Elementarereignisse mit positiver Wahrscheinlichkeit
nachzuweisen.
\item
F"ur unabh"angige ZV $X_1$ und $X_2$ mit Werten in $\N_0$ berechnet
sich das Verteilungsgesetz $\WKM_{X_1+X_2}$ durch
\[\WKM_{X_1+X_2}(k)=\sum_{j=0}^k \WKM(X_1=j)\cdot\WKM(X_2=k-j)
...
\[{\E}\,X=\sum_{j=1}^\infty p_j x_j =\sum_{j=1}^\infty x_j\,\WKM(X=x_j)\]
definiert ist.
\item
Besitzt $\WKM_X$ eine Verteilungsdichte $p$, so existiert f"ur
...
\[{\E}\,X=\int_0^\infty \WKM(X>t)dt= \int_0^\infty \WKM(X\ge t)dt\;.\]
...
&&\E\, f(X)=\sum_{j=1}^\infty f(x_j)\WKM(X=x_j),\quad X\;\mbox{diskret, bzw.}\qquad\\
...
\[{\E}\,X^n=\sum_{j=1}^\infty x_j^n\; \WKM(X=x_j)\]
...
Wahrscheinlichkeitsma"s $\WKM$ auf $\Omega$ die Aussage
$\WKM(\{\omega\})=1/N$ f"ur alle $\omega\in \Omega$, so hei"st $\WKM$
{\it Gleichverteilung}  oder  {\it klassische Wahrscheinlichkeit}
auf $\Omega$. F"ur eine Menge $A\subseteq \Omega$ folgt dann
$\WKM(A)=\#(A)/N$ (Anzahl der g"unstigen F"alle dividiert durch die
Anzahl aller F"alle).\newline Bemerkung: Ist eine ZG $X$
gleichverteilt auf $\{x_1,\ldots,x_N\}\subseteq\R$, d.h.~$\WKM_X$ ist
...
f"ur ein $x_0\in \R$ die Aussage $\WKM(X=x_0)=1$, so ist $\WKM_X$ die
...
d.h.~man hat $\WKM_X=B_{n,p}$, so gilt ${\E}\,X =np$ und
...
\[\WKM(X=k)=\frac{{M\choose k}{N-M\choose n-k}}{{N\choose n}}\]
f"ur $k\in\N_0$ mit $\max\{0,M-N+n\}\le k\le \min\{M,n\}$ und
$\WKM(X=k)=0$ sonst, so ist $X$ {\it hypergeometrisch} verteilt mit
...
\[\WKM\big(\,\mbox{"'1"' erscheint erstmals im $(k+1)$--ten Versuch}\,\big)
=p\, (1-p)^{k}\,.\] Eine ZG $X$ hei"st nun {\it geometrisch}
verteilt mit Parameter $p\in(0,1)$, wenn
\[\WKM(X=k)= p\,(1-p)^{k}\quad\mbox{f"ur}\;\;k\in\N_0\]
...
\[\WKM(X<x)=\frac{1}{\sqrt{2\pi}\,\sigma}\int_{-\infty}^x
...
$\lambda>0$, wenn $\WKM_X$ die Dichte $p$ der Gestalt
...
gleichverteilt}, wenn $\WKM_X$ die Dichte $p$ mit
...
die $X_j$ unabh"angig und die Verteilungsgesetze $\WKM_{X_j}$ sind
...
\[\WKM\big\{\,|X-\E\,X|\ge c\big\}\le\frac{\V(X)}{c^2}\quad
...
\[\lim_{n\to\infty}\WKM\Big\{\Big|\frac{X_1+\cdots+X_n}{n}-a
...
\[\WKM\Big\{\Big|\frac{X_1+\cdots+X_n}{n}-a
...
\[\lim_{n\to\infty}\WKM\Big\{\Big|\frac{\#\{j\le n;\,X_j\in A\}}
{n}-\WKM_{X}(A)\Big|>\varepsilon\Big\}=0\quad\mbox{f"ur alle}\;\;
...
\[\lim_{n\to\infty}\WKM\Big\{\Big|\frac{\mu_n}{n}-p\Big|<\varepsilon\Big\}=1\quad
...
$\Omega_0\subseteq\Omega$ mit $\WKM(\Omega_0)=1$  gilt sogar
...
\WKM_{X}(A)\;,\] d.h.~die relativen H"aufigkeiten einer Menge
...
\[\lim_{n\to\infty}\WKM\Big\{\alpha\le\sum_{j=1}^n\frac{X_j-a}{
...
hat man
\[\lim_{n\to\infty}\WKM\Big\{n^{-1/2}\sum_{j=1}^nY_j\le t\Big\}=\Phi(t)\]
...
folgt
\[\lim_{n\to\infty}\WKM\Big\{\alpha\le\sum_{j=1}^n\frac{X_j- p}
{\sqrt{np(1-p)}}\le
\beta\Big\}=\frac{1}{\sqrt{2\pi}}\int_\alpha^\beta
e^{-\tau^2/2}d\tau\quad\mbox{f"ur}\;\;\alpha<\beta\;.\] In anderen
Worten, es gilt
\[\lim_{n\to\infty}\sup_{t\in\R}\big|\WKM
\big\{\sum_{j=1}^nX_j\le t\big\}-\WKM(Y_n\le t)\big|=0\;,\] wobei die
