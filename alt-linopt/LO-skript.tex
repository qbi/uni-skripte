...
\newtheorem{defin}[bsp]{Definition}
...
Funktion $f:\R^n\rightarrow \R$ mit dem Ziel der Berechnung eines
Minimums von $f$.

\begin{bsp}
  \begin{inparaenum}[(1)]
  \item $f:\R\rightarrow\R$ mit $f(x)=x^2$ hat genau ein Minimum.
  \item $f:\R\rightarrow\R$ mit $f(x)=x$ ist nicht nach unten
    beschränkt und hat daher kein Minimum.
  \item $f:\R\rightarrow\R$ mit $f(x)=\sin x$.
  \end{inparaenum}
\end{bsp}

\begin{bsp}
  Die Funktion $f:\R^2\rightarrow\R$ mit $f(x_1,x_2)= x_1 \sin (x_1+
  x_1 x_2\sin(x_2))$.
\end{bsp}
...
Radius $r$ definiert durch $B(\tilde{x}, r)=\{x\in\R^n: \lVert
x-\tilde{x} \rVert< r\}$.

\paragraph{Allgemeine Formulierung des Optimierungsproblems}

Sei $D\subset \R^n$ nichtleer und offen, $f:D\rightarrow \R,
\FF\subset D$:
\begin{equation}
  \label{eq:P}
  \min_{x\in\FF} f(x)
...
\begin{bsp}
  ...
  In diesem Fall ist $\FF=\{x\in\R: x\geq 1\}$
\end{bsp}
...
\begin{defin}[Lokale Minima]
  ...
  (\ref{eq:P}), falls es ein $r>0$ mit $f(x)\geq f(\tilde{x})$ für
...
\end{equation}
mit der Zielfunktion $f:\R^n\rightarrow\R$ und der zulässigen Menge
...
\begin{bem*}
  ...
  \item Sinnvollerweise gilt: $l_x\leq u_x, l_A\leq u_A$. Gilt für
    einen Index $i\in\{1, \ldots, n\}: l_{x_i}=u_{x_i}$, d.\,h. wir
    fordern $l_{x_i}=x_i=u_{x_i}$, dann ist $x_i$ fest
  \item Das lineare Funktional $\FF=\{x\in\R^n : l_x\leq x\leq u_x,
    l_A\leq A^{(1)}\leq u_A, A^{(2)}x=b\}$ heißt zulässige Menge
    (feasible set).
  \item Ein Punkt $\tilde{x}\in\FF$ (zulässiger Punkt) heißt
    \emph{optimal}, falls $\forall x\in\FF: f(x)\geq f(\tilde{x})$
...
\begin{defin}
  ...
  Fall ist die zulässige Menge $\FF= \{x\in\R: Ax\geq b, x\geq 0\}$.
\end{defin}

\begin{bsp}
  siehe Beispiel \ref{bsp:prodplan-16}
  \begin{align*}
...
\begin{bem*}
  ...
  für konvexe Zielfunktion. Dabei heißt eine Funktion $f\colon
...
\begin{satz}
  ...
  \begin{proof}
    ...
    $\FF$ (Bemerkung~\ref{bem:224}). Sei jetzt $I(x)=\emptyset$ ($x$
...
\begin{bsp}[siehe Beispiel 2.26\todo{Referenz}]
  ...
  \end{align*}
  Um Satz~\ref{satz:228} anzuwenden, müssen wir das Problem in
...
\begin{satz}
  ...
  \begin{proof}
    ...
    \begin{itemize}
     ...
     \item[Induktionsvoraussetzung] Es gelte Gleichung~\ref{eq:234}
     ...
     \item[Induktionsbeweis] Sei $x\in\FF$ ein Vektor mit genau $p$
      ...
      0}$). Sei nun $x$ keine Ecke von $\FF$. Nach Satz~\ref{satz:228}
	...
	Beweisführung erfolgt wie im zweiten Fall.
      \end{enumerate}
      Sei $\FF$ nun  beschränkt. Dann folgt nach Lemma~\ref{lem:233},
      dass es kein $d\in\R^{n}\setminus \{0\}$ mit $d\geq 0$ und
      $Ad=0$ gibt. Damit hat jedes $x\in\FF$ eine Darstellung wie in
      Gleichung~\ref{eq:234} mit $d=0_{n}$, d.\,h. für alle $x\in\FF$
  ...
  \begin{enumerate}[(a)]
   ...
   \item Ist $\FF$ beschränkt, dann hat das lineare
    ...
    \begin{proof}
      ...
      Satz~\ref{satz:234} hat jedes $x\in\FF$ die Darstellung:
      \begin{align*}
	...
	\Rightarrow c^{T}x &= c^{T} \left(\sum_{i=1}^{k} \alpha_{i}
      ...
      Konvexkombination ist. Nach Satz~\ref{satz:234} gilt, $x=
...
$x_{B}$ durch die Gleichung~\ref{eq:25} bestimmt ist.
...
\begin{bsp}[siehe Beispiel~\ref{bsp:236}]
    \begin{align*}
    (A|b) &= \begin{pmatrix}0 & 1 & 5 & 1 & | & 2\\
	       1 & 0 & -1 & 0 & | & 2\\
	       1 & 0 & 2 & 1 & | & 3\end{pmatrix} & m &= 3 & n &= 4
  ...
  \begin{proof}
    \begin{itemize}
     \item["`$\Rightarrow$"'] Sei $x$ Ecke von $\FF$ und $I(x)=
      ...
      Nach Satz~\ref{satz:228} sind die Spaltenvektoren $A_{.i}$
     ...
     \item["`$\Leftarrow$"'] Sei $x$ zulässige Basislösung zu einer
      ...
      Satz~\ref{satz:228} ist $x$ eine Ecke.
    \end{itemize}
  \end{proof}
\end{satz}
...
\begin{kor}
  ...
  \begin{proof}
    Die Existenz einer Ecke ergibt sich aus Satz~\ref{satz:232}. Es
...
\begin{lemma}
  ...
  \begin{enumerate}[(i)]
   ...
   \item Ist $v^{*}\in\FF_{B}$ Lösung des reduzierten Problems und ist
    ...
    \begin{proof}
      Sei $v^{*}\in\FF_{B}$ Lösung von \ref{eq:29}, $x^{*}$ definiert
      durch $x^{*}_{N}=v, x_{B}^{*}= A_{B}^{-1}(b-A_{N} x_{N})$. Nach
      dem ersten Punkt ist $x^{*}\in\FF$. Sei $x\in\FF$ beliebig. Dann
      gilt nach Gleichung~\ref{eq:28}: $c^{T}x=z_{B}+s_{N}^{T} x_{N}
   ...
   \item Ist $x^{*}$ zulässige Basislösung des linearen Problems zur
      ...
      $x^{*}0_{n-m}$ Lösung des Problems~\ref{eq:29} ist.
    \begin{proof}
...
(theoretisch) das reduzierte Problem~\ref{eq:29} auf und prüft, ob
$v^{*} =0_{n-m}$ Lösung von \ref{eq:29} ist. Falls ja, ist $x^{*}$
Lösung des linearen Problems.
...
Gleichung~\ref{eq:27}, prüfe, ob $s_{N}\geq 0$. Falls ja, ist $x^{*}$
optimal.
...
\begin{lemma}
  ...
  \begin{proof}
    Nach Lemma~\ref{lem:31} gilt $v^{*}=x_{N}^{*}=0_{n-m}$ ist Lösung
...
Lemma~\ref{lem:31} ist $x(t)\in\FF$ mit $x_{N}(t)=v(t), x_{B}(t)=
...
Ist die Gleichung~\ref{eq:34} nicht erfült, dann existiert ein
...
gilt, $x^{+}\in\FF$ wegen des Lemma~\ref{lem:31}. Wir müssen noch eine
Basis $B^{+}$ von $A$ finden, so dass $x^{+}$ Basislösung von $B^{+}$
ist.

Sei $j\in\{1,\ldots,n-m\}$ der Index mit $s_{N}<0$ nach\ref{eq:36} und
...
\begin{bem*}
  ...
  $x^{+}$? Nach der Gleichung\ref{eq:33} gilt:
...
\begin{satz}
  ...
  \begin{proof}
    ...
    Satz~\ref{satz:310} folgt: Das Verfahren berechnet eine neue Basis
