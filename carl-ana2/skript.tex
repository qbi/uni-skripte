...
\begin{definition}
  ...
  so heißt $\frac{\partial f}{\partial h}(x)$ \highl{Richtungsableitung} von $f$ in $x$.
\end{definition}
...
\begin{theorem}
  ...
  für jede Richtung $h$, und es gilt $\frac{\partial f}{\partial
    h}(x)=f'(x)h$.
  \begin{proof}
...
\begin{remark}
  ...
  Ist $f(x)=f(x_1,\dots,x_n)$, so ist $\frac{\partial f}{\partial
    x_i}=
...
\begin{beispiel}
  \begin{gather*}
    ...
    \frac{\partial f}{\partial x_1}=2x+\sin x_2 \quad \frac{\partial
      f}{\partial x_2}=x_1\cos x_2
  \end{gather*}
...
\begin{theorem}
  ...
  \[\abs{r(h)}\leq \sqrt{\sum_{i=1}^n \abs{\frac{\partial f}{\partial x_i}
  (x)-\frac{\partial f}{\partial x_i}(y^{(i)})}^2}\cdot\norm{h}_2\]
  Daher folgt nun: $\frac{\abs{r(h)}}{\norm{h}_2}\leq\sqrt{\sum_{i=1}^n \abs{\frac{\partial f}{\partial x_i}
  (x)-\frac{\partial f}{\partial x_i}(y^{(i)})}^2}\cdot\norm{h}_2\rightarrow
...
\begin{remark}
  ...
  \item Ist $f$ in $x$ differenzierbar, so gilt:
    ...
    $\frac{\partial f}{\partial h}(x)=f'(x)h =(\text{grad}f(x))h$.
  \end{enumerate}
\end{remark}
...
\begin{theorem}
  ...
  Richtungsableitungen $\frac{\partial f}{\partial h}(x)$ mit $\norm{h}_2
  =1$ eine grösste, nämlich die des Gradienten $\text{grad}f(x)$ mit
  $\frac{\partial f}{\partial h}(x)=\norm{\text{grad}f(x)}_2$.
  \begin{proof}
    $\frac{\partial f}{\partial h}(x)=(\text{grad}f(x))h$
  \begin{align*}
    \abs{\frac{\partial f}{\partial h}(x)} & = \abs{\sum_{i=1}^n
  \frac{\partial f}{\partial x_i}(x)h_i}\\
  & \leq \sqrt{\sum_{i=1}^n
  \abs{\frac{\partial f}{\partial x_i}(x)}^2}\sqrt{\sum_{i=1}^n \abs{h_i}^2}\\
  & = \norm{\text{grad}f(x)}_2\norm{h}_2\\
...
\begin{theorem}
  ...
  \begin{align*}
    \abs{r(h)} & \leq
      \frac{1}{2}\left(\sum_{j,k=1}^n\sqrt{\abs{\frac{\partial^2
              f}{\partial x_k \partial x_j}(\xi+\Theta h)-
            \frac{\partial^2 f}{\partial x_k \partial x_j}}^2}\right)
    \underbrace{\sqrt{\sum_{j,k=1}^n \abs{h_j}^2\abs{h_k}^2}}_{\sum_{j,k=1}^n \abs{h_j}^2=\norm{h}_2^2}\\
  ...
  \end{align*}
  Also: $\frac{\abs{r(h)}}{\norm{h}_2^2}\leq \frac{1}{2} \sqrt{\sum_{j,k=1}^n \abs{\frac{\partial^2 f}{\partial x_k \partial x_j}
  (\xi+\Theta h)-\frac{\partial^2 f}{\partial x_k\partial x_j}(\xi)}}\rightarrow 0$\\
...
$g(x)\coloneqq Ax-b, h(y)\coloneqq \norm{y}_2^2, g'(x)=A, h'(y)=\left(\frac{\partial h}{\partial y_1},
...
\begin{theorem}
  ...
  In Komponenten:
  \[\sum_{i=1}^n (x-\xi)\frac{\partial f}{\partial
    x_i}(\xi_1,\dots,\xi_n)=0\]
  \begin{proof}
    Sei o.\,B.\,d.\,A. $\frac{\partial f}{\partial x_n}(\xi)\neq
...
Daher folgt: 
\begin{align*}
  \norm{f(x,y)-f(x,\tilde{y})}_2 &= \norm{\int_0^1 \frac{\partial
      f}{\partial y} (x,y+t(\tilde{y}-y))\,dt(\tilde{y}-y)}\\
  &\leq \int_0^1 \norm{\frac{\partial f}{\partial y}
