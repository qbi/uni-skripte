...
\begin{theorem}
  ...
  \textbf{Beweis}: $f(x+th)=f(x)+f'(x)(th)+r(th)$, wobei $\frac{r(th)}{\norm{th}}\xrightarrow{t\rightarrow 0} 0$\\
...
\begin{theorem}
  ...
  differenzierbar und es gilt die Formel aus  \autoref{satz:AblMatrix}.\\
...
$\R^n$\\
Beweis: Sei $u\coloneqq \int_a^b f(t)dt\in\R^n$. Dann $\norm{u}_2^2=
...
Gilt $f(t_1)=f(t_2)=x$ für $t_1\neq t_2$, so heißt $x$ \highl{Doppelpunkt}
der Kurve $f$. In $x$ hat dann $f$ zwei im Allgemeinen verschiedene Tangentialvektoren.
...
so ist \autoref{satz:linAbh} i.\,A. falsch. 
Beispiel:
