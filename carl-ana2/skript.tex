...
\newcommand*{\remarkautorefname}{Bemerkung}
...
\begin{definition}
  ...
  $\K=\C$}. Eine Funktion $\norm{\cdot}\colon E\rightarrow [0,\infty[$ heißt
  \highl{Norm} auf $E$, wenn für beliebige $x,y\in E$ und
  $\alpha\in\K$ gilt:
  \begin{enumerate}[(N1)]
    \item $\norm{x}\geq 0 \wedge \norm{x}=0\perdef x=0$
    \item $\norm{\alpha x}=\abs{\alpha}\cdot \norm{x}$ \highl{Homogenität}
    \item $\norm{x+y}\leq \norm{x}+\norm{y}$ Dreiecksungleichung
  \end{enumerate}
  Das Paar $[E,\norm{\cdot }]$ heißt \highl[Raum!normierter]{normierter
    Raum}.
\end{definition}

\begin{remark}
  Durch den Ansatz $d(x,y)\coloneqq \norm{x-y}$ wird auf $E$ eine
  ...
  Räume. Beispielsweise konvergiert $(x_n)\subset E$ in $E\colon  \exists x\in E\,\forall \varepsilon>0\,\exists n_\varepsilon\in \N\,\forall
  n>n_\varepsilon\colon \norm{x_n-x}\leq \varepsilon$
\end{remark}

\begin{definition}
  Ein vollständiger normierter Raum heißt \highl{Banachraum}.
\end{definition}

\begin{beispiel}
  \begin{enumerate}
  \item $[\R,\abs{\cdot}]$ ist der normierte Raum über den reellen Zahlen
    mit der Betragsfunktion.
  \item $[\R^n,\norm{\cdot}_p], x=(\xi_1,\dots,\xi_n),\norm{x}_p=
    \begin{cases}
      \left(\sum_{i=1}^n \abs{\xi_i}^p\right)^{\nicefrac{1}{p}} & 1\leq p<\infty\\
      \max_{1\leq i \leq n} \abs{\xi_i} & p=\infty
    \end{cases}$
    \begin{itemize}
    \item $\norm{\cdot}_1 = \sum_{i=1}^n \abs{\xi_i}$ heißt
      \highl{Summennorm}.
    \item $\norm{\cdot}_2= \sqrt{\sum_{i=1}^n \abs{\xi_i}^2}$ heißt
      \highl[Norm!euklidische]{euklidische Norm}.
      \item $\norm{\cdot}_\infty$ heißt \highl{Maximumnorm}.
    \end{itemize}
  \item
    \begin{enumerate}[(a)]
    \item Normierter Raum der beschränkten Funktionen auf einer Menge
      ...
      \norm{f}_\infty\coloneqq \sup_{x\in\Omega}\abs{f(x)}$\\
    ...
    \item Sei $[X,d]$ ein metrischer Raum. Normierter Raum der 
      beschränkt stetigen Funktionen auf $X\colon [C_b(X),\norm{\cdot}_\infty],
      C_b(X)\coloneqq \{f\colon X\rightarrow\R|f \text{ beschränkt stetig}\}$
    \end{enumerate}
  \item Ist $X$ ein kompakter metrischer Raum, so gilt: $C_b(X)=C(X),
    [C_b(X),\norm{\cdot}_\infty]=[C(X),\norm{\cdot}_\infty]$
  \item Der normierte Raum der $\RR$-integrierbaren Funktionen:
    $[\RR[a,b],\norm{\cdot}_\infty]$. Dabei ist $\RR[a,b]=\{f\colon [a,b]
    \rightarrow\R|f\text{ Riemann-integrierbar}\}, \norm{f}_\infty\coloneqq 
    \sup_ {x\in I}\abs{f(x)}$
  \item $[C^1[a,b],\norm{\cdot}_\infty], C^1[a,b]\coloneqq \{f\in C[a,b]|f 
    \text{ stetig differenzierbar auf } ]a,b[\}$
  \end{enumerate}
\end{beispiel}

\begin{theorem}
  $[B(\Omega),\norm{\cdot}_\infty], [C_b(X),\norm{\cdot}_\infty],[
  \RR[a,b],\norm{\cdot}_\infty]$ sind Banachräume. Daraus folgt,
  dass auch $[\R^n,\norm{\cdot}_\infty]$ ein Banachraum ist, denn 
  $B(\{1,\dots,n\})=\R^n$.
\end{theorem}
\begin{proof}
  Vollständigkeit von $[C_b(X),\norm{\cdot}_\infty]$:\\
  Sei $(f_n)\subset C_b(X)$ eine Cauchyfolge. Dann ist zu zeigen, dass
  ein  $f\in C_b(X)$ mit $\lim_{n\rightarrow\infty}\norm{f_n-f}_\infty=0$
  existiert.
  \begin{enumerate}
  \item Existenz des Limes\\
    $(f_n)$ ist eine Cauchyfolge, d.\,h. $\forall \varepsilon>0\,\exists
    n_\varepsilon \in \N\,\forall n,m \geq n_\varepsilon \colon 
    \norm{f_n-f_m}_\infty\leq\varepsilon$\\
    $\norm{f_n-f_m}=\sup_{x\in X} \abs{f_n(x)-f_m(x)}\leq\varepsilon
    \Leftrightarrow \forall x\in X \colon  \abs{f_n(x)-f_m(x)}\leq\varepsilon$
  ...
  \item Beschränkheit von $f$:\\
    $\forall x\in X\,\forall m\geq n\geq n_\varepsilon \colon  \abs{f_n(x)-f_m(x)}
    \leq\varepsilon\xrightarrow{m\rightarrow\infty}\forall x\in
    X\,\forall n\geq n_\varepsilon \colon  \abs{f_n(x)-f(x)}\leq\varepsilon
    \Rightarrow \norm{f_n-f}\leq\varepsilon \text{ für } n\geq n_\varepsilon$
    Insbesondere gilt: $\norm{f}_\infty=\norm{f_{n_\varepsilon}+f-f_{n_\varepsilon}
    }_\infty\leq \norm{f_{n_\varepsilon}}_\infty + \norm{f-f_{n_\varepsilon}}_\infty
    \leq \norm{f_{n_\varepsilon}}_\infty + \varepsilon <\infty$
  ...
  \item Stetigkeit von $f$:\\
    ...
    \delta_\varepsilon\Rightarrow\abs{f_{n_\varepsilon}(x)-f_{n_\varepsilon}(x_0)}
    \leq\varepsilon\Rightarrow \abs{f(x)-f(x_0)}\leq \abs{f(x)-f_{n_\varepsilon}(x)}
    +\abs{f_{n_\varepsilon}(x)-f_{n_\varepsilon}(x_0)}+\abs{f_{n_\varepsilon}
  ...
  $\lim_{n\rightarrow\infty}\norm{f_n-f}=0$. Also ist
  $[C_b(X),\norm{\cdot}_\infty]$ ein vollständiger normierter Raum.

  Die Vollständigkeit von $[B(\Omega),\norm{\cdot}_\infty]$ kann analog zu
  1. und 2. bewiesen werden.

  Für die Vollständigkeit von $[\RR[a,b],\norm{\cdot}_\infty]$ genügt es, zu
  zeigen, dass $[\RR[a,b],\norm{\cdot}_\infty]$ in $B[a,b]$ abgeschlossen
  ist:\\
  Sei $(f_n)\subset\RR[a,b]\colon f_n\rightarrow f\in B[a,b] (\norm{f_n-f}_\infty
  \rightarrow 0)$. zu zeigen: $f\in\RR [a,b]$:\\
  Sei $\varepsilon >0$. Für $\eta\coloneqq \frac{\varepsilon}{4(b-a)}$ wählen
  wir $n_0\in\N\colon \norm{f_n-f}_\infty=\sup_{x\in[a,b]} \abs{f_{n_0}(x)-f(x)}
...
\begin{remark}
  Man bezeichnet die Konvergenz bezüglich $\norm{\cdot}_\infty$ als \highl[Konvergenz!gleichmässige]{gleichmässige
  Konvergenz} von
  Funktionen in $B(\Omega), C_b(X)$ oder $\RR[a,b]$.\\
  $\norm{f_n-f}_\infty\rightarrow
  0\Leftrightarrow\forall\varepsilon>0\,\exists
  n_\varepsilon\in\N\,\forall n\geq
  n_\varepsilon\colon \norm{f_n-f}_\infty =\sup_x \abs{f_n(x)-f(x)}\leq\varepsilon
  \Leftrightarrow$
  \[\forall\varepsilon>0\,\exists n_\varepsilon\in\N\,\forall n\geq
  n_\varepsilon\,\forall x\colon  \abs{f_n(x)-f(x)}\leq\varepsilon\] Im
  Gegensatz dazu ist die \highl[Konvergenz!punktweise]{punktweise
  Konvergenz} einer Folge wie folgt
  definiert:
  \[\forall x\,\forall\varepsilon>0\,\exists n_\varepsilon\in\N\,\forall
  n\geq n_\varepsilon\colon \abs{f_n(x)-f(x)}\leq\varepsilon\]
...
\begin{remark}
\begin{enumerate}[(i)]
\item Der \autoref{satz:gleichKonvInt} gilt nicht für punktweise Konvergenz.
  \begin{beispiel}
    $f_n\colon [0,1]\rightarrow\R,f_n(x)\coloneqq \max\{n-n^2\abs{x-\frac{1}{n}};0\}=
    \begin{cases}
...
\begin{proof}
  ...
  Differential- und Integralrechnung schliessen. Aus dem
  \autoref{satz:gleichKonvInt} folgt
...
\begin{remark}
  \begin{itemize}
  \item Selbst wenn gilt $f_n\xrightarrow{\norm{\cdot}}f$, impliziert $f$
  ...
  \item $f_n\colon [-N,N]\rightarrow\R\quad f_n(x)=\frac{1}{n}\sin (nx)$
    ...
    $\norm{f_n}_\infty=\frac{1}{n}\rightarrow 0\Rightarrow \lim f_n=0$\\
    $f_n'(x)=\cos(nx)\not\rightarrow f'(x)=0$
  \end{itemize}
\end{remark}
...
\begin{definition}
  ...
  \highl{endlichdimensional}, wenn
  \[\exists m\in\N\,\exists x_1,\dots,x_m\in E\,\forall x\in
  E\,\exists \alpha_1,\dots,\alpha_m\in\K\colon  x=\sum_{i=1}^{m} \alpha_ix_i\]
\end{definition}
\[\dim E \coloneqq \inf\{m| \exists x_1,\dots,x_m\in E\,\forall x\in
A\,\exists \alpha_1,\dots,\alpha_m\in\K\colon x=\sum_{i=1}^m\alpha_ix_i\}\]
...
\begin{theorem}[Satz von Bolzano/Weierstrass]
  ...
  $[\R^n,\norm{\cdot}_\infty]$, wenn $ K$ abgeschlossen und beschränkt in
  $[\R^n,\norm{\cdot}_\infty]$ ist.
\end{theorem}
\begin{proof}
  Der Beweis wird auf den gleichlautenden Satz in $[\R,\abs{\cdot}]$
  zurückgeführt:
  \begin{enumerate}
    ...
    \item Sei $K$ abgeschlossen und beschränkt, d.\,h.
      \begin{enumerate}
      \item $\exists M\geq 0\,\forall x\in\K\colon \norm{x}_\infty\leq M$
      \item $\forall (x_n)\subset K\colon  x_n\xrightarrow{\norm{\cdot}}
        x\in\R^n\Rightarrow x\in K$
      \end{enumerate}
  \end{enumerate}
  Betrachten den Fall $n=2$: Sei $(x_n)\subset K, x_n =(\xi_1^{(n)},
  \xi_2^{(n)}), \norm{x_n}_\infty=\max\{\abs{\xi_1^{(n)}},\abs{\xi_2^{(n)}}\}\leq
  ...
  $\norm{x_{(n_k)_l}-x}_\infty
  =\max\{\abs{\xi_1^{(n_k)_l}-\xi_1},\abs{\xi_2^{(n_k)_l}-\xi_2}\}\leq
  \abs{\xi_1^{(n_k)_l}-\xi_1}+\abs{\xi_2^{(n_k)_l}-\xi_2}\rightarrow 0$, d.\,h.
...
\begin{definition}
  Sei $E$ ein Vektorraum und $\norm{\cdot}_0,\norm{\cdot}$ zwei Normen auf $E$.
  $\norm{\cdot}_0$ und $\norm{\cdot}$ heissen genau dann auf $E$ äquivalent,
  wenn 
  \[\exists c_0,c>0\,\forall x\in E\colon  \norm{x}_0\leq c_0\norm{x}\qquad
  \norm{x}\leq c\norm{x}_0\]
\end{definition}
...
\begin{proof}
  ...
  \alpha_ix_i$. Sei $\norm{\cdot}$ eine beliebige Norm auf $E$. Des
  Weiteren definieren wir eine Norm $\norm{\cdot}_0$ auf $E$ durch
  $\norm{x}_0\coloneqq \sum_{i=1}^n\abs{\alpha_i}=\norm{\alpha}_1$ mit $\alpha=
  (\alpha_1, \ldots, \alpha_n)\in\R^n$. Es genügt nun, zu zeigen, dass
  $\norm{\cdot}_0$ und  $\norm{\cdot}$ äquivalent sind.
  \[\norm{x}=\norm{\sum_{i=1}^n \alpha_i\cdot x_i}
  \leq \sum_{i=1}^n \abs{\alpha_i}\cdot \norm{x_i} \leq \max_{1<i\leq n}
  \norm{x_i} \sum_{i=1}^n \abs{\alpha_i}= \left(\underbrace{\max_{1<i\leq n}
      \norm{x_i}}_{=c}\right)\norm{x}_0\]
  Jetzt ist "`$\norm{\cdot}_0\leq c_0\norm{\cdot}$"'. Dazu betrachten wir
  die Funktion: $f\colon [\R^n,\norm{\cdot}_\infty]\rightarrow\R$ mit
  $f(\alpha_1, \ldots,\alpha_n)\coloneqq \norm{\sum_{i=1}^n \alpha_i
    x_i}$. Diese Funktion $f$  ist stetig, denn wir können folgende
  Abschätzung treffen:
  \begin{align*}
    \abs{f(\alpha_1,\dots,\alpha_n)-f(\beta_1,\dots,\beta_n)} &=
    \norm{\sum_1^n (\alpha_i-\beta_i)x_i}\\
    &\leq \sum_1^n \abs{\alpha_i-\beta_i} \cdot \norm{x_i}\leq
    \max_{1\leq i \leq n} \norm{x_i} \cdot\sum_1^n
    \abs{\alpha_i-\beta_i}\\
    &\leq n\cdot\underbrace{\max_{1\leq i\leq n}
    \abs{\alpha_i-\beta_i}}_{=\norm{(\alpha1,\dots,\alpha_n)-(\beta_1-\beta_n)}_\infty}
  \end{align*}

  $S=\{\alpha=(\alpha_1,\dots,\alpha_n)\in\R^n|\sum_{i=1}^n
  \abs{\alpha_i}=1\} \subset\R^n$ ist abgeschlossen und beschränkt in
  $[\R^n,\norm{\cdot}_\infty]$.  Denn für
  $g(\alpha_1,\dots,\alpha_n)\coloneqq \sum_{i=1}^n\abs{\alpha_i}, g\colon 
  [\R^n,\norm{\cdot}_\infty]\rightarrow\R$ gilt, dass $g$ stetig
  ist. Damit ist $S=g^{-1}(\{1\})$ (Urbild) abgeschlossen. $S$ ist
  beschränkt:
  $\norm{\alpha}_\infty=\norm{(\alpha_1,\dots,\alpha_n)}_\infty\leq\sum_{i=1}^n
  \abs{\alpha_i}=1$ für $\alpha=(\alpha_1,\dots,\alpha_n)\in S$. Somit ist
  $S$ in $[\R^n,\norm{\cdot}_\infty]$ kompakt nach \autoref{satz:bw}. Weil
  $f$ stetig ist, folgt, dass $f$ auf $S$ das Minimum annimmt, d.\,h.\\
  $\exists (\alpha_1^0,\dotsc,\alpha_n^0)\in S\,\forall
  (\alpha_1,\dotsc, \alpha_n)\in S\colon 
  0<\frac{1}{c_0}=f((\alpha_1^0,\dots,\alpha_n^0)\leq
  f(\alpha_1,\dots,\alpha_n)=\norm{\sum_1^n \alpha_ix_i}$.  Sei $0\neq
  x\in E$ beliebig. Dann existiert $0\neq(\alpha_1,\dots,
  \alpha_n)\in\R^n:x=\sum_{i=1}^n \alpha_ix_i$. Für
  $\left(\frac{\alpha_1}{\sum_{i=1}^n \abs{\alpha_i}},
    \frac{\alpha_2}{\sum_{i=1}^n \abs{\alpha_i}},\dots,
    \frac{\alpha_n}{\sum_{i=1}^n \abs{\alpha_i}}\right)\in S$ gilt
  $\frac{1}{c_0}\leq \norm{\sum_{k=1}^n \frac{\alpha_k}{\sum_{i=1}
    \abs{\alpha_i}} x_k}=\frac{1}{\sum_{i=1}^n
    \abs{\alpha_i}}\norm{\sum_1^n\alpha_n x_k} \Rightarrow
  \norm{x_0}=\sum_{i=1}^n \norm{\alpha_i}\leq c_0 \norm{\sum_1^n \alpha_k
  x_k}=c_0\norm{x}$. Für $x=0$ ist die Ungleichung ebenfalls erfüllt.
\end{proof}

\begin{theorem}
  \begin{enumerate}[(i)]
    \item Ein endlichdimensionaler normierter Raum $[E,\norm{\cdot}]$
...
\begin{proof}
  ...
  $\norm{x_0}\coloneqq \max_{1\leq i\leq n}\abs{\alpha_i}=\norm{\alpha}_\infty,\alpha=
  (\alpha_1,\dots,\alpha_n)\in\R^n$ ist eine Norm auf $E$. Nach 
  \autoref{satz:VRNorm} existieren $c, C>0\colon c\norm{x}_0\leq \norm{x}\leq C\norm{x}_0$.
  \begin{enumerate}[(i)]
  \item Sei $x_k=\sum_{i=1}^n \alpha_i^{(k)} y_i, \alpha_i^{(k)}=
    (\alpha_1^{(k)},\dots,\alpha_n^{(k)})\in\R^n$. Es gilt $c\norm{
    \alpha^{(k)}-\alpha^{(l)}}_\infty\leq \norm{x_k-x_l}\leq C
    \norm{\alpha^{(k)}-\alpha^{(l)}}_\infty$. Sei $(x_k)\subset E$ eine
    Cauchyfolge. Dann folgt, dass
    $(\alpha^{(k)})\subset[\R^n,\norm{\cdot}_\infty]$ eine Cauchyfolge
    ist. Da nun $[\R^n,\norm{\cdot}]$ vollständig ist, gibt es ein
    $\alpha =)\alpha_1,\dots,\alpha_n)\in\R^n\colon \norm{\alpha^{(k)}
    -\alpha}_\infty\rightarrow 0\Rightarrow \norm{x_k-x}\rightarrow 0,
  ...
  \item
    ...
    \item["`$\Leftarrow$"'] Definieren $f\colon [\R^n, \norm{\cdot}_\infty]
      ...
      \end{enumerate}
      Sei $K\subset E$ beschränkt und abgeschlossen. Dann ist $f^{-1}(K)=
      \{(\alpha_1,\dots,\alpha_n)\in\R^n\colon \sum_{i=1}^n \alpha_iy_i
      \in K\}$ beschränkt, da $K$ beschränkt (wegen Ungleichung
      oben) und abgeschlossen (Urbilder von abg. Mengen sind wieder
      abg.), da $f$ stetig ist. Nach  \autoref{satz:bw} folgt,
      $f^{-1}(K)\subset [\R^n,\norm{\cdot}_\infty]$ kompakt. Da $f$
...
\begin{remark}
  Es gilt sogar: Sind in einem Banachraum $E$ die beschränkten und
  abgeschlossenen Mengen kompakt, so ist $E$ endlichdimensional.
\end{remark}
...
\begin{definition}
  ...
  heißt \highl[Abbildung!lineare]{linear}, genau dann wenn:
...
\begin{remark}
  ...
  können durch Matrizen dargestellt werden.
\end{remark}

\begin{definition}
  Seien $E,F$ normierte Räume. Eine lineare Abbildung $A\colon E\rightarrow F$
  heißt beschränkt, genau dann wenn:
  \[\exists M\geq 0\,\forall x\in E\colon  \norm{Ax}_F \leq M\norm{x}_E\]
\end{definition}

Es gilt: $\LL(E,F)\coloneqq \{A\colon E\rightarrow F| A\text{ beschränkt}\}$ ist ein
Vektorraum. Durch $\norm{A}\coloneqq \sup_{\norm{x}_E\leq 1} \norm{Ax}_F$ wird auf $\LL(E,F)$
eine Norm (\highl{Operatorennorm}) erklärt.
$[\LL(E,F),\norm{\cdot}]$ ist ein normierter Raum.\\
Ferner gilt: $\forall x\in E\colon \norm{Ax}_F\leq \norm{A}\cdot \norm{x}_E$. Für
$x\neq 0\colon \left.
\begin{array}{rcl}
  \norm{A(\frac{x}{\norm{x}})} & \leq & \norm{A}\\
  \norm{Ax\frac{1}{\norm{x}}} & = & \frac{1}{\norm{x}}\norm{Ax}
...
B\circ A\in\LL(E,G)$ und es gilt $\norm{BA}\leq \norm{B}\cdot \norm{A}$.
...
\begin{proof}
  ...
  $x=\sum_{i=1}^n \alpha_i x_i, \norm{x}_0=\sum_1^n \abs{\alpha_i}$ ist eine
  äquivalente Norm. Somit
  \begin{align*}
    \norm{Ax_i} & = \norm{A\left(\sum_1^n \alpha_i x_i\right)}_F\\
    & =\norm{\sum_1^n \alpha_i Ax_i}_F\\
    & \leq \sum_1^n \abs{\alpha_i} \cdot \norm{Ax_i}\\
    & \leq \max_{1\leq i\leq n} \norm{Ax_i}\sum_1^n \abs{\alpha_i}\\
    & \leq (c\max_{1\leq i\leq n} \norm{Ax_i})\norm{x}
  \end{align*}
...
\begin{definition}
  ...
  \[
    ...
    \norm{h}\leq \delta \colon  x+h \in D\]
    \[\text{ und } f(x+h)=f(x)+Ah+r(h)
    \text{ mit } \lim_{h\rightarrow 0}\frac{r(h)}{\norm{h}}=0
  \]
\end{definition}

\begin{remark}
  Die Differentiation von $f$ im Punkt $x\in D$ bedeutet eine
Approximation durch eine affine lineare Abbildung $f(x)+Ah$ mit der
Eigenschaft, dass \[\lim_{h\rightarrow 0}\frac{f(x+h)-f(x)-Ah}{\norm{h}}=0.\]
...
\[f(x+h)=f(x)+Ah+\norm{h}\tilde{r}(h)\text{ mit } \lim_{h\rightarrow 0}
\tilde{r}(h)=0\]
\end{remark}
...
\begin{definition}
  ...
  Operator $A\in \LL(E,F)$ mit $f(x+h)=f(x)+Ah+r(h), \lim \frac{r(h)}{\norm{h}}
  =0$ \highl{Ableitung} (oder \highl{Fréchet-Ableitung})
  und $Ah$ \highl{Differential}
...
\begin{align*}
  ...
  g(f(x)+k) & = g(f(x))+g'(f(x))k+\norm{k}\tilde{r}_g(k)
  &\tilde{r}_g(k)\rightarrow 0\\
  (g\circ f)(x+h) & = g(f(x+h))=g(f(x)+\underbrace{f'(x)h+\norm{h}
    \tilde{r}_f(h)}_{k})\\
  & =  g(f(x)) +g'(f(x)) (f'(x)h+\norm{h}\tilde{r}_f(h)+\norm{f'(x)}h\\
  &\qquad +\norm{h} \tilde{r}_g(f'(x)h)+\norm{h}\tilde{r}_f(h)\\
  & = (g\circ f)(x)+g'(f(x))f'(x)h+r_{g\circ f}(h))\\
  & \text{wobei}\\
  r_{g\circ f}(h) & = \norm{h}g'(f(x))\tilde{r}_f(h)\\
  & \qquad +\norm{f'(x+h)+\norm{h} \tilde{r}_f(h)}\cdot
  \tilde{r}_g(f'(x)h+\norm{h}\tilde{r}_f(h))
\end{align*}
Es gilt zu zeigen:
\[\lim_{h\rightarrow 0}\frac{r_{g\circ f}(h)}{\norm{h}}\rightarrow 0\]
\begin{align*}
  \norm{r_{g\circ f}(h)} \leq & \norm{h}\cdot\norm{g'(f(x))}\cdot
  \norm{\tilde{r}_f(h)}\\
  & \qquad +(\norm{f'(x)}\norm{h}+\norm{h}
  \norm{\tilde{r}_f(h)})\norm{\tilde{r}_g(f'(x)h+\norm{h}\tilde{r}_g(h))}\\
  \frac{\norm{r_{g\circ f}(h)}}{\norm{h}} \leq & \norm{g'(f(x))}\cdot
  \norm{\tilde{r}_f(h)}\\
  & \qquad +(\norm{f'(x)}+\norm{\tilde{r}_f(h)}) \norm{\tilde{r}_g(f'(x)h+\norm{h}\tilde{r}_g(h))} \xrightarrow{h\rightarrow 0} 0\\
  \Rightarrow& \lim \frac{r_{g\circ f}(h)}{\norm{h}}\rightarrow 0\\
...
\begin{definition}
  ...
  \[\diffp{f}{h}(x)\coloneqq \lim_{\stackrel{t\rightarrow 0}{t\in\R}} \frac{f(x+th)-f(x)}{t}\],
  so heißt $\diffp{f}{h}(x)$ \highl{Richtungsableitung} von $f$ in $x$.
\end{definition}

\begin{theorem}
  Seien $E,F$ Banachräume und $f\colon D\rightarrow F, D\subset F$ offen, in 
  $x\in D$ differenzierbar, so ist $f$ in $x$ stetig.\\
  \textbf{Beweis}:$f(x+h)=f(x)+f'(x)h+\norm{h}\tilde{r}_f(h)
...
\begin{theorem}
  ...
  differenzierbar. Dann existiert die Richtungsableitung $\diffp{f}{h}(x)$
  für jede Richtung $h$, und es gilt $\diffp{f}{h}(x)=f'(x)h$.
  \begin{proof}
    $f(x+th)=f(x)+f'(x)(th)+r(th)$, wobei $\frac{r(th)}{\norm{th}}\xrightarrow{t\rightarrow 0} 0$\\
  $\frac{f(x+th)-f(x)}{t}=f'(x)h+\frac{r(th)}{t}\rightarrow f'(x)h$
  \end{proof}
\end{theorem}

\begin{remark}
  Die Umkehrung von  \autoref{satz:richtAbl} gilt i.\,A. nicht.
\end{remark}
...
Für $f$ gilt: 
\begin{enumerate}[(1)]
  \item $f$ ist in (0,0) \highl{nicht} stetig $\Rightarrow f$ in (0,0)
    nicht differenzierbar.
    \begin{proof}
      $(x_n)=(\frac{1}{n^2},\frac{1}{n})\rightarrow (0,0), 
    f(\frac{1}{n^2},\frac{1}{n})=\frac{\frac{1}{n^4}}{\frac{1}{n^4}
    +\frac{1}{n^4}}=\frac{1}{2}\not\rightarrow 0\Rightarrow f$ in
    (0,0) nicht stetig.
    \end{proof}
  \item $f$ besitzt in (0,0) Richtungsableitungen in jede Richtung
    $h\neq 0$
    \begin{proof}
      $\frac{f(0+th)-f(0)}{t}=\frac{1}{t}
    ...
    $\diffp{f}{h}(0)=\frac{h_2^2}{h_1}$ für $h_1\neq 0$ und
     $\diffp{f}{h}(0)=0$ für $h_1=0$. Ferner zeigt dieser 
     Ausdruck, dass $\diffp{f}{h}(0)$ nicht linear in $h$ ist.
    \end{proof}
\end{enumerate}
\end{beispiel}

\section{Differentiation von \texorpdfstring{$\R^n$-$\R^m$}{Rn-Rm}-Funktionen}
\subsection{Partielle Ableitung von \texorpdfstring{$\R^n$-$\R^m$}{Rn-Rm}-Funktionen}
\begin{definition}
  ...
  $\diffp{f}{{e_i}}(x)=\lim_{t\rightarrow 0}
  \frac{f(x+te_i)-f(x)}{t}, e_i=(0,\dots,1,\dots,0)\in\R^n$, so heißt
  $\diffp{t}{{x_i}}(x)\coloneqq \diffp{f}{{e_i}}(x)$
  \highl[Ableitung!partielle]{partielle Ableitung} von $f$ nach der
  $i$-ten Koordinate.
\end{definition}

\begin{remark}
  Die partielle Ableitung einer Funktion $f\colon D\rightarrow\R,D
  \subset \R^n$ offen, kann man als gewöhnliche Ableitung von
  Funktionen einer reellen Veränderlichen interpretieren, indem man
  nach der $i$-ten Koordinate differenziert und die übrigen festhält.

  Ist $f(x)=f(x_1,\dots,x_n)$, so ist $\diffp{f}{{x_i}}=
  \frac{f(x_1,\dots,x_i+t,x_1,\dots,x_n)-f(x_1,\dots,x_n)}{t}$. Also
  sind die partiellen Ableitungen Richtungsableitungen in die $i$-te
  Richtung.
\end{remark}

\begin{beispiel}
  \begin{gather*}
    f(x_1,x_2)=x_1^2+x_1\sin x_2 \quad f\colon \R^2\rightarrow \R\\
    \diffp{f}{{x_1}}=2x+\sin x_2 \quad \diffp{f}{{x_2}}=x_1\cos x_2
  \end{gather*}
  Die Existenz der partiellen Ableitung von $f\colon D\rightarrow \R,
  D\subset\R^n$ reicht i.\,A. nicht für die Differenzierbarkeit von
  $f$ aus!
\end{beispiel}
...
\begin{cases}
  ...
  0 & (x_1,x_2)=0
\end{cases}\]
\[\diffp{f}{h}(0,0)=\frac{h_2^2}{h_1} \text{ für } h_1\neq 0\wedge =0\text{ für } h_1=0\]
\[\diffp{f}{{x_1}}(0,0)=0 \quad \diffp{f}{{x_2}}(0,0)=0\]
Aber $f$ ist in $(0,0)$ nicht differenzierbar!
\end{beispiel}

\subsection{Darstellung der Ableitung von \texorpdfstring{$\R^n$-$\R^m$}{Rn-Rm}-Funktionen}
\begin{theorem}
  ...
  \[f'(x)=\begin{pmatrix}\diffp{f_1}{{x_1}} & \dots & \diffp{f_1}{{x_n}}\\
    \vdots & & \vdots\\
    \diffp{f_m}{{x_1}} & \dots & \diffp{f_m}{{x_n}}
  \end{pmatrix}\]
  \begin{proof}
    $f'(x)=A=(a_{ij})_{\stackrel{i=1,\dots,m}{j=1,\dots,n}}$
  Dann gilt: $f(x+h)=f(x)+Ah+r(h)$ mit $\frac{r(h)}{\norm{h}}
  ...
  $\Rightarrow \diffp{f}{{x_j}}=\lim_{t\rightarrow 0} 
...
\begin{theorem}
  ...
  partielle Ableitung $\diffp{f_i}{{x_j}}$ in $D$ 
  existieren und im Punkt $x\in D$ stetig sind, dann ist $f$ in $x$ 
  differenzierbar und es gilt die Formel aus
  \autoref{satz:AblMatrix}.
  \begin{proof}
    $f=\begin{pmatrix}f_1\\\vdots\\f_m
  ...
  $\theta_i\in [0,1]\colon f(z^{(i)})-f(z^{(i-1)})=\diffp{f}{{x_i}}
  ...
  {\small
    ...
    h_i=\diffp{f}{{x_i}}(y^{(i)})h_i$
  }
  Somit ist $f(x+h)-f(x)=\sum_{i=1}^n (f(z^{(i)})-f(z^{(i-1)})=
  \sum_{i=1}^n \diffp{f}{{x_i}}(y^{(i)})h_i=\sum_{i=1}^n
  \diffp{f}{{x_i}}(x)h_i+\underbrace{\sum_{i=1}^n
  (\diffp{f}{{x_i}}(y^{(i)})-\diffp{f}{{x_i}}
  (x))h_i}_{=\colon r(h)}$\\
  Nach der Cauchy-Schwartzschen Ungleichung gilt:
  \[\abs{r(h)}\leq \sqrt{\sum_{i=1}^n \abs{\diffp{f}{{x_i}}
  (x)-\diffp{f}{{x_i}}(y^{(i)})}^2}\cdot\norm{h}_2\]
  Daher folgt nun: $\frac{\abs{r(h)}}{\norm{h}_2}\leq\sqrt{\sum_{i=1}^n \abs{\diffp{f}{{x_i}}
  (x)-\diffp{f}{{x_i}}(y^{(i)})}^2}\cdot\norm{h}_2\rightarrow
  0$ für $h\rightarrow 0$, da $\diffp{f}{{x_i}}$ stetig
...
\rightarrow \R$ reicht i.\,A. \highl{nicht} für die Differenzierbarkeit von $f$!

\begin{definition}
  Sei $f\colon D\rightarrow \R, D\subset\R^n$ offen, eine reellwertige
  Funktion, so dass die partiellen Ableitungen $\diffp{f}{{x_i}}$
  existieren in $x\in D$. Dann heißt:
  \[\text{grad} f(x)=\left(\diffp{f}{{x_1}}(x),\ldots,\diffp{f}{{x_n}}(x)\right)\]
  \highl{Gradient} von $f$ an der Stelle $x$.
\end{definition}

\begin{remark}
  \begin{enumerate}[(i)]
  \item $f$ muss nicht notwendigerweise in $x$ differenzierbar sein!
  \item Ist $f$ in $x$ differenzierbar, so gilt:
    $f'(x)=\text{grad}f(x)$. Für die Richtungsableitung gilt dann
    $\diffp{f}{h}(x)=f'(x)h =(\text{grad}f(x))h$.
  \end{enumerate}
\end{remark}
...
\begin{theorem}
  ...
  Richtungsableitungen $\diffp{f}{h}(x)$ mit $\norm{h}_2
  =1$ eine grösste, nämlich die des Gradienten $\text{grad}f(x)$ mit
  $\diffp{f}{h}(x)=\norm{\text{grad}f(x)}_2$.
  \begin{proof}
    $\diffp{f}{h}(x)=(\text{grad}f(x))h$
  \begin{align*}
    \abs{\diffp{f}{h}(x)} & = \abs{\sum_{i=1}^n
  \diffp{f}{{x_i}}(x)h_i}\\
  & \leq \sqrt{\sum_{i=1}^n
  \abs{\diffp{f}{{x_i}}(x)}^2}\sqrt{\sum_{i=1}^n \abs{h_i}^2}\\
  & = \norm{\text{grad}f(x)}_2\norm{h}_2\\
  & = \norm{\text{grad}f(x)}_2 \text{ für }\norm{h}_2=1
\end{align*}

Für $h_0\coloneqq \frac{\text{grad}f(x)}{\norm{\text{grad}f(x)}_2}$ gilt:\\
$\diffp{f}{{h_0}}=(\text{grad}f(x))h_0=\left(\text{grad}f(x)\right)
\left(\frac{\text{grad}f(x)}{\norm{\text{grad}f(x)}_2}\right)=
\frac{\norm{\text{grad}f(x)}_2^2}{\norm{\text{grad}f(x)}_2}=\norm{\text{grad}f(x)}_2$
  \end{proof}
\end{theorem}

Die Funktion $f\colon D\subset\R^n\rightarrow \R$ sei differenzierbar
mit $f(a+h)=f'(a)h+r(h)$ und $\frac{r(h)}{\norm{h}}\rightarrow 0$.
\begin{enumerate}[(i)]
  \item Für "`kleine"' $h$, d.\,h. $\norm{h}$ "`klein"', ist eine Näherung
    für $f(a+h)$ die Formel
    \[f(a+h)\approx f(a)+f'(a)h=f(a)+(\text{grad}f(a))h\]
    Ausführlicher: $f(a_1+h_1,\dots,a_n+h_n)\approx f(a_1,\dots,a_n)+
    \sum_{i=1}^n \diffp{f}{{x_i}}(a_i)h_i$.
    Damit wird der Unterschied $f(a+h)-f(a)$ für "`kleine"' $h$ durch
    den Ausdruck $\diffp{f}{{x_1}}(a)h_1+\cdots+\diffp{f}{{x_n}}(a)h_n=f'(a)h$
    annähernd gegeben. Auf dieser Formel beruht die Fehlerrechnung.
  \item Für kleine $t\in\R$ und $h\in\R$ mit $\norm{h}_2=1$ wird der
    Unterschied $f(a+th)-f(a)\approx (\text{grad}f(x))th$ am grössten
    für $h=\frac{\text{grad}f(a)}{\norm{\text{grad}f(a)}_2}$, d.\,h. die
    Richtung des Gradienten.
\end{enumerate}

\begin{beispiel}[Zur Fehlerrechnung]
...
\end{pmatrix}
=V(l,h)+\diffp{V}{l}(l,h)\Delta l+\diffp{V}{h}
...
\diffp{f}{{x_k}}(x) &=\sum_{i=1}^m \diffp{g}{{u_i}}
(u(x))\diffp{u_i}{{x_k}}(x)
\end{alignat*}

Kettenregel: $f'(x)=g'(u(x))u'(x)$
\begin{align*}
\left(\diffp{f}{{x_1}}(x),\ldots,\diffp{f}{{x_n}}(x)\right)
& = & \left(\diffp{g}{{u_1}}(u(x)),\dots,\diffp{g}{{u_m}}(u(x))\right)
\begin{pmatrix}
  \diffp{u_1}{{x_1}}(x) & \dots & \diffp{u_1}{{x_n}}(x)\\
  \vdots & & \vdots\\
  \diffp{u_m}{{x_1}}(x)& \dots & \diffp{u_m}{{x_n}}(x)
\end{pmatrix}\\
& = & \left(\diffp{g}{{u_1}}(u(x))\diffp{u_1}{{x_1}}
(x)+\dots+\diffp{g}{{u_m}}(u(x))\diffp{u_m}{{x_1}}
(x),\dots,\right.\\
& & \quad\left.\diffp{g}{{u_1}}(u(x))\diffp{u_1}{{x_n}}(x)
+\dots+\diffp{g}{{u_m}}(u(x))\diffp{u_m}{{x_n}}(x)\right)\\
& \Rightarrow & \diffp{f}{{x_k}}(x)=\diffp{g}{{u_1}}
(u(x))\diffp{u_1}{{x_k}}(x)+\dots+\diffp{g}{{u_m}}
(u(x))\diffp{u_m}{{x_k}} (x)
\end{align*}
...
\begin{theorem}
  ...
  \begin{proof}[Anwendung des eindimensionalen Mittelwertsatzes]
...
\begin{enumerate}[(i)]
  \item In der Version von  \autoref{satz:MWSreell}
    gilt der Mittelwertsatz für vektorwertige Funktionen \highl{nicht} mehr.\\
    Beispiel: $f\colon [0,2\pi]\rightarrow\R^2, f(t)=\begin{pmatrix}\cos t\\\sin t
    \end{pmatrix}$ Zu zeigen ist: $\nexists 0<\vartheta<1\colon f(2\pi)-
    f(0)=f'(\vartheta 2\pi)2\pi$. Denn $f(2\pi)-f(0)=0, f'(t)=\begin{pmatrix}
      (\cos t)'\\(\sin t)'
    \end{pmatrix}=\begin{pmatrix}
      -\sin t\\\cos t
    \end{pmatrix},
    \norm{f'(t)}_2=1\Rightarrow f'(t)\neq 0\,\forall t$
...
gilt:
\[\norm{\int_a^bf(t)\,dt}_2\leq\int_a^b\norm{f(t)}_2\,dt\]
Diese Ungleichung gilt auch für jede beliebige Norm $\norm{\cdot}$ auf
$\R^n$
\begin{proof}
Sei $u\coloneqq \int_a^b f(t)dt\in\R^n$. Dann $\norm{u}_2^2=
\langle u,u\rangle=\langle\int_a^b f(t)dt, u\rangle=\int_a^b \langle f(t),u\rangle\,dt\leq
\int_a^b \norm{f(t)}_2 \norm{u}_2dt=\left(\int_a^b \norm{f(t)}_2
dt\right)\norm{u}_2\Rightarrow \norm{\int_a^b f(t)dt}_2=\norm{u}_2\leq \int_a^b \norm{f(t)}_2dt$  
\end{proof}
...
\begin{theorem}
  ...
  Ferner gilt: 
  \[\norm{f(x+h)-f(x)}_2\leq \sup_{0\leq t \leq 1}
  \underbrace{\norm{f'(x+th)}}_{\text{Operatorennorm}}\cdot \norm{h}_2\]
  Die Operatorennorm ist genauer $\norm{f'(x+th)}\colon [\R^n,\norm{\cdot}_2]
  \rightarrow[\R^m,\norm{\cdot}_2]$
  \begin{proof}
    Sei $f\colon \begin{pmatrix}f_1\\\vdots\\f_m
  \end{pmatrix}$ mit $f_i\colon D\rightarrow\R$ stetig differenzierbar. Wir
  ...
  Ferner: $\norm{f(x+h)-f(x)}_2=\norm{\int_0^1 
  f(x+th)dt h}\leq \norm{\int_0^1 f'(x+th)dt}\cdot\norm{h}_2$\\
  $\leq \int_0^1 \sup_{0\leq t\leq 1}
  \norm{f'(x+th)}dt\norm{h}_2=(\sup_{0\leq t\leq 1}\norm{f'(x+th)})\norm{h}_2$
  \end{proof}
\end{theorem}
...
\begin{definition}
  \begin{enumerate}[(i)]
    \item Sei $E$ ein normierter Raum und $x_0,x_1,\dots,x_n\in E$. Dann
      ...
      \highl{Polygonzug} durch die Punkte $x_0,\dots,x_n$.
    \item $D\subset E$ heißt genau dann \highl{polygonzugzusammenhängend},
...
\begin{theorem}
  ...
  \begin{proof}
    Es gilt: $f$ ist stetig differenzierbar auf $D$, da
  ...
  Aus \autoref{satz:MWSint} folgt> $f(x_0)=f(x_1)=\dots =f(x_n)=f(x)$.
  Also $f(x)=f(x_0)\,\forall x\in D$. Daher folgt, dass $f$ konstant ist.
  \end{proof}
\end{theorem}

$D$ heißt \highl{Gebiet}, wenn $D$ offen und
...
$\diffp{f}{{x_i}}\colon D\rightarrow\R$ selbst wieder partiell
differenzierbar, so heißt $f$ zweimal differenzierbar. Man kann die
partiellen Ableitungen 2.\,Ordnung bilden:
\[\diffp{f}{{x_j}{x_i}}\coloneqq \diffp{}{{x_j}}\left(\diffp{f}{{x_i}}\right)\]
...
alle partiellen Ableitungen der $k$-ter Ordnung $\diffp{f}{{x_{i_k}}\ldots {x_{i_2}} {x_{i_1}}}
...
\begin{theorem}[Satz von Schwartz über die Vertauschbarkeit der
  ...
  \diffp{f}{{x_j} {x_i}} (\xi)=\diffp{f}{{x_i} {x_j}}(\xi)\]
  \begin{proof}
    O.\,B.\,d.\,A.: $n=2, i=1, j=2, x_1\rightarrow x, x_2
  \rightarrow y$. Wir zeigen $\diffp{f}{{x} {y}}(\xi,\eta)=
  \diffp{{f}}{{y} {x}}(\xi,\eta)$ Wählen auf $\R^2$
  die Maximumnorm. Dann existiert die $\varepsilon$-Kugel
  $B_\varepsilon(\xi,\eta)\coloneqq \{(\tilde{\xi},\tilde{\eta})\in\R^2\colon 
  \max\{\abs{\tilde{\xi}-\xi},\abs{\tilde{\eta}-\eta}\}<\varepsilon\}\subset D$.
  Damit: $(\xi+h,\eta+k)\in B_\varepsilon(\xi,\eta)$ für $\abs{h},\abs{k}<
  \varepsilon$. Sei $0<\abs{h},\abs{k}<\varepsilon$. Wir definieren $\varphi (x)\coloneqq 
  ...
  \begin{align*}
    ...
    & = & h\left[\diffp{f}{x}(x_1,\eta+k)-
    \diffp{f}{x}(x_1,\eta)\right]\\
    & \Rightarrow & \exists y_1\in]\eta,\eta+k[\colon 
    \diffp{f(x_1,\eta+k)}{x}-\diffp{f}{x}(x_1,\eta)\\
    & = & k\diffp{f}{{y} {x}}(x_1,y_1)
  \end{align*}
  Also $F(h,k)=hk\diffp{{f}}{{y} {x}}(x_1,y_1)$ mit
  $x_1\in]\xi,\xi+h[$ und  $y_1\in]\eta,\eta+k[$. Nun 
  ...
  y_2\in]\eta,\eta+k[\colon F(h,k)=hk\diffp{{f}}{{x} {y}} (x_2,y_2)$\\
  $\stackrel{h,k\neq 0}{\Rightarrow} \diffp{f}{{y} {x}}
  (x_1,y_1)=\diffp{f}{{x}{y}}(x_2,y_2)
  \stackrel{h,k\rightarrow 0}{\Rightarrow} \diffp{f}{{y}{x}}
  (\xi,\eta)=\diffp{f}{{x}{y}}(\xi,\eta)$
  \end{proof}
\end{theorem}

\begin{remark}
  Der Beweis hat gezeigt, dass es genügt, die Existenz der partiellen
  Ableitungen der zweiten Ordnung in einer Umgebung von $(\xi,\eta)$ und die
  Stetigkeit in $(\xi,\eta)$ vorauszusetzen.
\end{remark}

\begin{Folg}
  Sei $f\colon D\rightarrow\R,D\subset\R^n$ offen, $k$ mal stetig differenzierbar.
  Dann gilt:
  \[\diffp[k]{f}{{x_{i_k}}{\ldots} {x_{i_1}}}(\xi)= \diffp[k]{f}{{x_{i_{\pi(k)}}\dots x_{i_{\pi(1)}}}}\]
...
\begin{theorem}
  ...
  \diffp{}{{x_1}}+h_2\diffp{}{{x_2}} +\dotsc+h_n\diffp{}{{x_n}}\right)^k
  f(\xi)\right)\]
  \[\qquad+\frac{1}{(m+1)!}\left(h_1\diffp{}{{x_1}}+\dotsc+
    h_n\diffp{}{{x_n}}\right)^{m+1}f(\xi+\Theta h)\]
  wobei 
  \begin{align*}
    (h_1\diffp{}{{x_1}}+\dotsc+h_n \diffp{}{{x_n}})^k f(\xi)
    &\coloneqq \sum_{i_1,\dots,i_k=1}^n \diffp[k]{f}{{x_{i_k}\ldots
        x_{i_1}}}(\xi) h_{i_1}\dots h_{i_k}\\
    &= \sum_{\alpha_1+\dotsc+\alpha_k}
    \frac{k!}{\alpha_1!\dots\alpha_n!} \cdot
    \diffp[k]{}{{x_{i_n}^{\alpha_n}\dots x_{i_1}^{\alpha_1}}}
    (\xi)h_1^{\alpha_1}\dots h_n^{\alpha_n}
  \end{align*}
  \begin{proof}
    Der Beweis erfolgt für den Spezialfall $n=2\colon $
    \[f(\xi+h)=\sum_{k=0}^m\frac{1}{k!}\left(h_1
      \diffp{}{{x_1}}+h_2\diffp{}{{x_2}}\right)^k
    f(\xi)+\frac{1}{(m+1)!}\left(h_1\diffp{}{{x_1}}+h_2
      \diffp{}{{x_2}}\right)^{m+1}f(\xi+\Theta h)\]
  wobei 
  \begin{align*}
    \left(h_1\diffp{}{{x_1}}+h_2 \diffp{}{{x_2}}\right)^k f(\xi)
    &\coloneqq \sum_{\alpha_1+\alpha_2=k}
    \frac{k!}{\alpha_1!\alpha_2!} \diffp[k]{}{{x_2^{\alpha_2}
        x_1^{\alpha_1}}} h_1^{\alpha_1}h_2^{\alpha_2}\\
    &=\sum_{j=0}^k\binom{k}{j} \diffp[k]{f}{{x_2^{k-j} x_1^j}} h_1^j h_2^{k-j}
  \end{align*}
  $\xi=\begin{pmatrix}
    \xi_1\\\xi_2
  \end{pmatrix}
...
$\varphi^{(k)}(t)=\sum_{j=0}^n \binom{k}{j} \diffp[k]{f}{{x_2^{k-j} x_1^j}}
  h_1^j h_2^{k-j}(\xi_1+th_1,\xi_2+th_2)$. Durch Induktion erhält man 
  folgendes:\\
  Induktionsanfang: 
  \begin{align*}
    \varphi'(t) & = & f'(\xi+th)h\\
    & = & \left(\diffp{f}{{x_1}}(\xi+th) \diffp{f}{{x_2}}(\xi+th)\right)
    \begin{pmatrix}
      h_1\\h_2
    \end{pmatrix}\\
    & = & h_1\diffp{f}{{x_1}}(\xi+th)h_2\diffp{f}{{x_2}}(\xi+th)
  \end{align*}
  für $h=1$\\
  ...
  \begin{align*}
    ...
    &= \left(\sum_{j=0}^k \binom{k}{j} h_i^j h_2^{k-j}
      \diffp[k]{f}{{x_1^j x_2^{k-j}}}(\xi+th)\right)'\\
    &= \sum_{j=0}^k\binom{k}{j} h_1^{j+1} h_2^{k-j}
    \diffp[k+1]{f}{{x_1^{j+1} x_2^{k-j}}} + \sum_{j=0}^k \binom{k}{j}
    h_1^j h_2^{k-j+1} \diffp[k+1]{f}{{x_1^j x_2^{k-j+1}}}(\xi+th)\\
    &= \sum_{j=1}^{k+1} \binom{k}{j-1}h_1^j h_2^{k-j+1}
    \diffp[k+1]{f}{{x_1^j x_2^{k-j+1}}}(\xi+th)\\
    &\quad+\sum_{j=0}^k \binom{k}{j}h_1^j h_2^{k-j+1}
    \diffp[k+1]{f}{{x_1^j x_2^{k-j+1}}}(\xi+th)\\
    &= \sum_{j=0}^{k+1} \binom{k+1}{j} h_1^j h_2^{k-j+1}
    \diffp[k+1]{f}{{x_1^j x_2^{k+1-j}}}(\xi+th)
  \end{align*}
  ...
  \diffp{}{{x_1 +h_2\diffp{}{{x_2}}}})^k f(\xi)
  +\frac{1}{(m+1)!}(h_1\diffp{}{{x_1}}+h_2\diffp{}{{x_2}})^{m+1}f(\xi+\Theta h)$
  \end{proof}
\end{theorem}
...
\begin{align*}
  f(\xi+h) &= f(\xi)+\left(h_1\diffp{}{{x_1}}+\dotsc+h_n
    \diffp{}{{x_n}}\right)f(xi)+\\
  &\quad \frac{1}{2}\left(h_1 \diffp{}{{x_1}}+\dotsc+
    h_n\diffp{}{{x_n}}\right) f(xi)^2f(\xi+\Theta h)\\
  &= f(\xi)+h_1\diffp{h}{{x_1}}+\dotsc+h_n \diffp{f}{{x_n}}(\xi)+
  \frac{1}{2}\sum_{j,k=1}^n \diffp{f}{{x_k} {x_j}}(\xi+\Theta
  h)h_jh_k\\
  &= f(\xi)+f'(\xi)h+\frac{1}{2}\sum_{j,k=1}^n \diffp{f}{{x_k}
    {x_j}}(\xi+\Theta h)h_jh_k
\end{align*}
...
\begin{theorem}
  ...
  \[f(\xi+h)=f(\xi)+f'(\xi)h+\frac{1}{2}\sum_{j,k=1}^n \diffp{f}{{x_k} {x_j}} (\xi)
  h_jh_k+\norm{h}_2^2\rho(h)\]
  mit $\lim_{h\rightarrow 0}\rho(h)=0$.
  \begin{proof}
    Nach dem Korollar oben $f(\xi+h)=f(\xi)+f'(\xi)h+
  \frac{1}{2} \sum_{j,k=1}^n \diffp{f}{{x_j} {x_k}}
  (\xi) h_jh_k+r(h)$ mit $r(h)=\frac{1}{2}\sum_{j,k=1}^n \left[
    \diffp{f}{{x_k} {x_j}}(\xi+\Theta h)- \diffp{f}{{x_k}
      {x_j}}(\xi)\right]h_jh_k$.
  Somit 
  \begin{align*}
    \abs{r(h)} &\leq \frac{1}{2} \left(\sum_{j,k=1}^n
      \sqrt{\abs{\diffp{f}{{x_k} {x_j}}(\xi+\Theta h)- \diffp{f}{{x_k}
            {x_j}}}^2}\right) \underbrace{\sqrt{\sum_{j,k=1}^n
        \abs{h_j}^2 \abs{h_k}^2}}_{\sum_{j,k=1}^n \abs{h_j}^2=\norm{h}_2^2}\\
    \sum_{j,k=1}^n \abs{h_j}^2\abs{h_k}^2 &= \sum_{k=1}^n \sum_{j=1}^n
    \abs{h_j}^2\abs{h_k}^2\\
    &= \sum_{k=1}^n (\abs{h_k}^2\sum_{j=1}^n \abs{h_j}^2)\\
    &= \left(\sum_{j=1}^n \abs{h_j}^2)(\sum_{k=1}^n \abs{h_k}^2\right)\\
    &= \left(\sum \abs{h_j}^2\right)
  \end{align*}
  Also: $\frac{\abs{r(h)}}{\norm{h}_2^2}\leq \frac{1}{2}
  \sqrt{\sum_{j,k=1}^n \abs{\diffp{f}{{x_k} {x_j}} (\xi+\Theta
        h)-\diffp{f}{{x_k} {x_j}}(\xi)}}\rightarrow 0$\\
...
\begin{definition}
  ...
  \highl[Maximum!lokales]{lokales Maximum} bzw. \highl[Minimum!lokales]{lokales
    Minimum}, falls eine Kugel $B_\varepsilon
...
spricht man von einem \highl[Maximum!isoliertes]{isoliertem lokalem Maximum}
  bzw. \highl[Minimum!isoliertes]{Minimum}.
...
\begin{theorem}
  ...
  Dies ist gleichbedeutend mit $\diffp{f}{{x_1}}(x)=\dotsc=
  \diffp{f}{{x_1}}(x)=0$
\end{theorem}
\begin{proof}
  ...
  $\varphi'_i(0)=0=\lim_{t\rightarrow 0} \frac{f(x+te_i)-
    f(x)}{t}=\diffp{f}{{x_i}}(x)$. Also ist der
  Gradient in $f(x)=\diffp{f}{{x_i}}(x),\ldots, \diffp{f}{{x_n}}(x))=0$
\end{proof}

\begin{definition}
  Sei $A=(a_{ij})_{i,j=1}^n$ eine symmetrische reelle $n\times n$ Matrix
  A heißt \highl{positiv definit}, genau dann
  wenn \[\forall x\in\R^n\backslash \{0\}\colon  \langle Ax,x\rangle >0\]
  A heißt \highl{positiv semidefinit}, genau dann wenn
  ...
  A heißt \highl{indefinit}, genau dann wenn
...
positiv, wenn $\exists \alpha >0\,\forall x\in\R^n\colon \langle Ax,x\rangle \geq \alpha \norm{x}_2^2$
oder alle Eigenwerte von $A$ sind positiv, d.\,h. $\lambda >0$
oder $\forall k=1,\dotsc,n\colon  \det\begin{pmatrix}a_{11}& \dotsc & a_{1k}\\
\vdots & & \vdots\\
a_{k1} & \dots & a_{kk}
\end{pmatrix}>0$\\
...
\begin{definition}
  Sei $D\subset\R^n$ offen und $f\in C^2(D)$. Die \highl[Matrix!Hessesche]{Hessesche
    Matrix} von $f$ im Punkt $x\in D$ ist
  definiert durch:
  \[(\text{Hess}f)(x)\coloneqq
  \left(
    \diffp{f}{{x_j} {x_i}}
  \right)_{i,j=1}^n\]
  Wegen $\diffp{f}{{x_j} {x_i}}= \diffp{f}{{x_i} {x_j}}$ ist $(\text{Hess} f)(x)$
  symmetrisch. Nach dem  \autoref{satz:DiffExist} hat man
  $f''(x)=(\text{Hess}f)(x)$.
\end{definition}

Mit $f'(x)h=\langle \text{grad }f(x),h\rangle $ und $\langle f''(x)h,h\rangle =\langle \text{Hess }f(x)h,h\rangle 
=\sum_{i,j=1}^n \diffp{f}{{x_j} {x_i}}h_ih_j$
kann  \autoref{satz:II36} wie folgt beschrieben werden:\\
...
+\langle f'(x),h\rangle +\frac{1}{2}\langle f''(x)h,h\rangle +\norm{h}^2\rho(h)$ wobei $\lim_{h\rightarrow 0}
\rho(h)=0$.
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    \item Ist $f''(x)=(\text{Hess }f)(x)$ positiv definit, so hat $f$ in
      ...
      \begin{proof}
        (Die Beweise zu (i) und (ii) können analog geführt werden.)
      $f(x+h)=f(x)+\langle f'(x),h\rangle +\frac{1}{2}\langle f''(x)h,h\rangle +\norm{h}^2\rho(h)
      =f(x)+\frac{1}{2}\langle f''(x)h,h\rangle +\norm{h}^2\rho(h)$\\
      Wählen $\delta>0\colon \forall h\colon \norm{h}_2\leq \delta\Rightarrow\abs{\rho(h)}
      \leq \frac{\alpha}{4}$. Damit erhält man für $0<\norm{h}_2\leq \delta\colon 
      f(x+h)=f(x)+\frac{1}{2}\langle f''(x)h,h\rangle +\norm{h}_2^2\rho(h)\geq f(x)+
      \frac{1}{2} \langle f''(x)h,h\rangle -\norm{h}_2^2\abs{\rho(h)}\geq f(x)+\frac{\alpha}{2}
      \norm{h}_2^2-\frac{\alpha}{4}\norm{h}_2^2=f(x)+\frac{\alpha}{4}\norm{h}_2^2
      >f(x)$
      \end{proof}
    ...
    \item Ist $f''(x)$ indefinit, so besitzt $f$ in $x$ kein lokales
      Extremum.
      \begin{proof}
        Zeigen: In jeder $\varepsilon$-Kugel $B_\varepsilon(x)$ um
      ...
      Für kleine $\abs{t}$ gilt: $f(x+t\xi)=f(x)+\frac{1}{2}\langle f''(x)t\xi,t\xi\rangle 
      +\norm{t\xi}_2^2\rho(t\xi)$ und für hinreichend kleine $\abs{t}$ gilt
      $\abs{\rho(t\xi)}\leq\frac{\alpha}{4}\frac{1}{\norm{\xi}_2^2}$. Also hat
      man $f(x+t\xi)=f(x)+\frac{t^2}{2}\langle f''(x)\xi,\xi\rangle +t^2\norm{\xi}_2^2
      \rho(t\xi)=f(x+\frac{\alpha}{2}t^2+t^2\norm{\xi}_2^2\rho(t\xi)\geq
      f(x)+\frac{\alpha}{2}t^2-t^2\norm{\xi}_2^2\abs{\rho(t\xi)}\geq f(x)+
      \frac{\alpha}{2}t^2-\frac{\alpha}{4}t^2=f(x)+\frac{\alpha}{4}t^2
      >f(x)$ für $0<\abs{t}\leq\delta_0$\\
      Ebenso zeigt man: Ist $\eta\in\R^n$ ein Vektor mit $\langle f''(x)\eta,\eta\rangle 
      <0$, so gilt für genügend kleine $\abs{t}>0\colon  f(x+t\eta)<f(x)$
      \end{proof}
  \end{enumerate}
  \qed
\end{theorem}
...
d.\,h. $\diffp{f}{x}(\xi,\eta)=\diffp{f}{y}(\xi,\eta)=0$\\
$f''=\text{Hess } f=\begin{pmatrix}
  \diffp[2]{f}{x} & \diffp{f}{{y} {x}}\\
  \diffp{f}{{y} {x}} & \diffp[2]{f}{y}
\end{pmatrix}$\\
$A=\begin{pmatrix}
  a_{11}&a_{12}\\
  a_{12}&a_{22}
...
Setzen $\Delta\coloneqq \det f'' =\diffp{f}{{x}{x}} \diffp{f}{{y}{y}}
-(\diffp{f}{{y} {x}})^2$, so hat
\begin{enumerate}[(i)]
  \item $f$ ein isoliertes lokales Minimum in $(\xi,\eta)$, wenn 
    $\diffp{f}{{x}{x}}>0,\Delta>0$
...
\begin{enumerate}
  \item $f(x,y)=c+x^2+y^2, \text{grad }f(x,y)=\left(
    \diffp{f}{x},\diffp{f}{y}\right)=
    (2x,2y)$\\
    $f''(x,y)=\begin{pmatrix}
      \diffp{f}{{x}{x}}&\diffp{f}{{y} {x}}\\
      \diffp{f}{{y} {x}}&\diffp{f}{{y}{y}}
    \end{pmatrix}=
    \begin{pmatrix}
      2&0\\0&2
    \end{pmatrix},
...
$g(x)\coloneqq Ax-b, h(y)\coloneqq \norm{y}_2^2, g'(x)=A,
h'(y)=(\diffp{h}{{y_1}}, \ldots,\diffp{h}{{y_n}})=2(y_1,\dots,y_n)=2y^T,
...
=\langle Ax,Ax\rangle =\norm{Ax}_2^2\Rightarrow Ax=0\Rightarrow x=0
...
\rangle +\langle Ah, Ah\rangle = \norm{Ax-b}_2^2+2\langle Ax-b,Ah\rangle+
\norm{Ah}_2^2=f(x)+2\langle A^T(Ax-b), h\rangle +\norm{Ah}_2^2=f(x)+\norm{Ah}_2^2$
für $x=(A^TA)^{-1}A^Tb\Rightarrow \forall h\neq 0\colon  f(x+h)=f(x)+\norm{Ah}_2^2
...
\leq N$\\
$f(x)=\sum_{k-1}^N \norm{Ax-b_k}_2^2\rightarrow$ Minimum.\\
...
Eine Funktion $f\colon D\rightarrow\R^m,D\subset\R^n$ wird \highl{implizit}
genannt, wenn sie durch eine Gleichung der Form
\[F(x,y)=0\]
gegeben ist, d.\,h. $F(x,f(x))=0$ für alle $x\in D$.
...
\begin{beispiel}
  $n=m, F(x,y)=Ay-x, A\colon \R^n\rightarrow\R^n, F\colon \R^n\times\R^n
\rightarrow\R^n$\\
...
\begin{enumerate}[(a)]
  ...
  \item $F\colon \R\times\R\rightarrow\R,F(x,y)\coloneqq x^2+y^2-1=0$\\
    ...
    $y=f(x)$ mit $\eta=f(\xi)$ definiert wird
    (\highl[Auflösbarkeit!lokale]{lokale Auflösbarkeit} von $F(x,y)=0$ nach $y$).
\end{enumerate}
...
\[\diffp{F}{x}\coloneqq \begin{pmatrix}
  \diffp{F_1}{{x_1}} & \dots & \diffp{F_1}{{x_n}}\\
  \vdots & & \vdots\\
  \diffp{F_m}{{x_1}} & \dots & \diffp{F_m}{{x_n}}
\end{pmatrix}\]
ist eine $m\times n$"=Matrix und
\[\diffp{F}{y}\coloneqq \begin{pmatrix}
  \diffp{F_1}{{y_1}} & \dots & \diffp{F_1}{{y_n}}\\
  \vdots & & \vdots\\
  \diffp{F_m}{{y_1}} & \dots & \diffp{F_m}{{y_n}}
\end{pmatrix}\]
...
\begin{theorem}
  ...
  $\eta \in G$ Punkte, für die $F(\xi,\eta)=0$ und $\diffp{F}{y}
  ...
  \begin{enumerate}[(i)]
    ...
    \item Es existiert eine weitere offene $\delta_1$-Kugel $B_{\delta_1}
      (\xi)$ mit $\delta_1\leq \delta$ und die Funktion $f\colon B_{\delta_1}(\xi)
      \rightarrow B_\varepsilon(\eta)$ ist stetig differenzierbar auf
      $B_{\delta_1}(\xi)$ und es gilt die Formel:
      \[f'(x)=-\left(\diffp{F}{y} (x,f(x))\right)^{-1}
      \diffp{F}{x} (x,f(x))\]
  \end{enumerate}
...
\begin{align*}
  ...
  F'(x,y) &=
  \left(\diffp{F}{x}\diffp{F}{y}\right) &
  g'(x)=\begin{pmatrix}I\\f'(x)
  \end{pmatrix}\\
  F'(x,f(x))g'(x) &=\diffp{F}{x}(x,f(x))I+
\diffp{F }{y}(x,f(x))f'(x)=h'(x)& h(x)=0
 \end{align*}
$\Rightarrow
h'(x)=0\Rightarrow\diffp{F}{x}(x,f(x))+
\diffp{F }{y}(x,f(x))f'(x)=0$\\
$f'(x)=-\left(\diffp{F}{y}(x,f(x))\right)^{-1}
\diffp{F}{x}(x,f(x))$.
...
\begin{beispiel}
  ...
  \diffp{F}{y} & = 2y \neq 0 & \text{für }y\neq 0\\
  \left(\diffp{F}{y}\right)^{-1} & =\frac{1}{2y} &
\end{align*}
...
auflösbar.\\
$F(x,f(x))=x^2+f(x)^2-1=0, \diffp{F}{x}=2x$\\
$f'(x)=-\left(\diffp{F}{y}\right)^{-1}(x,f(x))\cdot
\left(\diffp{F}{x}\right)(x,f(x))=-\frac{1}{2f(x)}\cdot
2x=-\frac{x}{f(x)}, y>0, f(x)=\sqrt{1-x^2}, f'(x)=-\frac{x}{\sqrt{1-x^2}}$
\end{beispiel}


\subsection{Umkehrsatz}
Problem: Unter welchen Bedingungen besitzt eine Funktion $f\colon
D\rightarrow \R^n, D\subset\R^n$, eine Umkehrung? Welche analytischen
Eigenschaften besitzt die Funktion $f^{-1}$? Dabei ist $f(x)=Ax,
A\colon \R^n\rightarrow\R^n, f(x)=Ax=y\rightarrow x=A^{-1}y,
f^{-1}(y)=A^{-1}y,f'(x)=A$.
...
\begin{theorem}
  ...
  mit $\diffp{F}{x}(\xi,f(\xi))=
  f'(\xi)$ und  $F(\xi,\eta)=0$ invertierbar. \autoref{satz:implFunktion} impliziert, dass es
  ...
  \autoref{satz:implFunktion} können wir $B_\varepsilon(\eta)$ so klein
  ...
  F(x_2,y)=0\stackrel{\text{\autoref{satz:implFunktion}}}{\Rightarrow}
...
\begin{enumerate}
  \item Sei $f\colon U\rightarrow V$ eine bijektive Abbildung der offenen
    ...
    heißt $f$ ein \highl{Diffeomorphismus}. In
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    \item $f$ ist eine \highl[Abbildung!offene]{offene Abbildung},
      d.\,h. das Bild jeder offenen Teilmenge von $D$ ist offen im
      $\R^n$.
      \begin{proof}
        $A\subset D$ offen in $D$. Da $D$ offen ist, folgt, dass
        $A$ in $\R^n$ offen ist.
        ...
        \end{enumerate}
      \end{proof}
    \item Die Funktion $\varphi\colon D\rightarrow\R, \varphi(x)\coloneqq \norm{f(x)}$,
      besitzt kein Maximum und, falls $f(x)\neq 0$ in $D$, auch kein
      Minimum on $D$.
      \begin{proof}
        Annahme: $x_0\in D$ sei eine Maximalstelle von $\varphi$,
      d.\,h. $\forall x\in D\colon \norm{f(x)}\leq \norm{f(x_0)}$. Dann ist $f(x_0)>0$.
      ...
      f(D)$. Insbesondere gilt für $\rho=\frac{\varepsilon}{2\norm{f(x_0)}}$\\
      $\exists x_1 \in D\,y_1=f(x_1)=f(x_0)+\rho f(x_0)=(1+\rho)f(x_0)$.
      Denn $\norm{y_1-f(x_0)}=\rho\norm{f(x_0)}=\frac{\varepsilon}{2}<\varepsilon$.
      Somit hat man $\norm{f(x_1)}=(1+\rho)\norm{f(x_0)}>\norm{f(x_0)}$\lightning
      \end{proof}
    \item Ist $f$ injektiv, so ist $f^{-1}\colon f(D)\rightarrow\R$ stetig
      differenzierbar.
      \begin{proof}
        Nach dem Punkt (i) ist $f(D)$ offen. Weiterhaben wir nach dem
        Umkehrsatz und wegen der Injektivität von $f$, die Folgerung
        $f^{-1}\colon f(D)\rightarrow\R^n$ ist stetig differenzierbar.
      \end{proof}      
  \end{enumerate}
\end{theorem}

\section{Extrema mit Nebenbedingungen}
Seien $f\colon X\rightarrow\R, X\subset\R^n$ offen und $g\colon
X\rightarrow\R^m$ mit $m<n$. Dann besitzt $f$ in $\xi\in X$ ein lokales Maximum bzw. lokales Minimum
...
möglich, dann hilft uns die so genannte Multiplikatorenmethode.
...
\begin{theorem}
  ...
  $g(x)=0$. Ferner existieren in $g'(\xi)=\begin{pmatrix}
    \diffp{g_1}{{x_1}}(\xi) & \dots & \diffp{g_1}{{x_n}}(\xi)\\
    \vdots & & \vdots\\
    \diffp{g_m}{{x_1}}(\xi) & \dots & \diffp{x_m}{{x_n}}(\xi)
  \end{pmatrix}$ eine $m$-reihige Unterdeterminante, die nicht
  verschwindet. Dann existieren $m$ Zahlen $\lambda_1,\dots,\lambda_m$
  (\highl{Lagrange Multiplikatoren}), so dass
  die Gleichung
...
\begin{proof} O.\,B.\,d.\,A. sei die Determinante der Matrix 
  ...
  impliziert:\\
$g(\xi)=g(\eta,\zeta)=0$ und wegen \autoref{eq:detA} ist
$\begin{pmatrix}
    \diffp{g_1}{{x_1}}(\eta,\zeta) & \dots & \diffp{g_1}{{x_n}}(\eta,\zeta)\\
    \vdots & & \vdots\\
    \diffp{g_m}{{x_1}}(\eta,\zeta) & \dots & \diffp{x_m}{{x_n}}(\eta,\zeta)
  \end{pmatrix}$.
  Nach \autoref{satz:implFunktion} folgt $g(y,z)=0$ ist stetig differenzierbar
  nach $y$ auflösbar, d.\,h.
  \begin{gather}
    \label{eq:Glb}
    \exists B_\delta(\zeta)\subset\R^{n-m}\,h
    \colon B_\delta(\zeta)\rightarrow \R^m\colon  h(\zeta)=\eta \wedge\forall
  z\in _\delta(\zeta)\colon  g(h(z),z)=0
  \end{gather}
  Ferner sei $\delta$ so klein, dass
  $\varphi(z)\coloneqq f(h(z),z)$ differenzierbar ist. Es gilt:
  \[\varphi'(\zeta)=\diffp{f}{y}(\xi)h'(\zeta)+
  \diffp{f}{z}(\xi)\]
  ...
  \begin{gather}
    \label{eq:Glc}
    \diffp{f}{y}
  (\xi)h'(\zeta)+\diffp{f}{z}(\xi)=0
  \end{gather}
  Ausserdem hat man wegen der  \autoref{eq:Glb}
  mit \autoref{satz:implFunktion}, dass 
 \[h'(\zeta)=-\left(\diffp{g}{y}(\xi)\right)^{-1}
  \diffp{g}{z}(\xi)\]
  Einsetzen in die \autoref{eq:Glc} liefert:
  $\underbrace{-\diffp{f}{y}(\xi)
  \left(\diffp{g}{y}(\xi)\right)^{-1}}_{\lambda^T=
  (\lambda_1,\dots,\lambda_m)}
  \diffp{g}{z}(\xi)+\diffp{f}{z}(\xi)
  =0$.\\
  Somit ist $\diffp{f}{y}(\xi)+\lambda^T
  \diffp{g}{y}(\xi)=0$ und $\diffp{f}{z}
  (\xi)+\lambda^T\diffp{g}{z}(\xi)=0$. Beide 
...
\[\diffp{f}{{x_k}}(x)+\lambda_1
\diffp{g_1}{{x_k}}(x)+\cdots+\lambda_m
\diffp{g_m}{{x_k}}(x)=0\]
und 
\[g_j(x)=0\quad j=1,\dots,m\]
für die $n+m$ Unbekannten $x_1,\dots,x_n, \lambda_1,\dots,\lambda_m$,
die sich aus \autoref{satz:lagrange} ergeben.
...
\begin{enumerate}
  \item Man bildet die Funktion $F\colon X\times\R^m\rightarrow\R, F(x,\lambda)
    ...
    \end{pmatrix}$ mit $\lambda=\begin{pmatrix}\lambda_1\\\vdots\\ \lambda_m
    \end{pmatrix}$
  \item Differentiation nach $(x,\lambda)$ und Null setzen. $F'(x,\lambda)
    =0=\left(\diffp{F}{x}, \diffp{F}{\lambda}\right)
...
\begin{enumerate}[(i)]
  ...
  \item Seien $A,B\subset\R^n$ und $A\neq\emptyset$ ist kompakt und $B$ 
    ist abgeschlossen. Dann existiert $\xi\in A, \eta\in B\colon \norm{\xi-\eta}
    =\inf_{\stackrel{x\in A}{y\in B}} \norm{x-y}$\\
    Die Kompaktheit der Menge ist notwendig, denn:\\
    $A=\{(t,\sqrt{t^2-1}\colon 1\leq t\leq\infty\}, \nexists \xi\in A,
    \eta\in B\colon \norm{\xi-\eta}=\inf_{\stackrel{x\in A}{y\in B}} \norm{x-y}$
\end{enumerate}
...
\item Bestimme Minima und Maxima der Funktion $f(x,y,z)=5x+y-3z$ unter
  ...
  Eine zweireihige Unterdeterminante von $g'$ auf $N$ ist $\neq 0$.\\
    ...
    \begin{align}
      \label{1}
      \diffp{F}{x} & = 5+\lambda+2\nu x=0\\
      \label{2}
      \diffp{F}{y} & = 1+\lambda+2\nu y=0\\
      \label{3}
      \diffp{F}{z} & = -3+\lambda+2\nu z=0\\
      \label{4}
      \diffp{F}{\lambda} & = x+y+z=0\\
      \label{5}
      \diffp{F}{\nu} & = x^2+y^2+z^2-1=0
    \end{align}
    \autoref{1}, \autoref{2} und \autoref{3}: $3+3\lambda+2\nu(\underbrace{x+y+z}_{=0}
    )=0\Rightarrow 3+3\lambda=0\Rightarrow \lambda=-1 \Rightarrow
    \left.\begin{array}{rcl}
    4+2\nu x & = & 0\\
    2\nu y & = & 0
  \end{array}\right\}\Rightarrow \nu\neq 0, y=0\substack{\text{\autoref{4},\autoref{5}}}{\Rightarrow}
  \left.\begin{array}{rcl}
    x+z & = & 0\\
    x^2+y^2 & = & 1
  ...
  \autoref{1} bis \autoref{5} in Verbindung mit $\lambda=-1, \nu=-2\sqrt{2}$
  bzw. $\nu=2\sqrt{2}$.\\
  ...
  \begin{align*}
    \abs{f(x,y,z)}&=4\abs{x-z}\leq 4(\abs{x}+\abs{z})\\
    &\leq 4(\sqrt{1^2+1^2})
    \sqrt{\abs{x}^2+\abs{z}^2}=4\sqrt{2}(\sqrt{\abs{x}^2+\abs{z}^2})\\
    &\leq 4\sqrt{2}
  \end{align*}
...
\item Sei $A=(a_{ij})_{i,j=1}^n$ eine reelle symmetrische $n\times n$-Matrix
  ...
  der Nebenbedingung $\norm{x}_2=1, x=\begin{pmatrix}x_1\\\vdots\\x_n
...
\item Man bestimme die Extrema der Funktion $f(x)=\sum_{i=1}^n a_ix_i$
  ...
  $N$ ist \highl{nicht} kompakt, da $N$ nicht beschränkt ist.\\
...
\begin{definition}
  Unter einer \highl{Kurve} im $\R^n$ versteht man eine stetige
...
\begin{definition}
  ...
  \highl{Tangentialvektor} der Kurve $f$ zum
...
\begin{remark}
  Eine Kurve $f\colon I\rightarrow\R^n$ braucht nicht injektiv zu
  sein.  Gilt $f(t_1)=f(t_2)=x$ für $t_1\neq t_2$, so heißt $x$
  \highl{Doppelpunkt} der Kurve $f$. In $x$ hat dann $f$ zwei im
  Allgemeinen verschiedene Tangentialvektoren.
\end{remark}
...
\begin{definition}
  ...
  Kurve heißt \highl{regulär}, falls $f'(t)\neq 0$ für alle
  $t\in I$. Ein Parameter $t\in I$ mit $f'(t)=0$ heißt \highl{singulär}.
\end{definition}

\begin{beispiel}
  $f\colon \R\rightarrow\R^2, f(t)=\begin{pmatrix}t^2\\t^3
...
\begin{definition}
  ...
  \highl{Schnittwinkel} $\vartheta$ der Kurven $f$ und
  ...
  \[\cos \vartheta = \frac{\langle f'(t_1),g'(t_2)\rangle}{\norm{f'(t_1)}_2\norm{g'(t_2)}_2}\quad 0\leq\vartheta\leq\pi\]
\end{definition}
...
\highl{Tangentialebene} von $f$ im Punkt 
...
\begin{enumerate}[(1)]
 ...
 \end{gather*}
    ...
    &=& \left\{\begin{pmatrix}\xi\\f(\xi)\end{pmatrix}+\sum_{i=1}^n h_i g_i'(\xi)\colon  h=\begin{pmatrix}h_1\\\vdots\\h_n\end{pmatrix}\in\R^n\right\}\\
      &=& \left\{\begin{pmatrix}
        \xi+h\\
        f(\xi)+\sum_{i=1}^n h_i \diffp{f}{{x_1}}(\xi)
...
\begin{definition}
  ...
  $\{x\in D |
  f(x)=c\}$ \highl{Niveaufläche} der Funktion $f$
  zum Niveau $c$.
\end{definition}
...
\begin{remark}
Man kommt von einer Niveaufläche $f(x)=c_1$ zu einer 
...
Es gilt: 
\[\max\{\abs{f'(x)h} | \norm{h}_2\leq 1\}=\norm{f'(x)}_2\]
für $h=\frac{(f'(x)^T}{\norm{f'(x)}_2}$ Richtung des Gradienten.  
\end{remark}
...
\begin{theorem}
  ...
  In Komponenten:
  \[\sum_{i=1}^n (x-\xi)\diffp{f}{{x_i}}(\xi_1,\dots,\xi_n)=0\]
  \begin{proof}
    Sei o.\,B.\,d.\,A. $\diffp{f}{{x_n}}(\xi)\neq
  ...
  -\tilde{\xi})=\xi_n-\diffp{f}{{x_n}}(\xi)^{-1}
  \diffp{f}{{\tilde{x}}}(\xi)(\tilde{y}-\tilde{\xi})
  \Leftrightarrow \diffp{f}{{x_n}}(\xi)(y_n-\xi_n)=
  \underbrace{-\diffp{f}{{x_n}}(\xi)(\tilde{y}-\tilde{\xi})}_{=\sum_{i=1}^n \diffp{f}{{x_i}}(\xi)(y-\xi_i)}
  \Rightarrow \sum_{i=1}^n \diffp{f}{{x_i}} (\xi)(y_i- \xi_i)=0$
  \end{proof}
\end{theorem}
...
\begin{definition}
  ...
  Für $n=1$ heißt \autoref{def:Diffgl} eine \highl[Differentialgleichung!einfache]{einfache Differentialgleichung} 1.\,Ordnung.

  Unter einer Lösung von \autoref{def:Diffgl} versteht man eine auf einem
...
\begin{enumerate}
  ...
  \item Treten in \autoref{def:Diffgl} neben $x$ weitere unabhängige 
    Variable auf sowie partielle Ableitungen, so spricht man von
    \highl[Differentialgleichungen!partielle]{partiellen Differentialgleichungen}.\\
    Beispiel: Wärmeleitungsgleichungen:\\
    $\diffp{u}{t}=\diffp[2]{u}{x}, u= u(x,t)$\\
    Korteweg-deVries-Gleichungen:\\
    $\diffp{u}{t}=\diffp[3]{u}{x}+ 6u\diffp{u}{x}$
\end{enumerate}  
\end{remark}

\paragraph{Anfangswertproblem}
Folgende Aufgabe heißt Anfangswertproblem für ein DGL-System 1.\,Ordnung:\\
Sei $(x_0,y_0)\in G$.
Gesucht ist eine Lösung $y=\varphi(x)$ von $y'=f(x,y)$ mit $\varphi(x_0)
=y_0$.

\begin{beispiel}
  $y'=f(x), f\colon I\rightarrow\R^n$ stetig, $x_0\in I, \varphi(x_0)
=y_0$\\
...
ein \highl{Richtungsfeld}: 
\[\{(x,y,y')\colon (x,y)\in G, y'=f(x,y)\}\]
$(x,y,y')$ heißt \highl{Linienelement}. In jedem
...
\begin{definition}
  Kurven konstanten Anstiegs heissen \highl{Isoklinen}.
\end{definition}


\begin{beispiel}
  $y'=f(x,y)=x, G\subset\R\times\R$\\
...
heißt \highl[Lösung!allgemeine]{allgemeine Lösung} von $y'=f(x,y)$.

Ein $\varphi\in L$ heißt \highl[Lösung!spezielle]{spezielle Lösung}
von $y'=f(x,y)$.

Die allgemeine Lösung kann oft durch Parameter beschrieben werden.

\section{Reduktion von Differentialgleichungssystemen höherer Ordnung
  auf Differentialgleichungssysteme 1. Ordnung}
\begin{definition}
...
Für $k=1$ erhält man \autoref{def:Diffgl} und für $n=1$ heißt
die \autoref{def:DiffglHoch} einfach Differentialgleichung $k$-ter
Ordnung.

Unter einer Lösung von \autoref{def:DiffglHoch} versteht man eine Funktion
...
\end{pmatrix}$, d.\,h. wegen \autoref{def:DiffglHoch} $\Leftrightarrow
y_i^{(k)}=f_i(x,y_1,\dots,y_n,y_1',\dots,y_n',\dots,y^{(k-1)}_1,\dots,
y^{(k-1)}_n)$

\paragraph{Reduktion von \autoref{def:DiffglHoch} auf DGL-Systeme 1.\,Ordnung}
...
Dann gilt:
\begin{enumerate}[a)] 
  \item $\varphi$ ist eine Lösung von \autoref{def:DiffglHoch} 
...
\begin{gather}
  \label{def:DiffglEin}
   Y'=F(x,Y)
\end{gather}
Denn: Sei $\varphi\colon I\rightarrow\R^n$ eine Lösung von \autoref{def:DiffglHoch},
...
\end{pmatrix}\\
  ...
  & = & F(x,\Phi(x))
\end{align*}
  \item $\Phi$ ist eine Lösung von \autoref{def:DiffglEin}. Dann ist $\varphi
    \coloneqq \varphi_0$ eine Lösung von \autoref{def:DiffglEin}.\\
...
Fazit: Die Lösungen \autoref{def:DiffglHoch} und \autoref{def:DiffglEin} sind
...
$\varphi(x)=C\varphi_0(x)+\varphi_{\text{inh}}(x),
\varphi_0(x)=e^{\int_{x_0}^x a(t)dt}$
\begin{proof}
  Sei $\psi(x)$ eine Lösung des inhomogenen DGL $y'=a(x)y+b(x)$.
...
\begin{beispiel}
Gesucht ist die allgemeine Lösung von $y'=ay+1$.
...
\subsection{Beispiele für Differentialgleichungen, die auf \texorpdfstring{$y'=f(x)g(y)$}{y'=f(x)g(y)}
...
\begin{enumerate}[(a)]
  \item Die inhomogene DGL $y'=f(\frac{y}{x})$\\
    ...
    \end{itemize}
    \end{proof}
  \item Die Differentialgleichung $y''=f(y), f\colon J\rightarrow\R$
    stetig.
    \begin{proof}
    ...
    \diff{}{x} ( (\varphi'(x)^2))=\diff{}{x}\int_{x_0}^x f(\varphi(z))
    \varphi'(z)dz=\diff{}{x}\int_{\varphi(x_0)}^{\varphi(x)} f(t)dt$\\
    Physiker setzen dann: $U(y)\coloneqq -\int_{y_0}^y f(t)dt\Leftrightarrow
    \diff{}{x} (\frac{1}{2}(\varphi'(x))^2+U(\varphi(x)))=0
...
\end{gather}
\highl[Differentialgleichungssystem!homogenes lineares]{homogenes lineares Differentialgleichungssystem} 1.\,Ordnung mit konstanten 
...
\autoref{def:DGLFunkt} wie folgt:
\[\begin{matrix}
...
\begin{remark}
Die Funktion $f\colon \R\times\R^n\rightarrow\R^n, f(x,y)\coloneqq Ay$
...
\begin{beispiel}
  Sei $A\in\LL(\K^n,\K^n), e^A\coloneqq \sum_{k=0}^\infty \frac{A^k}{k!}$
ist für jede Matrix $A\in\LL(\K^n,\K^n)$ konvergent. Denn $a_n=\frac{A^n}{n!}$. Die Norm ist $\norm{a_n}=\frac{\norm{A^n}}{n!}\leq \frac{\norm{A}^n}{n!}
=b_n$.
\[\lim\frac{b_{n+1}}{b_n}=\lim\frac{\norm{A}^{n+1}}{(n+1)!}\cdot\frac{n!}{\norm{A}^n}
=\lim \frac{\norm{A}}{n+1}=0\]
\end{beispiel}
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    ...
    \item Die allgemeine Lösung der inhomogenen DGL $y'=Ay+b(x)$ ist
      ...
      \begin{proof}
        Variation der Konstanten: Sei $\varphi(x)$ Lösung des
...
\begin{remark}
  $\Phi(x)=e^{Ax}, \Phi\colon \R\rightarrow\LL(\K^n,\K^n), \Phi'(x)
=Ae^{Ax}=e^{Ax}A$\\
...
\begin{definition}
  ...
  ein \highl{Fundamentalsystem} von Lösungen von
  ...
  \begin{enumerate}[(1)]
    \item Jedes Fundamentalsystem $\Phi$ hat die Gestalt $\varphi(x)=
      ...
      \begin{proof}
        $\Phi(x)=e^{Ax}S, \Phi'(x)=Ae^{Ax}S=A\Phi(x)$\\
    ...
    \item Die allgemeine Lösung von $y'=Ay$ ist $\varphi(x)=\Phi(x)c, c
      \in\K^n$.
      \begin{proof}
        $\varphi(x)=\Phi(x)c=e^{Ax}Sc$ ist Lösung von $y'=Ay$. Zu
...
Es gilt:
\begin{theorem}
  Die Matrix $\MM_n(\C)$ besitzt die Eigenwerte $\mu_1,\ldots,\mu_p,
  \lambda_1,\ldots,\lambda_q$ und das System $\{v_{10}, v_{20}, \ldots,
  v_{p0}, v_{11}, \ldots, v_{r_11}, \ldots, v_{12}, \ldots, v_{r_22},
  \ldots, v_{1q},\ldots,v_{r_qq}\}, p+r_1+\cdots+r_q=n$ von Eigenvektoren
...
\begin{remark}
  Sei $A\in\MM_n(\K)$ und sei $S\in\MM_n(\K)$ invertierbar.  Dann gilt
  für $J\coloneqq S^{-1}AS$ die Formel $e^{Ax}S=Se^{Jx}$. Denn $J^2=
  (S^{-1}AS)^2=S^{-1}ASS^{-1}AS=S^{-1}A^2S$ und somit ist $e^{Jx}=
  \sum_{k=0}^\infty \frac{J^kx^k}{k!}=\sum_{k=0}^\infty
  \frac{S^{-1}ASx^k}{k!}  =S^{-1}(\sum_{k=0}^\infty
  \frac{A^kx^k}{k!})S=S^{-1}e^{Ax}S\Leftrightarrow e^{Ax}S=Se^{Jx}$.
\end{remark}
...
\begin{definition}
  ...
  \begin{enumerate}[(a)]
    \item $f$ genüge in $G$ einer \highl{Lipschitzbedingung}
      \[\exists L\geq 0\,\forall(x,y),(x,\tilde{y})\in G\colon  \norm{f(x,y)-f(x,\tilde{y})}\leq L\norm{y-\tilde{y}}\]
      (Genauer: $f=f(x,y)$ genüge in $G$ bzgl. $y$ einer Lipschitzbedingung)
    \item $f$ genüge in $G$ \highl{lokal} einer Lipschitzbedingung
...
y=\begin{pmatrix}y_1\\\vdots\\y_n\end{pmatrix}$ \highl{stetig} partiell
differenzierbar. Dann genügt $f$ in $G$ lokal einer
Lipschitzbedingung.
\begin{proof}
  $(a,b)\in G\colon \exists r>0$, so dass $U\coloneqq \{(x,y)\in\R\times\R^n\colon 
\norm{x-a}\leq r, \norm{y-b}\leq r\}\subset G$. $U$ ist in $\R\times\R^n$
abgeschlossen und beschränkt und somit also kompakt. Somit ist wegen
Stetigkeit von $\diffp{f}{y}=\begin{pmatrix}
  \diffp{f_1}{{y_1}} & \dots & \diffp{f_1}{{y_n}}\\
  \vdots & & \vdots\\
  \diffp{f_n}{{y_1}} & \dots & \diffp{f_n}{{y_n}}
\end{pmatrix}$ auf $U$: $L\coloneqq \sup\{\norm{\diffp{f}{y}
(x,y)}\colon (x,y)\in U\}<\infty$.\\
Nach dem Mittelwertsatz %Satz II.3.2
gilt:
\[f(x,y)-f(x,\tilde{y})=\left(\int_0^1 \diffp{f}{y}
(x,y+t(\tilde{y}-y))dt\right)(\tilde{y}-y)\]
Daher folgt: 
\begin{align*}
  \norm{f(x,y)-f(x,\tilde{y})}_2 &= \norm{\int_0^1 \diffp{f}{y}
    (x,y+t(\tilde{y}-y))\,dt(\tilde{y}-y)}\\
  &\leq \int_0^1 \norm{\diffp{f}{y}
    (x,y+t(\tilde{y}-y))(\tilde{y}-y)}_2\,dt\\
  &\leq \int_0^1 \norm{\diffp{f}{y}
    (x,y+t(\tilde{y}-y))}\norm{\tilde{y}-y}_2\,dt\\
  &\leq \int_0^1\underbrace{\norm{\diffp{f}{y}
      (x,y+t(\tilde{y}-y))}_{\leq L\,\forall (x,z)\in U}}\,dt
  \norm{\tilde{y}-y}_2\\
  &\leq L\int_0^1 1\,dt\norm{\tilde{y}-y}_2=L\norm{\tilde{y}-y}_2
\end{align*}
Da auf endlichdimensionalen Räumen die Normen äquivalent sind, erhalten
wir unter der Ausgangsnorm $\norm{\cdot}$: $\norm{f(x,y)-f(x,\tilde{y}}\leq
\tilde{L}\norm{\tilde{y}-y}$.
\end{proof}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item Raum der stetigen Funktionen\\
      ...
      \norm{\varphi}\coloneqq \sup_{x\in I}\norm{\varphi(x)}_2$. Der Raum $[\CC(I,\R^n),
      \norm{\cdot}]$ ist ein vollständiger normierter Raum. Sei $I_\varepsilon
      \coloneqq [a-\varepsilon,a+\varepsilon], \varepsilon>0$. Wählen $\varepsilon,r>0$
      so, dass $U=\{(x,y)\in I_\varepsilon\times\R^n\colon  \norm{y-c}_2\leq r\}
      \subset G$ und $f$ auf $U$ einer Lipschitzbedingung genügt, d.\,h.
      $\exists L\geq 0\,\forall (x,y),(x,\tilde{y})\in U\colon \norm{f(x,y)-f(x,
      \tilde{y})}_2\leq \norm{y-\tilde{y}}_2$. $U$ ist kompakt und $f$
      stetig auf $U$. Man setzt $B_r^\varepsilon(c)\coloneqq \{\varphi\in\CC(
      I_\varepsilon,\R^n)\colon \norm{\varphi-c}\leq r\}$ (ist i.\,A. nicht kompakt).
    ...
    \item Es gilt die Abschätzung
      \begin{align*}
    \norm{(T\varphi)(x)-c}_2&=\norm{\int_a^x f(t,\varphi(t))\,dt}_2\\
    &\leq \abs{\int_a^x \norm{f(t,\varphi(t))}_2\,dt}\\
    & \leq \abs{x-a}M\leq \varepsilon M\quad \forall x\in I_\varepsilon
      \end{align*}
      Hieraus folgt: 
      \[\boxed{\forall \varphi\in B_r^\varepsilon(c)\colon \norm{T\varphi-
      c}=\sup_{x\in I_\varepsilon}\norm{(T\varphi)(x)-c}_2\leq \varepsilon M}\]
      \item
    \begin{align*}
      \norm{(T\varphi)(x)-(T\tilde{\varphi})(x)}_2&= \norm{\int_a^x
      (f(t,\varphi(t))-f(t,\tilde{\varphi}(t)))\,dt}_2\\
      &\leq \abs{\int_a^x \underbrace{\norm{f(t,\varphi(t))-f(t,\tilde{\varphi}(t)))}_2}_{L\norm{\varphi-\tilde{\varphi}}}
      dt}\\
      &\leq L\abs{x-a} \norm{\varphi-\tilde{\varphi}}\\
      &\leq \varepsilon L \norm{\varphi-\tilde{\varphi}}\quad x\in I_\varepsilon
    \end{align*}
    \[\Rightarrow\boxed{\forall \varphi,\tilde{\varphi}\in
    B_r^\varepsilon(c)\colon \norm{T\varphi-T\tilde{\varphi}}\leq\varepsilon L
    \norm{\varphi-\tilde{\varphi}}}\]
    \end{enumerate}
    ...
    \norm{T\varphi-T\tilde{\varphi}}\leq \frac{1}{2}\norm{\varphi-\tilde{\varphi}}$
    \end{enumerate}
...
\begin{beispiel}[Beispiele für Anfangswertprobleme]
  \begin{enumerate}[(a)]
  \item $y'=\sqrt[3]{y^2}, y=\varphi(a)=c, f=f(x,y)=\sqrt[3]{y^2}, f\colon 
    \R\times\R\rightarrow\R$
    \begin{enumerate}[1.\,F{a}ll]
      \item $a\in\R,c\neq 0\colon \varphi(a)=c$\\
    $\diffp{f}{y}(x,y)=\frac{2}{3}y^{\frac{2}{3}-1}
...
\begin{beispiel}
  $y'=y, y=\varphi(0)=c$ Die Lösung ist $\varphi(x)=ce^x$. Mit
Hilfe des Iterationsverfahrens wird ebenfalls die Lösung ermittelt:
$f(x,y)=y, \abs{f(x,y)-f(x,\tilde{y})}=\abs{y-\tilde{y}}$\\
...
\begin{theorem}
  ...
  \varphi\colon I\rightarrow\K^n\colon  y'=A(x)y+b(x),
  \varphi(x_0)=c\]
  \begin{proof}
    Setzen $f(x,y)=A(x)y+b(x)$. Man zeigt, dass für jedes
  kompakte Intervall $J\subset I$ ein $L\geq 0$ existiert mit
  $\forall x\in J\,\forall y,\tilde{y}\in\K^n\colon  \norm{f(x,y)-f(x,\tilde{y})}\leq
  L\norm{y-\tilde{y}}$\\
  ...
  \sup_{x\in J} \abs{a_{ij}(x)}<\infty$. Für $y=\begin{pmatrix}y_1\\\vdots\\y_n
  \end{pmatrix}, \tilde{y}=\begin{pmatrix}\tilde{y_1}\\\vdots\\\tilde{y_n}
  \end{pmatrix}$ gilt:
  \begin{align*}
   \norm{f(x,y)-f(x,\tilde{y})}_1&=& \norm{A(x)y-A(x)\tilde{y}}_1
  =\sum_{i=1}^n\abs{\sum_{j=1}^n a_{ij}(x)(y_j-\tilde{y_j})}\\
  & \leq& \sum_{i=1}^n\sum_{j=1}^n \abs{a_{ij}(x)} \abs{y_j-\tilde{y_j}}\leq nM_J
  \sum_{i=1}^n \abs{y_j-\tilde{y_j}}\\
  &=& nM_j\norm{y-\tilde{y}}_1
  \end{align*}
    Da die $\norm{\cdot}_1$ äquivalent zur Ausgangsnorm $\norm{\cdot}$ ist, 
  erhält man mit einer weiteren Konstanten $\alpha\geq 0$ die 
  Abschätzung $\norm{f(x,y)-f(x,\tilde{y})}\leq\alpha nM_J\norm{y-\tilde{y}}$.
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    \item $\varphi_1,\varphi_2\in\LL_h\Rightarrow c_1\varphi_1+c_2\varphi_2
      \LL_h, c_1,c_2\in \K$.
      \begin{proof}
        $\varphi_1,\varphi_2\in\LL_h$, d.\,h. $\varphi_i(x)=A(x)
      ...
      \end{proof}
    \item $\varphi\in\LL_h, \psi\in\LL_{inh}\Rightarrow
      \varphi+\psi\in\LL_{inh}$
      \begin{proof}
        $\varphi\in\LL_h, \psi\in\LL_{inh}\Rightarrow\varphi'(x)=
    ...
    \item $\psi_1,\psi_2\in\LL_{inh}\Rightarrow\psi_1-\psi_2\in\LL_h$
      \begin{proof}
        $\psi_1,\psi_2\in\LL_{inh}, \psi_i'(x)=A(x)\psi_i(x)+b(x),
...
\begin{definition}
  \begin{enumerate}[(i)]
    \item $k$ stetige Funktionen $\varphi_1,\dots,\varphi_k\in\CC(I,\K^n)$
      heissen \highl{linear unabhängig}, g.\,d.\,w.:
      \[\forall \alpha_1,\dots,\alpha_k\in\K (\sum_{i=1}^k \alpha_i\varphi_i
      =0\Rightarrow \alpha_1=\cdots=\alpha_k=0\]
    \item $k$ stetige Funktionen $\varphi_1,\dots,\varphi_k\in\CC(I,\K^n)$
      heissen \highl{linear abhängig}, g.\,d.\,w.:
      \[\exists\alpha_1,\dots,\alpha_k\in\K\colon \sum_{i=1}^k \alpha_i\varphi_i
      =0\wedge\sum_{i=1}^k\abs{\alpha_i}>0\]
  \end{enumerate}
\end{definition}

\begin{remark}
  $\sum_{i=1}^k \alpha_i\varphi_i=0\colon \forall x \in I\colon
  \sum_{i=1}^k \alpha_i\varphi_i(x)=0$
\end{remark}
...
\begin{theorem}
  ...
  \begin{enumerate}[(a)]
    ...
    \item $\forall x\in I\colon  \varphi_1(x),\dots,\varphi_k(x)\in\K^n$
      ...
      \end{enumerate}
      \begin{proof}
        Der Beweis von (iii) nach (ii) nach (i) ist 
      ...
      \alpha_k\in\K\colon \sum_{i=1}^k \abs{\alpha_i}>0\wedge \sum_{i=1}^k \alpha_i
    ...
    \item $\dim \LL_h=n$
      \begin{proof}
        Man zeigt zuerst, dass die Dimension größer oder
...
\begin{remark}
  Sind $\varphi_1,\dots,\varphi_k$ keine Lösungen des homogenen DGL
  $y'=A(x)y$, d.\,h. $\varphi_1,\dots,\varphi_k\notin\LL_k$, so ist
  \autoref{satz:linAbh} i.\,A. falsch.
\end{remark}
...
\begin{definition}
  ...
  \end{pmatrix}$ \highl{Fundamentalmatrix} von
  $y'=A(x)y$.
\end{definition}

\begin{remark}
\begin{enumerate}[(a)]
  \item Nach \autoref{satz:linAbh} gilt: $\varphi_1,\dots,\varphi_n$
...
\subsection{Die inhomogene Differentialgleichung \texorpdfstring{$y'=A(x)y+b(x)$}{y'=A(x)y+b(x)}}
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item Zu zeigen ist $\LL_{inh}\subset\psi_0+\LL_h$. Sei $\psi\in\LL_{inh}$.
      Daraus folgt nach \autoref{satz:LTheorie} $\psi-\psi_0\in\LL_h
...
\highl[Differentialgleichung!homogene]{homogene lineare Differentialgleichung} $n$-ter Ordnung\index{Differentialgleichung!lineare},
falls $f=0$. Ansonsten heißt es inhomogen. $L$ heißt
\highl[Differentialausdruck!linearer]{linearer Differentialausdruck},
...
\begin{theorem}
  \begin{enumerate}[(i)]
    ...
    \item $n$-Lösungen $\varphi_1,\dots,\varphi_n\in\LL_h$ der homogenen
      ...
      W(x)\footnote{heißt Wronskideterminante}\coloneqq \det\begin{pmatrix}
  ...
  \end{enumerate}
  \begin{proof}
    Die DGL $L(y)=f(x)$ ist äquivalent zu dem inhomogenen
...
\begin{definition}
  ...
  $L(y)=0$ heißt \highl{Lösungsfundamentalsystem}.
...
\subsection{Reduktion der Ordnung einer Differentialgleichung \texorpdfstring{$L(y)=0$}{L(y)=0} bei Kenntnis einer Lösung}
...
\begin{gather}
  ...
  y''+a(x)y'+b(x)=0
\end{gather}
Sei $\varphi\neq 0$ eine Lösung von \autoref{eq:Red}.\\
Einsetzen in DGL:\\
...
\autoref{eq:Redv}, so ist $\psi(x)=\varphi(x)u(x)$, wobei
...
\begin{proof}
  ...
  \item Man zeigt, dass $\varphi_{km}(x)=x^me^{\lambda_kx}$ 
    ...
    $L(x^me^{\lambda x})=L\left(\diffp[m]{}{\lambda}
      (e^{\lambda x})\right)$\\
    Vertauschung der Differentiation (nach Satz von Schwartz):
    $=\diffp[m]{}{\lambda}L(e^{\lambda x})= \diffp[m]{}{\lambda}
    (P_L(\lambda)e^{\lambda x})$.  Ist $\lambda=\lambda_j$ eine
    $n_j$-fache Nullstelle von
    $P_L, P_L(\lambda)=\prod_{k=1}^s (\lambda-\lambda_k)^{n_k}$, dann
    gilt $\diffp[m]{}{\lambda}(P_L(\lambda)e^{\lambda
      x})= 0$ für $m=0,1,\dots,n_j-1$. Denn $P_L(\lambda)=Q_j(\lambda)
    (\lambda-\lambda_j)^{n_j}, Q_j(\lambda)=\prod_{k=1,k\neq j}^s
    (\lambda-\lambda_j)^{n_k}$.\\
    Leibnitzsche Regel:
    \begin{align*}
      \diffp[m]{}{\lambda} (P_L(\lambda)e^{\lambda x}) &= \sum_{k=0}^m
      \binom{m}{k} \diffp[k]{}{\lambda} (P_L(\lambda))
      \diffp[m-k]{}{\lambda} (e^{\lambda k}),
      \diffp[m]{}{\lambda}(P_L(\lambda))\\
      &= \diffp[m]{}{\lambda}( (\lambda-\lambda_j)^{n_j} Q_j(\lambda))\\
      &=\sum_{k=0}^m \diffp[k]{}{\lambda} ((\lambda-\lambda_j)^{n_j}
      \diffp[m-k]{}{\lambda} (Q_j(\lambda)), \diffp[k]{}{\lambda}
      (\lambda-\lambda_j)^{n_j}\\
      &=n_j(n_j-1)\ldots (n_j-k+1)(\lambda-\lambda_j)^{n_j-k}\\
      &\Rightarrow \diffp[k]{}{\lambda} ((\lambda-\lambda_j)^{n_j})\\
      &=0
    \end{align*}
    Somit ist $\diffp[m]{}{\lambda}(P_L(\lambda)) =0$
    für $m=0,1,\dots,n_j-1$ und $\diffp[m]{}{\lambda}
  ...
  \item Die $\varphi_{km}(x)=x^me^{\lambda_k x}, m=0,1,\dots,n_k-1,n=
    ...
    \begin{lemma}
      ...
      \begin{align*}
        0&=\diffp[m]{}{x} \left(\sum_{j=1}^{r-1} P_j(x)
          e^{(\lambda_j-\lambda_r)x} + P_r(x) \right)\\
        &= \diffp[m]{}{x} \left(\sum_{j=1}^{r-1} P_j(x)
          e^{(\lambda_j-\lambda_r)x}\right)\\
        &=\sum_{j=1}^{r-1} \diffp[m]{}{x} (P_j(x)
...
\begin{theorem}
  %Satz IV.3.1
  Sei $G\subset\R^n$ ein \highl{Gebiet} und $f\colon G\rightarrow\R^n$
  ...
  \begin{proof}
    Aus $f=\text{ grad} \Phi_0\Rightarrow\text{ grad}
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \item["`$\Rightarrow$"'] Sei $f=\text{ grad}\Phi=\Phi'$. Seien 
      ...
      \diff{}{t} (\Phi(\gamma(t)))dt=\Phi(\gamma(b))-\Phi(\gamma(a))=
      \Phi(A)-\Phi(B)$.\\
    ...
    \item["`$\Leftarrow$"'] Sei $\xi\in G$ und $B_\varepsilon(\xi)\subset
      ...
      f(\xi)hdt=f(\xi)h$. Somit hat man $\abs{\Phi(\xi+h)-\Phi(\xi)-f(\xi)h}
      =\abs{\int_\sigma f(y)-f(\xi))dy}\leq \max_{y\in \{\xi+th\colon 0\leq t\leq 1}
      \norm{f(y)-f(\xi)}_2 L(\sigma)$. Da $f$ stetig ist, folgt $\max_{y\in \{\xi+th\colon 0\leq t\leq 1}
      \norm{f(y)-f(\xi)}_2\xrightarrow{h\rightarrow 0} 0$. Damit gilt
...
\begin{Folg}
  Es gilt $\int_\gamma \text{grad} \Phi dx=\Phi(B)-\Phi(A),
...
\begin{enumerate}[(i)]
  \item Notwendiges Kriterium\\
    ...
    Wegen dem Schwatzschen Satz folgt nun, $\diffp{f_j}{{x_k}}
    =\diffp{\Phi}{{x_k} {x_j}}=\diffp{\Phi}{{x_j} {x_k}}
    =\diffp{f_k}{{x_j}}$. Dann ist $f$ ein Gradientenfeld.
  \item Hinreichende Bedingungen\\
    Sei $G\subset\R^n$ eine offene und \highl[Menge!sternförmig]{sternförmige Menge},
    d.\,h. $\exists a \in G\,\forall x \in G\,\forall t\in[0,1]\colon a+tx\in G$,
    und $f=
    \begin{pmatrix}
      f_1\\\cdots\\f_n
    \end{pmatrix}\colon G\rightarrow\R^n$ ein stetig differenzierbares
    Vektorfeld mit $\diffp{f_j}{{x_k}}=\diffp{f_k}{{x_j}},
    ...
    \begin{align*}
      \Phi(x) &= \int_0^1 f(tx)x\,dt = \int_0^1 \sum_{j=1}^n f_j(tx)x_j\,dt\\
      \diffp{\Phi(x)}{{x_k}} &= \int_0^1 \diffp{}{{x_k}} (f(tx)x)\,dt\\
      &= \int_0^1 \diffp{}{{x_k}} \left(\sum_{j=1}^n f_j(tx)
        x_j\right)\,dt \sum_{j=1}^n \diffp{}{{x_k}} (f_j(tx)x_j)\,dt\\
      &= \sum_{j=1}^n \left(\diffp{f_j(tx)}{{x_k}} x_j + f_j(tx)
        \diffp{x_j}{{x_k}}\right)\\
      &=f_k(tx)+ \sum_{j=1}^n \diffp{f_j(tx)}{{x_k}} x_j= f_k(tx)+
      \sum_{j=1}^n \diffp{f_j(tx)}{{u_k}} \diffp{u_k}{{x_k}} x_j\\
      &= f_k(tx)+\sum_{j=1}^n \diffp{f_j(tx)}{{u_k}}tx_j =
      f_k(tx)+\sum_{j=1}^n \diffp{f_k(tx)}{{u_j}}tx_j\\
      &= \diff{}{t}(tf_k(tx))\\
      &\text{Somit ist } \diffp{\Phi(x)}{{x_k}} =\int_0^1 \diff{}{t}
      (ff_k(tx))dt=f_k(x)
  \end{align*}
\end{enumerate}
\end{Folg}

\subsection{Praktische Bestimmung von \texorpdfstring{$\Phi$}{Phi}}
Sei o.\,B.\,d.\,A. $f=\text{ grad }\Phi, \Phi=\Phi(x_1,x_2), f_1=
\diffp{\Phi}{x}, f_2=\diffp{\Phi}{{x_2}}$. 
\begin{enumerate}
