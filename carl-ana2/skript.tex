...
\begin{definition}
  ...
  \begin{enumerate}[(N1)]
    ...
    \item $||\alpha x||=|\alpha|\cdot ||x||$ \highl{Homogenität}
    \item $||x+y||\leq ||x||+||y||$ Dreiecksungleichung
  \end{enumerate}
  Das Paar $[E,||\cdot ||]$ heißt \highl[Raum!normierter]{normierter
    Raum}.
\end{definition}
...
\begin{beispiel}
  ...
  \item $[\R^n,||\cdot||_p], x=(\xi_1,\dots,\xi_n),||x||_p=
    ...
    \item $||\cdot||_2= \sqrt{\sum_{i=1}^n |\xi_i|^2}$ heißt
      ...
      \item $||\cdot||_\infty$ heißt \highl{Maximumnorm}.
    \end{itemize}
  \item
    \begin{enumerate}[(a)]
...
\begin{proof}: Vollständigkeit von $[C_b(X),||\cdot||_\infty]$:\\
  ...
  \item Beschränkheit von $f$:\\
    ...
    X\,\forall n\geq n_\varepsilon \colon  |f_n(x)-f(x)|\leq\varepsilon
...
\begin{remark}
  Konvergenz bezüglich $||\cdot||_\infty$ heißt \highl[Konvergenz!gleichmässige]{gleichmässige
  Konvergenz} von
  ...
  n\geq n_\varepsilon\colon |f_n(x)-f(x)|\leq\varepsilon\]
...
\begin{theorem}[\highl[Bolzano/Weierstrass!Satz von]{Satz von
  Bolzano/Weierstrass}]
  \label{satz:bw}
...
\begin{proof}
  ...
  $S$ in $[\R^n,||\cdot||_\infty]$ kompakt nach \autoref{satz:bw}. Weil
...
\begin{proof}
  ...
  \autoref{satz:VRNorm} existieren $c, C>0\colon c||x||_0\leq ||x||\leq C||x||_0$.
  \begin{enumerate}[(i)]
...
\begin{theorem}
  ...
  $\forall x\in J\,\forall y,\tilde{y}\in\K^n\colon  ||f(x,y)-f(x,\tilde{y})||\leq
  L||y-\tilde{y}||$\\
