...
\begin{beispiel}
  ...
  \item Normierter Raum der $\RR$-integrierbaren Funktionen:
    $[\RR[a,b],\norm{\cdot}_\infty], \RR[a,b]=\{f\colon [a,b]
...
\begin{theorem}
  ...
  $B(\{1,\dots,n\})=\R^n$.
\end{theorem}
\begin{proof}: Vollständigkeit von $[C_b(X),\norm{\cdot}_\infty]$:\\
...
\begin{remark}
  Konvergenz bezüglich $\norm{\cdot}_\infty$ heißt \highl[Konvergenz!gleichmässige]{gleichmässige
  Konvergenz} von
...
\begin{proof}
  ...
  folgende Funktion: $f\colon [\R^n,\norm{\cdot}_\infty]\rightarrow\R,
  f(\alpha_1, \dots,\alpha_n)\coloneqq \norm{\sum_{i=1}^n \alpha_i x_i}$. Es
  gilt, dass $f$  stetig ist.\\
  Denn $\abs{f(\alpha_1,\dots,\alpha_n)-f(\beta_1,\dots,\beta_n)}
  =\norm{\sum_1^n (\alpha_i-\beta_i)x_i}\leq \sum_1^n \abs{\alpha_i-\beta_i}
  \cdot \norm{x_i}\leq \max_{1\leq i \leq n} \norm{x_i} \cdot\sum_1^n
  \abs{\alpha_i-\beta_i}\leq n\cdot\underbrace{\max_{1\leq i\leq n}
    \abs{\alpha_i-\beta_i}}_{=\norm{(\alpha1,\dots,\alpha_n)-(\beta_1-\beta_n)}_\infty}$. Daraus folgt, dass $f$ stetig ist.\\
...
\begin{theorem}
  ...
  \rightarrow[\R^m,\norm{\cdot}_2]$
  \begin{proof}
    $f\colon \begin{pmatrix}f_1\\\vdots\\f_m
...
\begin{remark}
  ...
  Ableitungen 2.\,Ordnung in einer Umgebung von $(\xi,\eta)$ und die
...
\begin{theorem}
  ...
  \begin{enumerate}[(i)]
    ...
    \item Ist $f$ injektiv, so ist $f^{-1}\colon f(D)\rightarrow\R$ stetig
      differenzierbar.
      \begin{proof}
        $f(D)$ ist nach (i) offen. Nach dem Umkehrsatz und da $f$
