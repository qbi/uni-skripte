...
%%%%%%%%%%%%%%%%%%%
\begin{defi}{defi:8.1}
Ein (linearer) Code der Länge $n$ ist ein Untervektorraum $C \subseteq K^n$. Man nennt $k loneqq \dim C$ die Dimension von $C$. Außerdem heißt
\[\delta loneqq \min{\left\{d(x, y) \colon x, y \in C, x \neq y\right\}} = \min{\left\{w(z) \colon 0 \neq z \in C\right\}}\]
...
Viele \randbem{Vorteil dieser Konstruktion, zu Codes als Untervektorraum: man muss nur $\phi$ und $n$ speichern, keine Basis eines Untervektorraumes mit $k$ Elementen} in der Praxis auftretende Codes werden mit Hilfe von Polynomringen konstruiert. Dabei identifiziert man $K^n$ mit $K[X] / (X^n - 1)$ durch \[(a_0, a_1, \ldots, a_{n-1}) \leftrightarrow a_0 + a_1X + a_2X^2 + \ldots + a_{n-1}X^{n-1} + (X^n - 1)\]
Für jeden Teiler $\phi$ von $X^n - 1$ gilt: $(X^n - 1) \subseteq (\phi)$.\\
Daher ist $C loneqq (\phi) / (X^n - 1) loneqq \left\{\alpha\phi + (X^n - 1) \colon \alpha \in K[X]\right\}$ ein Untervektorraum von $K[X] / (X^n - 1)$. Man nennt $C$ den \highl{Zyklischer Code}{zyklischen Code} der Länge $n$ über $K$ mit \highl{Generatorpolynom} $\phi$.
\end{bem}
...
Sei $C loneqq (\phi) / (X^n - 1)$ wie oben und $l loneqq \deg\phi$, also $l \leq n$. Dann gilt: $\dim C = n - l$.
\end{satz}
...
%%%%%%%%%%%%%%%%%%%
\begin{beweis}
Schreibe $\phi = a_0 + a_1X + \ldots + a_lX^l \ (a_0, \ldots, a_l \in K, a_l \neq 0)$.\\
Dann sind 
\[\begin{array}{rcllllll}
\phi + (X^n - 1) & = & a_0 & + & a_1X + & & \ldots + a_lX^l & + (X^n - 1)\\
X\phi + (X^n - 1) & = & & & a_0X + a_1X^2 + & & \ldots + a_lX^{l + 1} & + (X^n - 1)\\
& \vdots \\
X^{n - l - 1}\phi + (X^n - 1) & = & & & & a_0X^{n - l - 1} + & \ldots + a_lX^{n - 1} & + (X^n - 1)
\end{array}\]
offenbar linear unabhängig in $C$.\\
Also: $\dim C \geq n - l$.\\
Wegen $\phi\mid X^n - 1$ existiert ein $\psi \in K[X]$ mit $X^n - 1 = \phi\psi$. Sei $\alpha \in (\phi)$, etwas $\alpha = \beta\phi$ mit $\beta \in K[X]$.\\
Division mit Rest: $\beta = \kappa\psi + \rho$ mit $\deg\rho < \deg\psi = n - l$.\\
Schreibe $\rho = r_0 + r_1X + \ldots + r_{n - l - 1}X^{n - l - 1} \ (r_i \in K)$.\\
Dann: \[
\begin{array}{lcl}
\alpha + (X^n - 1) & = & \beta\phi + (X^n - 1) = \kappa\underbrace{\phi\psi}_{= (X^n - 1)} + \rho\phi + (X^n - 1)\\
& = & \rho\phi + (X^n - 1)\\
& = & r_0\phi + r_1X\phi + \ldots + r_{n - l - 1}X^{n - l - 1}\phi + (X^n - 1)
\end{array}\]
Daher bilden $\phi + (X^n - 1), X\phi + (X^n - 1), \ldots, X^{n - l - 1}\phi + (X^n - 1)$ eine Basis von $C$.
\end{beweis}
...
\begin{enumerate}[(a)]
	...
	$q = 3, n = 11, \phi = X^5 + X^4 - X^3 + X^2 - 1 \in \F_3[X], C loneqq (\phi) / (X^{11} - 1)$. [Man kann zeigen, dass $\phi$ ein irreduzibler Teiler von $X^{11} - 1$ ist.]\\
	...
	$q = 2, n = 23, \phi loneqq X^{11} + X^{10} + X^6 + X^5 + X^4 + X^2 + 1 \in \F_2[X], C loneqq (\phi) / (X^n - 1)$. [Man kann zeigen, dass $\phi$ ein irreduzibler Teiler von $X^{23} - 1$ ist.]\\
