...
Daher ist $C loneqq (\varphi) / (X^n - 1) loneqq \left\{\alpha\varphi + (X^n - 1) \colon \alpha \in K[X]\right\}$ ein Untervektorraum von $K[X] / (X^n - 1)$. Man nennt $C$ den \highl{Zyklischer Code}{zyklischen Code} der Länge $n$ über $K$ mit \highl{Generatorpolynom} $\varphi$.
\end{bem}
...
Sei $C loneqq (\varphi) / (X^n - 1)$ wie oben und $l loneqq \deg\varphi$, also $l \leq n$. Dann gilt: $\dim C = n - l$.
\end{satz}
...
\begin{enumerate}[(a)]
	...
	$q = 3, n = 11, \varphi = X^5 + X^4 - X^3 + X^2 - 1 \in \F_3[X], C loneqq (\varphi) / (X^{11} - 1)$. [Man kann zeigen, dass $\varphi$ ein irreduzibler Teiler von $X^{11} - 1$ ist.]\\
	...
	$q = 2, n = 23, \varphi loneqq X^{11} + X^{10} + X^6 + X^5 + X^4 + X^2 + 1 \in \F_2[X], C loneqq (\varphi) / (X^n - 1)$. [Man kann zeigen, dass $\varphi$ ein irreduzibler Teiler von $X^{23} - 1$ ist.]\\
