...
Daher ist $C := \left(\varphi\right) / \left(X^n - 1\right) := \left\{\alpha\varphi + \left(X^n - 1\right) \colon \alpha \in K[X]\right\}$ ein Untervektorraum von $K[X] / \left(X^n - 1\right)$. Man nennt $C$ den \highl{Zyklischer Code}{zyklischen Code} der Länge $n$ über $K$ mit \highl{Generatorpolynom} $\varphi$.
\end{bem}
