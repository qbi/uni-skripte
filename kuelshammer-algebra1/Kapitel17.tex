...
%%%%%%%%%%%%%%%%%%%
\begin{beweis}[Witt]
Für $x\in D$ heißt $C_D(x)\coloneqq\left\{c\in D\colon cx=xc\right\}$ \highl{Schiefkörper!Zentralisator}{Zentralisator} von $x$ in $D$.\\
Offenbar ist $C_D(x)$ ein "`Teilschiefkörper"' von $D$.
Daher ist das Zentrum 
\[Z(D)\coloneqq\bigcap\limits_{x\in D}C_D(x)=\left\{z\in D\colon zx=xz \text{ für alle } x\in D\right\}\]
auch ein "`Teilschiefkörper"' von $D$.
Da $Z(D)$ kommutativ ist, ist $Z(D)$ ein Körper.
Daher ist $\abs{Z(D)}=:q$ eine Primzahlpotenz.\\
Offenbar ist $D$ ein $Z(D)$-Vektorraum, und für jedes $x\in D$ ist $C_D(x)$ ein $Z(D)$-Untervektorraum von $D$.

Wir setzen $n\coloneqq\dim_{Z(D)}D$ und $n_x\coloneqq\dim_{Z(D)}C_D(x)$ für $x\in D$.
Dann $\abs{D}=q^n$ und $\abs{C_D(x)}=q^{n_x}$ $(x\in D)$.\\
Seien $K_1,\ldots,K_t$ die Konjugationsklassen der multiplikativen
Gruppe $G\coloneqq U(D)=D\backslash\left\{0\right\}$, dabei sei $\abs{K_i}=1$ für $i=1,\ldots,s$ und $\abs{K_i}>1$ für $i=s+1,\ldots,t$.
...
Für $i=s+1,\ldots,t$ ist $q^{n_i}-1=\abs{C_G(x_i)}\Bigl|\abs{G}=q^n-1$, also:
...
Dann $\Phi_n(q) = \prod\limits_{j=1}^{\phi(n)}(q-\zeta_j)$, wobei $\zeta_1,\ldots,\zeta_{\phi(n)}$ die primitiven $n$-Einheitswurzeln in $\C$ sind.\\
Schreibe $\zeta_j=a_j+ib_j$ mit $a_j,b_j\in\R$ $(j=1,\ldots,\phi(n))$.
...
\begin{bem}\label{bem:17.2}
  \textsc{Frobenius} hat gezeigt, dass $\R, \C, \HQ$ die einzigen
  Schiefkörper sind, die gleichzeitig endliche Dimension über $\R$
  haben.
\end{bem}