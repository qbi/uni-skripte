...
Eine \highl{"Aquivalenzrelation}{Äquivalenzrelation} auf einer Menge $M$ ist eine Relation der Form $(M,M,R)$ mit folgenden Eigenschaften:
\begin{enumerate}[(i)]
  \item $x \in M \Rightarrow xRx$ (Reflexivität)
  \item $xRy \Rightarrow yRx$ (Symmetrie)
  \item $xRy \wedge yRz \Rightarrow xRZ$ (Transitivität)
\end{enumerate}
Wichtige Beispiele sind die Kongruenz von Dreiecken oder die Ähnlichkeit von Matrizen. Für eine beliebige Äquivalenzrelation $(M, M, R)$ und $x \in M$ heißt $[x]_R loneqq {y \in M \colon xRy}$ die Äquivalenzklasse von $x$ bezüglich $R$. Mit $M/R loneqq \left\{[x]_R \colon x \in M\right\}$ bezeichnet man die Menge aller Äquivalenzklassen von $M$ bezüglich $R$. Bekanntlich sind für $x, y \in M$ die folgenden Eigenschaften gleichbedeutent:
\begin{enumerate}[(1)]
  \item $xRy$
  \item $[x]_R = [y]_R$
  \item $[x]_R \cap [y]_R \neq \varnothing$
...
die Restklassen von $a$ modulo $m$. $( \text{z.\,B.: ist } 2 + 6\Z = \left\{\ldots, -10, -4, 2, 8, 14, \ldots\right\})$ Die Bezeichnung "`\highl{Restklasse}"' erklärt sich folgendermaßen: Division mit Rest liefert $q, r \in \Z$ mit $a = qm + r$ und $0 \leq r < m$. Wegen $m\mid qm = a - r$ ist dann $a \equiv r\pmod m$, d.\,h. $a + m\Z = r + m\Z$.\\ 
...
\begin{enumerate}[(i)]
  ...
  \item \begin{itemize}
  	\item["`$\Rightarrow$"'] Sei $ab \equiv ac\pmod m$, d.\,h. $m\mid ab - ac$, etwa $a(b - c) = mz$ für ein $z \in \Z$. Dann $\frac{a}{d}(b - c) = \frac{m}{d}z$ mit $d loneqq \ggt(a, m)$. Wegen $\ggt(\frac{a}{d}, \frac{m}{d}) = 1$ folgt aus (i): \[\frac{m}{d}\mid b - c \text{, d.\,h. } b \equiv c\pmod{\frac{m}{d}}\]
...
Seien $a, b \in \Z, m \in \N$ und $d loneqq \ggt(a, m)$. Genau dann hat die Kongruenz $ax \equiv b\pmod m$ eine Lösung $x \in \Z$, wenn $d\mid b$ gilt.\\
...
\begin{itemize}
	...
	\item["`$\Leftarrow$"'] Sei $d\mid b, a_0 loneqq \frac{a}{d}, m_0 loneqq \frac{m}{d}, b_0 loneqq \frac{b}{d}$. Wegen $\ggt(a_0, m_0) = 	1$ existiert $y_0, z_0 \in \Z$ mit $1 = a_0y_0 + m_0z_0$. Daher: $b = \underbrace{a_0b}_{= ab_0}y_0 + \underbrace{m_0b}_{= 	mb_0}z_0 \equiv ax_0\pmod{m}$ mit $x_0 = b_0y_0 (\Rightarrow \text{ Existenz einer Lösung})$\\
	Für $x \in \Z$ gilt: 
	\[\begin{array}{lcl}
...
Daher ist $k loneqq \prod\limits_{p \in \PZ}p^{\max{\left\{b_{1p}, \ldots, b_{np}\right\}}}$ das einzige kgV von $a_1, \ldots, a_n$. Dies zeigt, dass beliebige $a_1, \ldots, a_n \in \Z$ stets genau ein kgV haben. Man schreibt: $k = \kgV(a_1, \ldots, a_n)$.
\end{bem}
...
\begin{itemize}
	...
	Sei also $k \geq 2$, $d loneqq \ggt(m_1, m_2)$, $e loneqq \kgV(m_1, m_2)$.\\
	Schreibe $d = ym_1 + zm_2 (y, z \in \Z)$.\\
	Setze: $b loneqq a_1 + (a_2 - a_1)\frac{ym_1}{d} = a_2 + (a_1 - a_2)\frac{zm_2}{d}$.\\
