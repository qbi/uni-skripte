...
Sei $X$ ein reeller oder komplexer Vektorraum und $p$ ein reellwertiges, subadditves und betragshomogenes Funktional auf $X$ ($p(\alpha x) = |\alpha| p(x), \alpha \in \K$). Außerdem sei auf einem linearen Unterraum $E \subset X$ ein lineares Funktional $l \colon E \nach \C$ mit $|l(x)| \leq p(x) \quad \forall x \in E$ gegeben. Dann hat $l$ eine lineare Fortsetzung $L$ auf ganz $X$ mit $|L(x)| \leq p(x) \quad \forall x \in X$.
\end{satzs}
\begin{proof}
Siehe Vorlesung.
\end{proof}
