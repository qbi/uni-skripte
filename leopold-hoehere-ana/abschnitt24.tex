...
In diesem Abschnitt sei $X$ ein normierter Raum und $Y=\C$ oder $\R$. Man betrachtet nun $\L(X, \C)$ bzw. $\L(X, \R)$ - d.h. lineare und beschränkte Operatoren $T \colon X \nach \C$.\\
Da $\C$ Banachraum ist, folgt, dass auch $\L(X, \C)$ zum Banachraum wird.
\medskip

\begin{defis}
\highl[$X'$|see{Dualraum}]{$X'$} $:= \L(X, \C)$ heißt Raum der linearen und stetigen (beschränkten) \highl[Funktionale über $X$|see{Operator}]{Funktionale über $X$} oder auch \highl[Raum!Dualraum $X'$ von $X$]{Dualraum von $X$}. \highl[$x'$|see{Funktional}]{Elemente aus $X'$} werden Funktionale genannt und mit \highl[Operator!Funktional $x'$]{$x'$} bezeichnet.\\
Man schreibt auch $\norm{ x' | X' } = \sup_{\norm{x|X}=1}{\abs{x'(x)}} = \sup_{\norm{x|X}=1}{\abs{<x',x>}}$
\end{defis}
\medskip

Wie sieht $X'$ in konkreten Fällen aus?
\medskip
...
\begin{satz}
Sei $1 \leq p < \infty$ und $\frac{1}{p}+\frac{1}{q}=1$. Ferner sei $(\Omega, \Sigma, \mu)$ ein $\sigma$-endlicher Maßraum. Dann definiert $T \colon L_q(\Omega, \Sigma, \mu) \nach (L_p(\Omega, \Sigma, \mu))'$ mit $(Tg)(f):=\int_{\Omega} (fg)d\mu$ einen isometrischen Isomorphismus.
\end{satz}
\begin{proof}
...
\begin{bsp}
Für $f \in L_p([a,b])$ mit Lebesgue-Maß und $0<p<1$ ist $\norm{f|L_p}=\left( \int_a^b \abs{f(t)}^p dt\right)^{\frac{1}{p}} < \infty$. $L_p$ ist in diesem Fall ein Quasi-Banachraum, da es sich nur um eine Quasi-Norm handelt (Dreiecksungleichung mit einer Konstanten $>1$).\\
Es gilt $(L_p([a,b]))'=\{ 0 \}$.
\end{bsp}
\begin{proof}
Siehe Vorlesung.
...
\begin{bem}
Wie "`reichhaltig"' ist $X'$ also? Kann man durch $X'$ Elemente aus $X$ "`trennen"' - d.h. existiert für $x,y \in X$ mit $x \neq y$ ein $x' \in X'$ mit $x'(x) \neq x'(y)$ bzw. $x'(x-y) \neq 0$?\\
...
$\exists y_1 \in X \setminus E$. Damit ist $E \cup \{y_1\}$ linear unabhängig, denn sonst wäre $y_1 \in E$.\\
...
Setzt man $E_1:=\spn{E \cup \{y_1\}}$ so folgt $l_1 \colon E_1 \nach \C$ ist eine lineare Fortsetzung von $l$. Führt man diesen Prozess itterativ weiter so erhält man $\bigcup_{j=1}^{\infty} E_j$. Ist dies bereits der gesamte Raum $X$ (was in der Regel nicht der Fall ist) so ist man fertig. Der Fortsetzungsprozess ist für Räume diesen "`Typs"' also so möglich.
\end{bem}
\medskip

\begin{defi}
Eine Abbildung $p \colon X \nach \R$ heißt \highl[sublineare Abbildung]{sublinear} \gdwdef für beliebige $x,y \in X$ gilt:
\begin{align*}
...
Sei $X$ ein reeller oder komplexer Vektorraum und $p$ ein reellwertiges, subadditves und betragshomogenes Funktional auf $X$ ($p(\alpha x) = \abs{\alpha} p(x), \alpha \in \K$). Außerdem sei auf einem linearen Unterraum $E \subset X$ ein lineares Funktional $l \colon E \nach \C$ mit $\abs{l(x)} \leq p(x) \quad \forall x \in E$ gegeben. Dann hat $l$ eine lineare Fortsetzung $L$ auf ganz $X$ mit $\abs{L(x)} \leq p(x) \quad \forall x \in X$.
\end{satzs}
\begin{proof}
Siehe Vorlesung.
\end{proof}
...
\begin{folg}
Ist $X$ ein normierter Raum, so gilt für jedes $x \in X$ stets
\begin{gather*}
\norm{x} = \sup_{\norm{ L | X' } \leq 1} \abs{L(x)} = \sup_{L \in X' \setminus \{ \0 \}} \frac{\abs{L(x)}}{\norm{ L | X'}}
\end{gather*}
\end{folg}
\begin{proof}
Siehe Vorlesung.
...
\begin{folg}
Gilt für ein Element $x$ eines normierten Raumes $X$ stets $L(x)=0$ für jedes Funktional $L \in X'$, so ist $x = 0$.
\end{folg}
\begin{proof}
klar.
\end{proof}
...
\begin{bem}
Ist $X$ ein normierter Raum, so ist $X'=\L(X,\C)$ der Raum der linearen und stetigen Funktionale über $X$ (Dualraum) - ein Banachraum, da $\R$ bzw. $\C$ Banachräume sind. Man kann nun über $X'$ abermals lineare, stetige Funktionale definieren.
\end{bem}
\medskip

\begin{defi}
Für einen normierten Raum $X$ wird \highl[$X''$|see{Bidualraum}]{$X''$} $:=(X')'=\L(X', \C)$ der \highl[Raum!Bidualraum $X''$ von $X$]{Bidualraum von $X$} genannt.
\end{defi}
\medskip

\begin{lemmas}
Ist $x \in X$ und $\K=\R$ oder $\C$, so ist $i_x \colon X' \nach \K$, definiert durch $i_x (x'):=x'(x)$ für $x' \in X'$, ein lineares und stetiges Funktional über $X'$, d.h. $i_x \in X''$.
\end{lemmas}
\begin{proof}
Siehe Vorlesung.
\end{proof}
...
\begin{bem}
Es gilt wegen $\norm{x} = \sup_{\norm{x'| X'} \leq 1} \abs{x'(x)}$ (vgl. Folgerung 2.4.2) sogar $\norm{i_x |X''}=\norm{x}$.
\end{bem}
\medskip
...
\end{folgs}
\begin{proof}
Linearität klar: $J(x+y)(x') = i_{x+y}(x') = x'(x+y)=x'(x)+x'(y)=i_x(x') + i_y(x') = J(x)(x')+J(y)(x') \quad \forall x' \in X'$, damit folgt $J(x+y)=J(x)+J(y)$. \\
...
(iii) Ist X überdies vollständig (Banachraum) so ist $J(X)$ abgeschlossen (im topol. Sinne) und damit auch Banachraum.
\end{lemmas}
\begin{proof}
(i) Wegen Satz 2.3.2, da $\C$ vollständig ist.\\
(ii) Siehe obige Folgerung und Lemma aus Abschnitt 2.3.\\
(iii) Wähle eine beliebige Cauchy-Folge $(x''_k)_{k=1}^{\infty}$ in $J(X) \subset X''$. Dann existiert wegen (i) ein Grenzelement $\widehat{x''} \in X''$. Die Linearität und Normerhaltung von $J$ liefern ferner, dass auch die Urbild-Folge $(x_k)_{k=1}^{\infty} \subset X$ eine Cauchy-Folge ist:\\ $\norm{x_m-x_n|X}=\norm{J(x_m-x_n)|X''}=\norm{J(x_m)-J(x_n)|X''}=\norm{x''_m-x''_n|X''}<\epsilon$.\\
...
$c_0$ mit der Supremumsnorm $\norm{\cdot}_{\infty}$ ist ein Banachraum. Frühere Überlegungen zeigten, dass $(c_0)' \cong l_1$ und $(l_1)' \cong l_{\infty}$ gilt. Damit folgt unmittelbar $(c_0)'' \cong l_\infty$.\\
...
Für $1<p<\infty$ gilt $(l_p)' \cong l_q$ mit $\frac{1}{p} + \frac{1}{q}=1$ (siehe oben). Dann ist offensichtlich $1<q<\infty$ und damit nach gleichem Argument $(l_p')'=(l_p)'' \cong (l_q)' \cong l_p$. Daher ist $l_p'' \cong l_p$.\\
D.h. mit $x=(\xi_k)_{k=1}^{\infty} \in l_p$ und $y=(\eta_k)_{k=1}^{\infty} \in l_q \cong (l_p)'$ ist $J_{l_p}(x)(y)=\sum_{k=1}^{\infty} \xi_k \eta_k$.
\end{bsp}
\medskip

\begin{defis}
Ist $X$ Banachraum und die Abbildung $J$ surjektiv so heißt $X$ \highl[Raum!reflexiv (Banachraum)]{reflexiv} oder selbstbezüglich, d.h. es gilt $X'' \cong X$.
\end{defis}
\medskip
...
Sei $1 < p < \infty$. Man betrachte die Folge der Einheitsvektoren $(e_j)_{j=1}^{\infty} \subset l_p$ mit \\ $e_j=(0,\ldots,0,1,0,\ldots)=(\xi_k^{(j)})_{k=1}^{\infty}$ und wähle ferner ein $x'=(\eta_k)_{k=1}^{\infty} \in (l_p)' \cong l_q$. Dann gilt (siehe früher): $x'(e_j)=\sum_{k=1}^{\infty} \eta_k \xi_k^{(j)}=\eta_j$. Damit ist $\lim_{j \nach \infty} x'(e_j)=\lim_{j \nach \infty} \eta_j=0$, wegen $\left( \sum_{k=1}^{\infty} \abs{\eta_k}^q \right)^{\frac{1}{q}}<\infty$. Also gilt $\wlim{j \nach \infty} e_j = 0$, aber $\norm{e_j}=1$, d.h. es existiert kein $\lim_{j \nach \infty} e_j$.
\end{bsp}
\medskip
