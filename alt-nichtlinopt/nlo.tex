...
\begin{gather*}\tag{3}
-(\frac 12 -\delta)\nabla f(x^{(k)})^Td^{(k)}\geq (\frac 12-\delta)\beta_1 \norm{\nabla f(x^{(k)})}^2 \geq (\frac 12-\delta)\beta_1 \beta_2 ^{-2}\norm{d^{(k)}}^2.
\end{gather*}
Den Term auf der rechten Seite von (2) kann man wie folgt abschätzen

$ \frac 12 (d^{(k)})^T \left[f''(x^{(k)}+\theta_kd^{(k)})-f''(x^{(k)}\right]d^{(k)} \leq \norm{f''(x^{(k)}+\theta_kd^{(k)})-f''(x^{(k)})} \cdot \norm{d^{(k)}}^2$
...
$$\sigma_k \leq \frac {\alpha_2 \norm{d^{(k)}}^2}{\alpha_1 \norm{d^{(k)}}^2}=\frac {\alpha_2}{\alpha_1}$$
