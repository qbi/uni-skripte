...
\usepackage{amsfonts} %math. Symbole und Umgebungen
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{amsmath} 
\usepackage{hyperref}
...
Wir definieren $f\colon \R^2\rightarrow\R$ durch \begin{align*}f(x)=f(x_1,x_2)&=\sum_{i=1}^m(g(\xi_i;x_1,x_2)-\eta_i)^2\\
...
$f\colon D\rightarrow \R, D\subset\R^n$ offen, $K\subset D$ kompakt, $f$ stetig auf $D$
Satz von Weierstraß
\paragraph[1.2.1]{Def} Sei $f\colon  D\rightarrow\R, D\subset\R^n, \alpha\in\R.$ Die Mengen $N(f,x)=\{x\in D|f(x)\leq\alpha\}$ heißen \textbf{Niveau-Mengen} von $f$.
\begin{satz}[1.1.2] Sei $D\subset\R^n$. Die Fkt. $f\colon D\rightarrow\R$ sei stetig und für ein $\omega\in D$ sei die Niveaumenge $N(f,f(\omega))=\{x\in D| f(x)\leq f(\omega)\}$ kompakt. Dann gibt es (mindestens) ein globales Minimum von $f$ auf $D$.
...
\begin{lemma}[4.3.1] Sei $D\subseteq \R^n$ offen, $C\subseteq D$ konvex, $F\colon D\rightarrow \R^n$ differenzierbar und $F'$ sei Lipschitzstetig auf $C$, d.h. mit einer Konstante $L\geq 0$ gilt $||F'(x)-F'(y)||\leq L||x-y|| \quad \forall x,y\in C.$ Dann gilt: $||F(x)-F(y)-F'(y)(x-y)||\leq \frac L2 ||x-y||^2$
...
\sigma_E \leq -\frac{\nabla f(x)^Td}{L||d||^2}=\colon \tilde \sigma \quad \text{d.h. $\sigma_E$ erfüllt (4.4.8))}
\end{gather*}
...
\exists L_2>0,\quad \exists r>0\colon  \quad || f''(x)-f''(y)||\leq L_2||x-y||\quad \forall x,y\in B(\tilde x, r)
\end{gather*}
...
Weiter gilt wegen $x^{(k)}\rightarrow \tilde x, x^{(k)}+\lambda_k d^{(k)}\rightarrow \tilde x\colon  \qquad \sigma_k\rightarrow 1$. 
