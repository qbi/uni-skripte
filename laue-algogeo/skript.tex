...
Falls $S \subseteq \R^d, p \in \operatorname{conv}(S)$ gilt, so existiert eine Teilmenge $A \subseteq S$ mit $\abs{A} \leq d+1$, so dass $p \in \operatorname{conv}(A)$.
\end{thrm}
