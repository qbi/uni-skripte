...
% \g ist das gleiche wie \help nur für offene Aufgaben
...
\[c := \sum_{i \in I} \lambda_i = - \sum_{i \in [1..n] \ I} \lambda_i\]
...
\begin{satz}
Haben alle Dreieke in K die lokale Delaunay-Eigenschaft, dann ist K die (gewichtete) Delaunaytriangulierung
\end{satz}
...
%Crust(p)
%1. Berechne das Voronoi-Diagramm von $p$. $V:=$ Menge der Voronoi-Vertices.
...
\begin{lemma}
\label{crust2}
Unter den Vorraussetzungen von \ref{crust1} ist keine nicht-korrekte Kante in $\operatorname{crust}(P)$.
...
\caption{Skizze zum Beweis des Small Angle-Lemmas (\ref{smallangle})} 
\end{figure} 
...
\caption{Skizze zum Beweis von Lemma \ref{crust2}} 
\end{figure} 
...
Sei o.b.d.A. $v$ auf gleicher Seite von $\Sigma$ wie $m_1$. Wir sind interessiert an
\[
...
Die Menge $C_p := \{y \in V_p | \measuredangle (\overrightarrow{py}, v_p) \geq \frac{3}{8} \pi\}$ heißt Co-Kegel (Cocone) von $p$.
...
$K_0 \subset K_1 \subset ... \subset K_n$ ("$\subset$" heißt, alle Simplices in $K_i$ sind auch in $K_{i+1}$ enthalten)
...
Sei $p \in P$ und $\sigma_1,...,\sigma_k$ seien alle Delaunay-Simplices inzident zu $p$, geordnet so dass $\alpha(\sigma_i) \leq \alpha(\sigma_{i+1})$

Seien $\alpha_p^+ > \alpha_p^-$ Werte aus $\alpha(\sigma_1),...,\alpha(\sigma_k)$
...
\begin{beob}
$\hat \alpha(\sigma) = \max_{i = 1,...,k} \inf\{\hat \alpha | \alpha_{p_i}(\hat \alpha) \geq \alpha(\sigma)\}$ wobei $\sigma = conv(p_1,...,p_k), p_i \in P$
...
$p \in P$, $\sigma_1, ..., \sigma_k$ Delaunay Simplices inzident zu $p$ in Delaunay Triangulierung von $P$. Sei $\alpha(\sigma_i)$ der Alphawert für den $\sigma_i$ im Alpha Shape von $P$ auftaucht. Annahme $\alpha(\sigma_i) \leq \alpha(\sigma_{i+1})$
...
\item[zu (3)] Beobachtung: $\hat \alpha(\sigma)$ (Wert, bei dem $\sigma$ im konformen Alpha Shape auftaucht.) $= max_{p_1,...,p_k} inf\{\hat \alpha | \alpha_{p_i}(\hat \alpha) \geq \alpha(\sigma)  \}$
wobei $p_1,...,p_k$ die Vertices des Simplex $\sigma$ sind.
\end{enumerate}
...
\begin{proof}
Sei $\sigma$ ein $\alpha$-kritischer Simplex mit Vertices $p_1, ..., p_k$, d.h. $\sigma$ ist $\alpha$-spät (erscheint also später als alle seine Faces im Alpha Shape) und $\alpha$-früh (d.h. erscheint vor all seinen Co-Faces im Alpha Shape). Zu zeigen: $\alpha$ ist $\hat \alpha$-spät und $\hat \alpha$-früh.
...
\leq max_{p_j \in \{p_1,...,p_k\}} inf \{\hat \alpha | \alpha_{p_j}(\hat \alpha) \geq \alpha(\sigma)\} = \hat \alpha(\sigma) \text{=> $\sigma$ ist $\hat \alpha$-spät}
\]


(2) $\alpha$-früh. Reicht zu zeigen für $k$-Dimensionale Co-Faces von $\sigma$. Sei $\tau$ so ein Co-Face mit zusätzlichem Vertex $p$.
\[
\hat \alpha(\sigma) = max_{p_j \in \{p_1,...,p_k\}} inf \{\hat \alpha | \alpha_{p_j}(\hat \alpha) \geq \alpha(\sigma)\}
< \hat \alpha(\sigma) = max_{p_j \in \{p_1,...,p_k\}} inf \{\hat \alpha | \alpha_{p_j}(\hat \alpha) \geq \alpha(\tau)\} (Weil \alpha(\sigma) < \alpha(\tau))\text{, da $\sigma \alpha$-früh ist.}
\leq < \hat \alpha(\sigma) = max_{p_j \in \{p_1,...,p_{k+1}\}} inf \{\hat \alpha | \alpha_{p_j}(\hat \alpha) \geq \alpha(\tau)\} = \hat \alpha (\tau) \text{=> $\sigma$ ist $\hat \alpha$-früh.}
\]
...
\begin{lemma}
Sei $\Sigma$ eine glatte Fläche, $P$ sei ein $\epsilon$-Sampling von $\Sigma$. Dann ist für $\hat \alpha \geq \eta := \frac{\epsilon}{1-\epsilon}$ die Delaunay Triangulierung von $P$  eingeschränkt auf $\Sigma$ (d.h. $D_\Sigma(P))$ im konformen Alpha Shape zu 2 enthalten.
\end{lemma}
...
\begin{defini}
[Homeomorphismus] $f: X \mapsto Y$ heißt homeomorphismus, falls $f$ bijektiv ist und sowohl $f$ als auch $f^{-1}$ stetig sind.
\end{defini}

\begin{defini}[Homeomorph]
$X,Y$ heißen homeomorph, falls es homeomorphismus $f: X \mapsto Y$ gibt.  [$X$ und $Y$ sind im topologischen Sinne gleich]
\end{defini}
...
\begin{defini}
[Homotopyäquivalenz] Seien X,Y topologische Räume. Zwei stetige Abbildungen $f,g: X \mapsto Y$ heißen homotop zueinander, falls es eine stetige Abbildung
\[H: X \times [0,1] \mapsto Y\] gibt, so dass $H(x,0) = f(x)$ und $H(x,1) = g(x) \forall x \in X$.
\end{defini}

\begin{defini}[Homotopyäquivalent]
$X,Y$ sind homotopyäquivalent, falls es stetige Abbildungen $f:X\mapsto Y$ und $g:X\mapsto Y$ gibt, so dass $g \circ f$ homotop ist zu $ID_x$ und $f \circ g$ homotop ist zu $ID_y$.
$ID_x:X \mapsto X$ identische Abbildung $x \mapsto x$.
\end{defini}
...
\[a,b \in C_k: a+b := (a \cup b)\setminus (a \cap b) \text{ (symmetrische Differenz)}\]
\end{defini}
...
\[
a,b \in C_k: a+b := (a \cup b)\setminus (a \cap b)
\] (Addition Modulo 2)
\end{defini}
...
Achtung bei den Randfällen: $\log(b_{k-1}) := 0$
d.h. Eulerzahle ist alternierende Summe der Bettizahlen.

=> Wenn Bettizah homotopieinvariante, dann auch Eulerzahl.
