...
Projetion von 4D Tetraeder in den $\R^3$
...
CL\{ x \in \R^d \\ \Sigma | x hat mindestens 2 nächste Nachbarn in \Sigma \}$
\end{defini}
...
\begin{enumerate}
\item $f(x) \leq \frac{1}{1 - \epsilon} f(x)$
...
\section{Korrektheitsbeweis für Crust Algorithmus}
...
Crust gubt keine nicht-korrekten Kanten aus für $\epsilon < \frac{1}{5}$. Annahme für widerspruch $pq$ nicht-korrekte Kante in $\operatorname{Crust}(p)$.

=> Es gibt Ball $D$ mit $p,q \in \partial D$ (Rand von D) und $D \cap P = \emptyset, D \cap \{v | \text{ $v$ Voronoi Vertex von $P$ }\} = \emptyset$
...
$B \cap V \neq \emptyset$ [$V$ Voronoiverices des Voronoi-Diagramms von $P$]
...
Homeomorphismus: Bijektive Abbildung zwischen zwei Flächen, die in beide Richtungen stetig ist
...
Mann nennt $\overrightarrow{vp} = p^+ - p^-$ den Pol-Vektor von $p$.
...
Die Menge $C_p := \{y \in V_p | \measuredangle (\overrightarrow{py}, v_p) \geq \frac{3}{8} \pi\}$ heißt Co-Kegel (Cocone) vpn $p$.
