...
\begin{folg}
Alle Punkte in einem $k$-dimensionalen Voronoi-Face (Voronoi $k$-Face) sind gleich abständig zu $d+1-k$ Punken in $p$.
\end{folg}
...
\begin{itemize}
\item k-dimensionalen Delauanay-Simplex
...
Das Linienegment $qp$ schneidet Dreiecke $t_1, t_2, ... , t_k$ der Triangulierung.
Dazu folge von gewichteten Punkten $S_1, S_2, ... , S_k$ mit Gewichten $w_1, w_2, ... , w_k$
$w_i = \text{Radius der Umkreisscheibe von} t_i$
Damit ergeben sich eine Folge von Power-Abständen $\Pi_1(p), ... , \Pi_k()$
...
\begin{itemize}
 ...
 \item 	positiv, falls ausserhalb der Kreisscheibe
\end{itemize}
...
Der Lawson-Flip-Algorithmus muss entscheiden, ob $bc$ die lokale Delaunay-Eigenschaft ha, d.h. ob $d$ in der Umkreisscheib von $abc$ enthalten ist. Dazu benötigt man den IN\_CIRCLE\_TEST.
...
Idee: Wenn immer eine Kante $bc$ nicht die lokale Delaunay-Eigenschaft hat, dann "Flip" zu ab
...
\todo{versteh das nicht.. evtl. abschreibfehler?}
...
Starten beim keinsten Punkte. Gehen zum übernächsten Punkt. (zweimal Next)
....
...
\begin{algorithm}
	...
	Sortiere $\{p_1 ... p_k\} lexikographisch$\;
...
%    \KwIn{$P \subset \R^2, P = \{p_1, p_2, ... , p_k \}$}
%    \KwOut{Triangulierung und konvexen Hülle}
	drehe Reihenfolge von $p_1 ... p_k$ um\;
	LowerHull(p)\;
\end{algorithm}
...
\begin{algorithm}
    \dontprintsemicolon
    \caption{Sortieren mit konexen Hüllen}
    \KwIn{$a_1 ... a_n \in \R \geq 0;  a \geq 0$}
    \KwOut{Zahlen in sortierter Reihenfolge -> $\Omega(n \log n)$}
	konstruiere Punkte $p_i := (a_i, a_i^2)$\;
	berechne die Konvexe Hülle\;
\end{algorithm}

%gegeben: $a_1 ... a_n \in \R \geq 0;  a \geq 0$
%ausgabe: Zahlen in sortierter Reihenfolge -> $\Omega(n \log n)$
%konstruiere Punkte $p_i := (a_i, a_i^2)$
Konvexe Hülle würde die Zahlen $a_1 .. a_n$ sortieren. Damit ist die untere Schranke $\Omega(n \log n)$ wenn $n$ Punkte auf der konvexen Hülle liegen.
...
$P = \{p_1, ... , p_n\}, S_1 = \{p_1\}, S_n = \{p1, ... , p_n\} = P, P_i = \{1, ..., p_i\}$
...
$xyz >> max\{ \norm{ p_i - p_j } | p_i, p_j \in P \} \forall i = 1, ..., n$.
...
\For{i=1 TO n}
{
	...
	\While{$\exists$ Edge $bc$ WITHOUT "lokale Delaunay-Eigenschaft"}
	{
		Flip $bc$ to $ad$\;
	}	
}
...
Der Algorithmus untertei sich in 2 Teilprobleme: Das Lokalisierungsproblem in Zeile 2 und das Update-Problem in Zeile 3-6. Das Update besteht aus einer Split-Operation (genannt "1-3-Flip")
und mehreren Flip-Operationen (genannt "2-2-Flip").
...
In diesem Fall bedeutet das, dass wir die Reihenfolge der Punkte $p_1, ... , p_n$ zufällig wählen.
...
Gerichteter, anzyklischer Graph. Abgeleitet aus Split und Flip.
...
\begin{enumerate}
 \item 1-Skelett der Delaunay-Triangulierung. (D.h. der planare Graph der Delaunay-Kanten und Verticies.)
